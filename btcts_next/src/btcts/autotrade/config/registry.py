...
from .models import ParameterSet, ParameterSetBundle, ParameterSetBundleRegistry, ParameterSetRegistry
...
def write_parameter_bundle(path: Path, bundle: ParameterSetBundle) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(bundle.to_dict(), ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")


def write_bundle_registry(path: Path, registry: ParameterSetBundleRegistry) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(registry.to_dict(), ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
