...
def build_market_regime_calibration_read_model(
    *,
    ...
    trust_view = {
        ...
        "current_primary_parameter_set_count": _safe_int(calibration_trust.get("current_primary_parameter_set_count")),
        ...
        "current_primary_cohort_started_at": str(calibration_trust.get("current_primary_cohort_started_at") or daily.get("current_primary_cohort_started_at") or ""),
        "current_primary_row_count": _safe_int(calibration_trust.get("current_primary_row_count")),
        "trusted_legacy_reference_row_count": _safe_int(calibration_trust.get("trusted_legacy_reference_row_count")),
        "compatibility_reference_row_count": _safe_int(calibration_trust.get("compatibility_reference_row_count")),
        "promotion_candidates_use_current_primary_cohort_only": bool(calibration_trust.get("promotion_candidates_use_current_primary_cohort_only", False)),
    }
    return {
        ...
        "primary_current": _bucket_view(daily.get("primary_current") if isinstance(daily.get("primary_current"), Mapping) else {}),
        "trusted_legacy_reference": _bucket_view(daily.get("trusted_legacy_reference") if isinstance(daily.get("trusted_legacy_reference"), Mapping) else {}),
        "compatibility_reference": _bucket_view(daily.get("compatibility_reference") if isinstance(daily.get("compatibility_reference"), Mapping) else fallback_bucket),
        "compatibility_overall": _bucket_view(daily.get("compatibility_overall") if isinstance(daily.get("compatibility_overall"), Mapping) else daily.get("overall") if isinstance(daily.get("overall"), Mapping) else {}),
