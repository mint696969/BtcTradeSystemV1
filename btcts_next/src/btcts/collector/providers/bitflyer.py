...
def fetch_executions(*, product_code: str = "BTC_JPY", count: int = 50) -> HttpResult:
    ...
    return _get_json(url, params={"product_code": product_code, "count": int(count)})
...
def compact_executions(raw: Dict[str, Any], *, limit: int = 50) -> Dict[str, Any]:
    """
    約定を軽量化。必要最低限だけ（ts, price, size, side）
    """
    ...
    return {
        ...
        "exchange": "bitflyer",
        "topic": "trades",
        "product_code": (out[0].get("product_code") if out and isinstance(out[0], dict) else None),
        "items": out,
    }
