# path: ./btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_panel.py
# desc: PS-Q18K pure-data status rows for latest_prediction_summary_widget mapped-payload render-disabled packet validation. No Streamlit import, no Q18J checker invocation, no component packet builder invocation, no source read, no D-hot discovery, no writes.
...
LATEST_PREDICTION_SUMMARY_WIDGET_MAPPED_PAYLOAD_RENDER_DISABLED_PACKET_STATUS_PANEL_VERSION = "prediction_warroom_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_panel.ps_q18k.v1"
...
def build_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_rows(*, supplied_validation_report: Mapping[str, Any] | Any | None = None) -> list[dict[str, Any]]:
    ...
    mapped_generated_at = str(report.get("mapped_source_generated_at") or "")
    mapped_artifact_ref = str(report.get("mapped_source_artifact_ref") or "")
    component_generated_at = str(report.get("component_source_generated_at") or "")
    component_artifact_ref = str(report.get("component_source_artifact_ref") or "")
    status_state = "observed_mapped_payload_render_disabled_packet_valid_status_only" if observed_ok else "no_validation_report_supplied_status_only"
    return [
        _status_row("mapped_payload_render_disabled_validation_report", status_state, observed_ok, "Q18J validation report status; WarRoom does not run Q18J."),
        ...
        _status_row("mapped_source_generated_at", "observed" if mapped_generated_at else "not_supplied", mapped_generated_at, "Mapped payload generated_at displayed only."),
        _status_row("mapped_source_artifact_ref", "observed" if mapped_artifact_ref else "not_supplied", mapped_artifact_ref, "Mapped payload source artifact ref displayed only."),
        _status_row("component_source_generated_at", "observed" if component_generated_at else "not_supplied", component_generated_at, "Component packet generated_at displayed only."),
        _status_row("component_source_artifact_ref", "observed" if component_artifact_ref else "not_supplied", component_artifact_ref, "Component packet source artifact ref displayed only."),
        _status_row("render_boundary", "mapped_payload_render_disabled_status_only", "q18j_mount=false; component_builder_mount=false; streamlit_render=false; real_render=false; actual_read=false", "Boundary row; WarRoom status mount does not validate, render, or read."),
    ]


def build_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_packet(*, supplied_validation_report: Mapping[str, Any] | Any | None = None) -> dict[str, Any]:
    report = _as_mapping(supplied_validation_report)
    rows = build_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_rows(supplied_validation_report=report)
    failures: list[str] = []
    if len(rows) != 12:
    ...
    return {
        ...
        "status_mount_state": "latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_rows_ready_no_validation_or_rendering",
