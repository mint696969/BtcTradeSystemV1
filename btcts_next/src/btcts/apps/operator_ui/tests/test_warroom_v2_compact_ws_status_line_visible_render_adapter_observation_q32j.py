# path: ./btcts_next/src/btcts/apps/operator_ui/tests/test_warroom_v2_compact_ws_status_line_visible_render_adapter_observation_q32j.py
# desc: PS-Q32J guards for hidden session_state compact WS status line visible render adapter observation. No UI mount and no socket open.
...
DOC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q32J_WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_RENDER_ADAPTER_OBSERVATION_TO_HIDDEN_SESSION_STATE_NO_UI_MOUNT_2026-07-03.md"
...
def test_q32j_contract_is_hidden_state_for_visible_render_adapter() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_render_adapter_observation_contract()
    assert packet["state_key"] == WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_RENDER_ADAPTER_OBSERVATION_STATE_KEY
    assert packet["observation_kind"] == "warroom_v2_hidden_compact_ws_status_line_visible_render_adapter_observation_packet"
...
def test_q32j_default_hidden_observation_keeps_render_adapter_unmounted() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_render_adapter_observation_packet(fragment_summary={"fragment_widget_count": 9})
    assert packet["packet_kind"] == "warroom_v2_compact_ws_status_line_visible_render_adapter_observation_packet"
...
def test_q32j_hidden_observation_can_record_ready_payload_but_still_not_mount() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_render_adapter_observation_packet(
    ...
    assert packet["adapter_status"] == "compact_ws_status_line_visible_render_payload_ready_not_rendered"
    assert packet["upstream_visible_mount_ready"] is True
...
def test_q32j_warroom_page_records_hidden_visible_render_adapter_observation_only() -> None:
    text = WARROOM_PAGE.read_text(encoding="utf-8-sig")
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_RENDER_ADAPTER_OBSERVATION_STATE_KEY" in text
    assert "warroom_v2_compact_ws_status_line_visible_render_adapter_observation_q32j" in text
    assert "build_warroom_v2_compact_ws_status_line_visible_render_adapter_observation_packet" in text
    assert "st.session_state[WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_RENDER_ADAPTER_OBSERVATION_STATE_KEY]" in text
    assert "build_warroom_v2_compact_ws_status_line_visible_render_adapter_packet" not in text
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_RENDER_ADAPTER_VERSION" not in text
    forbidden_visible_labels = (
        "compact WS status line visible render",
        "WS状態: WS未接続（準備中）",
...
def test_q32j_doc_modules_preserve_no_side_effect_boundary() -> None:
    text = DOC.read_text(encoding="utf-8-sig")
    assert "state_key=warroom_v2_compact_ws_status_line_visible_render_adapter_observation_q32j" in text
    assert "default_adapter_status=compact_ws_status_line_visible_render_hidden_default" in text
