# path: ./btcts_next/src/btcts/apps/operator_ui/tests/test_warroom_v2_compact_ws_status_line_visible_mount_gate_observation_q32h.py
# desc: PS-Q32H guards for hidden session_state compact WS status line visible mount gate observation. No UI mount and no socket open.
...
from btcts.apps.operator_ui.prediction_warroom.v2 import (  # noqa: E402
    ...
    build_warroom_v2_compact_ws_status_line_visible_mount_gate_observation_contract,
    build_warroom_v2_compact_ws_status_line_visible_mount_gate_observation_packet,
)

REPO_ROOT = Path(__file__).resolve().parents[6]
DOC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q32H_WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_MOUNT_GATE_OBSERVATION_TO_HIDDEN_SESSION_STATE_NO_UI_MOUNT_2026-07-03.md"
...
def test_q32h_contract_is_hidden_state_for_visible_mount_gate() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_mount_gate_observation_contract()
    assert packet["state_key"] == WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_MOUNT_GATE_OBSERVATION_STATE_KEY
    assert packet["observation_kind"] == "warroom_v2_hidden_compact_ws_status_line_visible_mount_gate_observation_packet"
...
def test_q32h_default_hidden_observation_keeps_visible_mount_unmounted() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_mount_gate_observation_packet(fragment_summary={"fragment_widget_count": 9})
    assert packet["packet_kind"] == "warroom_v2_compact_ws_status_line_visible_mount_gate_observation_packet"
...
def test_q32h_hidden_observation_can_record_ready_mount_gate_but_still_not_mount() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_mount_gate_observation_packet(
    ...
    assert packet["gate_status"] == "compact_ws_status_line_visible_mount_ready_not_mounted"
    assert packet["status_gate_ready"] is True
    assert packet["visible_mount_ready_for_future_mount"] is True
    assert packet["status_line_visible_now"] is False
    assert packet["status_line_mounted_now"] is False
...
def test_q32h_warroom_page_records_hidden_visible_mount_gate_observation_only() -> None:
    text = WARROOM_PAGE.read_text(encoding="utf-8-sig")
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_MOUNT_GATE_OBSERVATION_STATE_KEY" in text
    assert "warroom_v2_compact_ws_status_line_visible_mount_gate_observation_q32h" in text
    assert "build_warroom_v2_compact_ws_status_line_visible_mount_gate_observation_packet" in text
    assert "st.session_state[WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_MOUNT_GATE_OBSERVATION_STATE_KEY]" in text
    assert "build_warroom_v2_compact_ws_status_line_visible_mount_gate_packet" not in text
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_MOUNT_GATE_VERSION" not in text
...
def test_q32h_doc_modules_preserve_no_side_effect_boundary() -> None:
    text = DOC.read_text(encoding="utf-8-sig")
    assert "state_key=warroom_v2_compact_ws_status_line_visible_mount_gate_observation_q32h" in text
    assert "default_gate_status=compact_ws_status_line_visible_mount_hidden_default" in text
