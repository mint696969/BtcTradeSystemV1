# path: ./btcts_next/src/btcts/apps/operator_ui/tests/test_warroom_v2_compact_ws_status_line_visible_render_mount_gate_q32k.py
# desc: PS-Q32K guards for compact WS status line visible render mount gate. Default-off; no UI mount and no socket open.
...
from btcts.apps.operator_ui.prediction_warroom.v2 import (  # noqa: E402
    ...
    build_warroom_v2_compact_ws_status_line_visible_render_mount_gate_packet,
)

REPO_ROOT = Path(__file__).resolve().parents[6]
DOC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q32K_WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_RENDER_MOUNT_GATE_DEFAULT_OFF_2026-07-03.md"
...
def test_q32k_contract_is_default_off_visible_render_mount_gate() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_render_mount_gate_contract()
    assert packet["gate_kind"] == "warroom_v2_compact_ws_status_line_visible_render_mount_gate_default_off"
    ...
    assert packet["ready_gate_status"] == "compact_ws_status_line_visible_render_mount_ready_not_mounted"
...
def test_q32k_default_packet_keeps_mount_gate_hidden_but_preserves_payload() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_render_mount_gate_packet(fragment_summary={"fragment_widget_count": 9})
    assert packet["packet_kind"] == "warroom_v2_compact_ws_status_line_visible_render_mount_gate_packet"
...
def test_q32k_ready_requires_mount_ack_and_upstream_payload_ready_but_still_not_mounted() -> None:
    blocked = build_warroom_v2_compact_ws_status_line_visible_render_mount_gate_packet(
    ...
    assert blocked["gate_status"] == "compact_ws_status_line_visible_render_mount_blocked_payload_not_ready"
    ready = build_warroom_v2_compact_ws_status_line_visible_render_mount_gate_packet(
        visible_render_mount_requested=True,
        operator_visible_render_mount_ack=True,
    ...
    assert ready["gate_status"] == "compact_ws_status_line_visible_render_mount_ready_not_mounted"
    assert ready["upstream_render_payload_ready"] is True
...
def test_q32k_mount_payload_exposes_exact_japanese_compact_items() -> None:
    packet = build_warroom_v2_compact_ws_status_line_visible_render_mount_gate_packet()
...
def test_q32k_doc_modules_and_warroom_page_preserve_no_side_effect_boundary() -> None:
    text = DOC.read_text(encoding="utf-8-sig")
    assert "gate_kind=warroom_v2_compact_ws_status_line_visible_render_mount_gate_default_off" in text
    ...
    assert "build_warroom_v2_compact_ws_status_line_visible_render_mount_gate_packet" not in page
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_VISIBLE_RENDER_MOUNT_GATE_VERSION" not in page
    # Later slices may record hidden observation state in WarRoom page, but must not mount
    # the Q32K direct render mount gate packet/version or any visible status-line label.
    visible_status_labels = (
        "compact WS status line visible render mount",
        "WS状態: WS未接続（準備中）",
