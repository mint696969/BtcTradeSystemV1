# path: ./btcts_next/src/btcts/apps/operator_ui/tests/test_ai_signal_state.py
# desc: Verify ai_signal_state adapts shared market signal context for panel use.
...
import btcts.apps.operator_ui.components.ai_signal_state as state_mod  # noqa: E402
...
def main() -> int:
    ...
    try:
        state_mod.load_market_signal_context = lambda: {
            ...
            "event_ts": "2026-04-14T13:20:00Z",
            "regime": "trend_up",
            "best_strategy": "microstructure_v1",
            "data_source": "live_canonical",
        }

        live_state = state_mod.build_ai_signal_state()
        ...
        assert live_state["replay_ts"] == "2026-04-14T13:20:00Z"
        ...
        state_mod.load_market_signal_context = lambda: {
            ...
            "event_ts": "2026-04-14T13:25:00Z",
            "regime": "range",
            "best_strategy": "baseline_none",
            "data_source": "replay_research",
        }

        replay_state = state_mod.build_ai_signal_state()
        ...
        assert replay_state["replay_ts"] == "2026-04-14T13:25:00Z"
        assert replay_state["delta"] == -0.2

        state_mod.load_market_signal_context = lambda: None
        assert state_mod.build_ai_signal_state() is None
    finally:
