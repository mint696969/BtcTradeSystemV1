# path: ./btcts_next/src/btcts/apps/operator_ui/tests/test_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_q32p.py
# desc: PS-Q32P guards for hidden session_state compact WS status line Streamlit visible mount gate observation. No UI mount and no socket open.
...
from btcts.apps.operator_ui.prediction_warroom.v2 import (  # noqa: E402
    ...
    build_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_contract,
    build_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_packet,
)

REPO_ROOT = Path(__file__).resolve().parents[6]
DOC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q32P_WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_VISIBLE_MOUNT_GATE_OBSERVATION_TO_HIDDEN_SESSION_STATE_NO_UI_MOUNT_2026-07-03.md"
...
def test_q32p_contract_is_hidden_state_for_visible_mount_gate() -> None:
    packet = build_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_contract()
    assert packet["state_key"] == WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_VISIBLE_MOUNT_GATE_OBSERVATION_STATE_KEY
    assert packet["observation_kind"] == "warroom_v2_hidden_compact_ws_status_line_streamlit_visible_mount_gate_observation_packet"
...
def test_q32p_default_hidden_observation_keeps_visible_mount_gate_hidden() -> None:
    packet = build_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_packet(fragment_summary={"fragment_widget_count": 9})
    assert packet["packet_kind"] == "warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_packet"
...
def test_q32p_hidden_observation_can_record_ready_gate_but_still_not_mount() -> None:
    packet = build_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_packet(
    ...
    assert packet["gate_status"] == "compact_ws_status_line_streamlit_visible_mount_ready_not_mounted"
    assert packet["upstream_renderer_model_ready"] is True
    assert packet["mount_allowed_for_future_warroom_page"] is True
...
def test_q32p_warroom_page_records_hidden_visible_mount_gate_observation_only() -> None:
    text = WARROOM_PAGE.read_text(encoding="utf-8-sig")
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_VISIBLE_MOUNT_GATE_OBSERVATION_STATE_KEY" in text
    assert "warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_q32p" in text
    assert "build_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_packet" in text
    assert "st.session_state[WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_VISIBLE_MOUNT_GATE_OBSERVATION_STATE_KEY]" in text
    assert "build_warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_packet" not in text
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_VISIBLE_MOUNT_GATE_VERSION" not in text
    for label in ("compact WS status line visible mount", "WS状態: WS未接続（準備中）", "WarRoom WebSocket 状態"):
        assert label not in text


def test_q32p_doc_modules_preserve_no_side_effect_boundary() -> None:
    text = DOC.read_text(encoding="utf-8-sig")
    assert "state_key=warroom_v2_compact_ws_status_line_streamlit_visible_mount_gate_observation_q32p" in text
    assert "default_gate_status=compact_ws_status_line_streamlit_visible_mount_hidden_default" in text
