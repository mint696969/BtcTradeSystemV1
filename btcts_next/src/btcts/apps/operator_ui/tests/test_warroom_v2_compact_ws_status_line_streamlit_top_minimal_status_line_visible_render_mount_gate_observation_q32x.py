# path: ./btcts_next/src/btcts/apps/operator_ui/tests/test_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_q32x.py
# desc: PS-Q32X guards for hidden session_state compact WS status line top minimal visible render mount gate observation. No UI mount and no socket open.
...
DOC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q32X_WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_TOP_MINIMAL_STATUS_LINE_VISIBLE_RENDER_MOUNT_GATE_OBSERVATION_TO_HIDDEN_SESSION_STATE_NO_UI_MOUNT_2026-07-03.md"
...
def test_q32x_contract_is_hidden_state_for_top_minimal_visible_render_mount_gate() -> None:
    packet = build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_contract()
    assert packet["state_key"] == WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_TOP_MINIMAL_STATUS_LINE_VISIBLE_RENDER_MOUNT_GATE_OBSERVATION_STATE_KEY
    assert packet["observation_kind"] == "warroom_v2_hidden_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_packet"
    ...
    assert packet["render_mount_ready_for_future_streamlit_mount_default"] is False
...
def test_q32x_default_hidden_observation_keeps_mount_gate_hidden() -> None:
    packet = build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_packet(fragment_summary={"fragment_widget_count": 9})
    assert packet["packet_kind"] == "warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_packet"
...
def test_q32x_hidden_observation_can_record_ready_gate_but_still_not_mount() -> None:
    packet = build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_packet(
    ...
    assert packet["gate_status"] == "compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_ready_not_mounted"
    assert packet["upstream_render_payload_ready_for_future_streamlit_mount"] is True
    assert packet["render_mount_ready_for_future_streamlit_mount"] is True
...
def test_q32x_warroom_page_records_hidden_top_minimal_mount_gate_observation_only() -> None:
    text = WARROOM_PAGE.read_text(encoding="utf-8-sig")
    assert "WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_TOP_MINIMAL_STATUS_LINE_VISIBLE_RENDER_MOUNT_GATE_OBSERVATION_STATE_KEY" in text
    assert "warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_q32x" in text
    assert "build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_packet" in text
    assert "st.session_state[WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_TOP_MINIMAL_STATUS_LINE_VISIBLE_RENDER_MOUNT_GATE_OBSERVATION_STATE_KEY]" in text
...
def test_q32x_doc_modules_preserve_no_side_effect_boundary() -> None:
    text = DOC.read_text(encoding="utf-8-sig")
    assert "state_key=warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_gate_observation_q32x" in text
    assert "default_gate_status=compact_ws_status_line_streamlit_top_minimal_status_line_visible_render_mount_hidden_default" in text
