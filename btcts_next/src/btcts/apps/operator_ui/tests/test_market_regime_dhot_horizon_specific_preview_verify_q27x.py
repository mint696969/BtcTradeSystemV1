...
def test_q27x_doc_records_dhot_horizon_specific_verify_boundary() -> None:
    ...
    assert "classifier_version=prediction.market_regime.regime_classifier.ps_q27w.v1" in text
...
def test_q27x_classifier_uses_dhot_like_horizon_specific_labels(tmp_path: Path) -> None:
    ...
    assert packet.logic_version == "prediction.market_regime.regime_classifier.ps_q27w.v1"
...
def test_q27x_warroom_preview_binding_uses_ps_q27w_stage_version_without_ui_change(tmp_path: Path) -> None:
    _build_dhot_like_fixture(tmp_path)
    ...
    assert packet["stage_versions"]["classifier"] == "prediction.market_regime.regime_classifier.ps_q27w.v1"
