# path: ./btcts_next/src/btcts/apps/operator_ui/hub/decision_policy_gate_visibility_chain_summary_registry_visibility.py
# desc: Read-only registry visibility packet for the decision policy gate visibility chain summary. Catalog/registry metadata only; no UI implementation, commands, runtime payload loading, summary builder execution, writes, mode apply, grants, or broker behavior.
...
VISIBILITY_CHAIN_SUMMARY_SOURCE_KEY = "decision_policy_gate_visibility_chain_summary"
VISIBILITY_CHAIN_SUMMARY_SOURCE_TYPE = "decision_policy_gate_read_only_visibility_chain_summary_packet"

VISIBILITY_CHAIN_SUMMARY_REGISTRY_VISIBILITY_CONTRACT = {
    "visibility_packet_type": "decision_policy_gate_visibility_chain_summary_registry_visibility",
    "source_key": VISIBILITY_CHAIN_SUMMARY_SOURCE_KEY,
    "source_type": VISIBILITY_CHAIN_SUMMARY_SOURCE_TYPE,
    "dashboard_role": "operator_ui_visibility_chain_summary_registry_visibility",
    "read_only_contract": True,
    "non_executing": True,
    "data_model_only": True,
    "catalog_metadata_only": True,
    "registry_visibility_only": True,
    "visibility_summary_visibility_only": True,
...
def visible_pages_for_decision_policy_gate_visibility_chain_summary(
    registry: Mapping[str, Any] | None = None,
) -> tuple[str, ...]:
    payload = registry or load_dashboard_hub_display_source_registry()
    return tuple(
        str(item.get("page_key"))
        for item in _page_visibility(payload, VISIBILITY_CHAIN_SUMMARY_SOURCE_KEY)
        if item.get("visible") and item.get("page_key")
    )


def build_decision_policy_gate_visibility_chain_summary_registry_visibility_packet(
    ...
    source_entry = _source_entry(catalog_payload, VISIBILITY_CHAIN_SUMMARY_SOURCE_KEY)
    page_visibility = _page_visibility(registry_payload, VISIBILITY_CHAIN_SUMMARY_SOURCE_KEY)
    ...
    return {
        **VISIBILITY_CHAIN_SUMMARY_REGISTRY_VISIBILITY_CONTRACT,
        ...
        "summary_module": source_entry.get("summary_module"),
        "summary_builder": source_entry.get("summary_builder"),
        ...
        "health_page_visible": VISIBILITY_CHAIN_SUMMARY_SOURCE_KEY in page_key_map["health"],
        "future_widget_page_visible": VISIBILITY_CHAIN_SUMMARY_SOURCE_KEY in page_key_map["logs"],
