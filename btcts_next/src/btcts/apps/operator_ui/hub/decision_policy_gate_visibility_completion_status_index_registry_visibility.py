# path: ./btcts_next/src/btcts/apps/operator_ui/hub/decision_policy_gate_visibility_completion_status_index_registry_visibility.py
# desc: Read-only registry visibility packet for the decision policy gate visibility completion status index. Catalog/registry metadata only; no UI implementation, commands, runtime payload loading, status index builder execution, writes, mode apply, grants, or broker behavior.
...
VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_KEY = "decision_policy_gate_visibility_completion_status_index"
VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_TYPE = "decision_policy_gate_read_only_visibility_completion_status_index_packet"

VISIBILITY_COMPLETION_STATUS_INDEX_REGISTRY_VISIBILITY_CONTRACT = {
    "visibility_packet_type": "decision_policy_gate_visibility_completion_status_index_registry_visibility",
    "source_key": VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_KEY,
    "source_type": VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_TYPE,
    "dashboard_role": "operator_ui_visibility_completion_status_index_registry_visibility",
    "read_only_contract": True,
    "non_executing": True,
    "data_model_only": True,
    "catalog_metadata_only": True,
    "registry_visibility_only": True,
    "visibility_completion_status_index_visibility_only": True,
...
def visible_pages_for_decision_policy_gate_visibility_completion_status_index(
    registry: Mapping[str, Any] | None = None,
) -> tuple[str, ...]:
    payload = registry or load_dashboard_hub_display_source_registry()
    return tuple(
        str(item.get("page_key"))
        for item in _page_visibility(payload, VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_KEY)
        if item.get("visible") and item.get("page_key")
    )


def build_decision_policy_gate_visibility_completion_status_index_registry_visibility_packet(
    ...
    source_entry = _source_entry(catalog_payload, VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_KEY)
    page_visibility = _page_visibility(registry_payload, VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_KEY)
    ...
    return {
        **VISIBILITY_COMPLETION_STATUS_INDEX_REGISTRY_VISIBILITY_CONTRACT,
        ...
        "visibility_completion_status_index_contract": dict(DECISION_POLICY_GATE_VISIBILITY_COMPLETION_STATUS_INDEX_CONTRACT),
        "page_visibility": page_visibility,
        "visible_pages": visible_pages,
        "visible_page_count": len(visible_pages),
        "health_page_visible": VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_KEY in page_key_map["health"],
        "future_widget_page_visible": VISIBILITY_COMPLETION_STATUS_INDEX_SOURCE_KEY in page_key_map["logs"],
