# path: ./btcts_next/src/btcts/apps/operator_ui/prediction_warroom/v2/transport/compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_gate.py
# desc: WarRoom v2 compact WS status line top minimal actual mount gate. Pure packet only; no UI, sockets, IO, order send, or polling fallback.

from __future__ import annotations

from typing import Any, Iterable, Mapping

from .compact_ws_status_line_streamlit_top_minimal_status_line_renderer_observation import build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_renderer_observation_packet

WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_TOP_MINIMAL_STATUS_LINE_ACTUAL_MOUNT_GATE_VERSION = "prediction_warroom.v2.transport.compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_gate.ps_q32s.v1"
_LABELS_JA: tuple[str, ...] = ("WS状態", "データ鮮度", "最終更新", "受信数", "破棄数", "案内")


def _gate_status(*, requested: bool, ack: bool, upstream_ready: bool) -> str:
    if not requested:
        return "compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_hidden_default"
    if not ack:
        return "compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_blocked_ack_required"
    if not upstream_ready:
        return "compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_blocked_renderer_not_ready"
    return "compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_ready_not_mounted"


def build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_gate_contract() -> dict[str, Any]:
    return {
        "ok": True,
        "gate_version": WARROOM_V2_COMPACT_WS_STATUS_LINE_STREAMLIT_TOP_MINIMAL_STATUS_LINE_ACTUAL_MOUNT_GATE_VERSION,
...
def build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_gate_packet(
    *,
    ...
    observation = build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_renderer_observation_packet(
    ...
    upstream_ready = bool(observation.get("future_streamlit_call_ready"))
    gate_status = _gate_status(requested=bool(actual_mount_requested), ack=bool(operator_actual_mount_ack), upstream_ready=upstream_ready)
    ready = gate_status == "compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_ready_not_mounted"
    return {
        **build_warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_gate_contract(),
        "packet_kind": "warroom_v2_compact_ws_status_line_streamlit_top_minimal_status_line_actual_mount_gate_packet",
        "fragment_summary": dict(fragment_summary or {}),
        "compact_ws_status_line_streamlit_top_minimal_status_line_renderer_observation_packet": observation,
        "gate_status": gate_status,
        "actual_mount_requested": bool(actual_mount_requested),
        "operator_actual_mount_ack": bool(operator_actual_mount_ack),
        "upstream_future_streamlit_call_ready": upstream_ready,
        "actual_mount_allowed_for_future_warroom_page": bool(ready),
