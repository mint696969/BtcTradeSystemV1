...
WARROOM_V2_TOPIC_CATALOG_VERSION = "prediction_warroom.v2.topic_catalog.ps_q29a.v1"
...
def build_warroom_v2_widget_topic_catalog() -> dict[str, Any]:
    rows = [
        {
            "topic": topic,
            "role": _TOPIC_ROLES[topic],
            "widget_update_unit": True,
            "push_transport_required_now": False,
            "future_websocket_compatible": True,
            "future_sse_compatible": True,
            "page_reload_required": False,
        }
        for topic in WARROOM_V2_WIDGET_TOPICS
    ]
    packet: dict[str, Any] = {
        "ok": True,
        "topic_catalog_version": WARROOM_V2_TOPIC_CATALOG_VERSION,
        "topic_count": len(rows),
        "topics": [row["topic"] for row in rows],
        "rows": rows,
        "contract_only": True,
        "warroom_legacy_retained": True,
