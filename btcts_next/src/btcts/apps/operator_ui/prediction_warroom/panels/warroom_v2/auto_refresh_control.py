...
WARROOM_V2_AUTO_REFRESH_CONTROL_RENDERER_VERSION = "prediction_warroom.v2.auto_refresh_control.ps_q29x.v1"
...
def build_warroom_v2_auto_refresh_control_packet(*, enabled: bool = False, interval_ms: int = DEFAULT_INTERVAL_MS, source: str = "local_default", last_rendered_at: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "transport_kind": "browser_timer_polling", "auto_refresh_available": True, "auto_refresh_enabled": bool(enabled),
        "auto_refresh_enabled_default": False, "auto_refresh_source": source, "sidebar_auto_refresh_consumed": source == "operator_sidebar",
        "interval_ms": bounded, "min_interval_ms": MIN_INTERVAL_MS, "max_interval_ms": MAX_INTERVAL_MS,
        "last_rendered_at": last_rendered_at or _now_iso(), "observable_status_strip": True,
...
def render_warroom_v2_auto_refresh_status_strip(packet: dict[str, Any]) -> None:
    ...
    c3.metric("Source", str(packet["auto_refresh_source"]))
    c4.metric("Last render UTC", str(packet["last_rendered_at"])[11:19])
    st.caption("observable browser-timer status / push_connected=false / websocket=false / sse=false")
...
def render_warroom_v2_auto_refresh_control() -> dict[str, Any]:
    ...
    with st.expander("Auto refresh / 高頻度更新", expanded=False):
        ...
        st.json({"enabled": packet["auto_refresh_enabled"], "interval_ms": packet["interval_ms"], "last_rendered_at": packet["last_rendered_at"], "transport_kind": packet["transport_kind"]}, expanded=False)
