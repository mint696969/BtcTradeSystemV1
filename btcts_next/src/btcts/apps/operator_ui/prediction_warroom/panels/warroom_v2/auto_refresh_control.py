...
# desc: WarRoom v2 auto refresh control. Sidebar state drives metrics-first Streamlit fragment refresh.
...
WARROOM_V2_AUTO_REFRESH_CONTROL_RENDERER_VERSION = "prediction_warroom.v2.auto_refresh_control.ps_q29z.v1"
...
def build_warroom_v2_auto_refresh_control_packet(*, enabled: bool = False, interval_ms: int = DEFAULT_INTERVAL_MS, source: str = "local_default", last_rendered_at: str | None = None, active_fragment_targets: list[str] | None = None) -> dict[str, Any]:
    bounded = _bounded_interval(interval_ms)
    active_targets = list(active_fragment_targets or ["market_snapshot_strip"])
...
def render_warroom_v2_auto_refresh_status_strip(packet: dict[str, Any]) -> None:
    ...
    c3.metric("Target", "metrics only" if packet["metrics_only_auto_refresh_default"] else "metrics + chart")
    c4.metric("Last render UTC", str(packet["last_rendered_at"])[11:19])
    st.caption("observable browser-timer status / fragment polling active / metrics-only default / page_reload=false / push_connected=false / websocket=false / sse=false")
...
def render_warroom_v2_auto_refresh_control() -> dict[str, Any]:
    ...
    with st.expander("Auto refresh / 高頻度更新", expanded=False):
        st.caption("Uses left sidebar setting / metrics-only Streamlit fragment refresh by default / no page reload")
        st.json({"enabled": packet["auto_refresh_enabled"], "interval_ms": packet["interval_ms"], "transport": packet["effective_transport_kind"], "active_fragment_targets": packet["active_fragment_targets"], "page_reload_enabled": packet["page_reload_enabled"], "last_rendered_at": packet["last_rendered_at"]}, expanded=False)
    return packet
