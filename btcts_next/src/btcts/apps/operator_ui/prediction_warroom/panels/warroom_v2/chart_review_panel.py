...
WARROOM_V2_CHART_REVIEW_PANEL_RENDERER_VERSION = "prediction_warroom.v2.chart_review_panel_renderer.ps_q29v.v1"
...
def _markdown_packet(payload: dict[str, Any]) -> str:
    s, r, w = payload["market_snapshot"], payload["range_summary"], payload["chart_window"]
    return "\n".join(["# WarRoom Chart Review Packet", "", f"schema_version: {payload['schema_version']}", f"exchange: {payload['exchange']}", f"market: {payload['market']}", f"timeframe: {payload['timeframe']}", f"chart_window: {w['timeframe']} / row_limit={w['row_limit']}", "", "## Market Snapshot", f"ltp: {s['ltp']}", f"best_bid: {s['best_bid']}", f"best_ask: {s['best_ask']}", f"spread: {s['spread']}", f"spread_bps: {s['spread_bps']}", f"data_state: {s['data_state']}", "", "## Range Summary", f"open: {r.get('open')}", f"high: {r.get('high')}", f"low: {r.get('low')}", f"close: {r.get('close')}", f"change_pct: {r.get('change_pct')}", f"range_pct: {r.get('range_pct')}", "", "## Safety", "read_only: true", "push_connected: false", "would_send_to_broker: false"])
...
def _render_chart_views(chart: dict[str, Any]) -> None:
    ...
    latest, summary, window = view.iloc[-1], dict(chart.get("range_summary") or {}), dict(chart.get("chart_window") or {})
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Rows", f"{int(summary.get('row_count') or len(view))} / {window.get('row_limit')}")
    ...
    st.caption(f"chart window: {window.get('timeframe')} / row_limit={window.get('row_limit')} / {window.get('window_policy')}")
...
def render_warroom_v2_chart_review_panel(*, source_packet: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    st.caption("D-hot market chart series read-only / price + bps views / timeframe-aware window / no push connection")
