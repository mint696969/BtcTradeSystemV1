...
WARROOM_V2_CHART_REVIEW_PANEL_RENDERER_VERSION = "prediction_warroom.v2.chart_review_panel_renderer.ps_q29s.v1"
...
def _base_payload(timeframe: str) -> dict[str, Any]:
    ...
    return {
        ...
        "selection": {"clicked_at": None, "range_start": None, "range_end": None}, "operator_intent": "この範囲で当時の予測が妥当だったか確認したい。地合い・方向感・ボラ・反転警戒・市場間確認を見直したい。",
        ...
        "range_summary": {"open": None, "high": None, "low": None, "close": None, "change_pct": None, "range_pct": None, "row_count": 0},
        "prediction_context": {"snapshot_time": None, "selected_horizon": None, "items": {"地合い": None, "方向感": None, "ボラ": None, "反転警戒": None, "市場間確認": None}},
        "annotations": {"predictions": [], "orderbook": [], "orders": [], "manual": []}, "safety": {"read_only": True, "display_only": True, "runtime_connected": False, "push_connected": False, "would_send_to_broker": False},
    }
...
def build_warroom_v2_chart_review_panel_packet(*, timeframe: str = "5m", source_packet: dict[str, Any] | None = None, chart_series_packet: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    connected, chart_connected = bool(source.get("data_connected")), bool(chart.get("chart_series_connected"))
    return {"ok": True, "renderer_version": WARROOM_V2_CHART_REVIEW_PANEL_RENDERER_VERSION, "schema_version": WARROOM_V2_CHART_REVIEW_SCHEMA_VERSION, "placement": "bottom_of_warroom_v2", "chart_placeholder_only": not chart_connected, "actual_chart_series_bound": chart_connected, "timeframes": list(WARROOM_V2_CHART_REVIEW_TIMEFRAMES), "selected_timeframe": payload["timeframe"], "annotation_layers": list(payload["annotations"].keys()), "payload": payload, "markdown_preview": _markdown_packet(payload), "json_preview": json.dumps(payload, ensure_ascii=False, indent=2), "copy_for_gpt_ready": True, "market_snapshot_source": source, "chart_series": chart, "push_ready": True, "auto_refresh_ready": True, "data_connected": connected, "chart_series_connected": chart_connected, "runtime_connected": False, "push_connected": False, "websocket_enabled": False, "sse_enabled": False, "display_only": True, "read_only": True, "would_send_to_broker": False}
...
def render_warroom_v2_chart_review_panel(*, source_packet: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if chart.get("chart_rows"):
        ...
        st.line_chart(df.dropna(subset=["ts"]).set_index("ts")[["mid_price", "best_bid", "best_ask"]], height=280, width="stretch")
    else:
        st.markdown("▧ chart placeholder: market series is not available yet")
    st.caption("D-hot market chart series read-only / no push connection")
