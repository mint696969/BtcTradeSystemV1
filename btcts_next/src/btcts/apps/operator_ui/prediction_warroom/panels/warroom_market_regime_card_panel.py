    ...
    renderer_packet.update(
        {
            ...
            "source_snapshot_missing_sources": list(binding_packet.get("source_snapshot_missing_sources", [])) if isinstance(binding_packet, Mapping) else [],
            "source_snapshot_warnings": list(binding_packet.get("source_snapshot_warnings", [])) if isinstance(binding_packet, Mapping) else [],
            "prediction_warnings": list(binding_packet.get("prediction_warnings", [])) if isinstance(binding_packet, Mapping) else [],
            "feature_bundle_available_signal_count": int(binding_packet.get("feature_bundle_available_signal_count", 0)) if isinstance(binding_packet, Mapping) else 0,
            "live_data_connected": False,
            "warroom_page_changed": False,
            "warroom_page_mounted": False,
            "streamlit_render_invoked_by_page": False,
