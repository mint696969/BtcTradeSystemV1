...
foreach ($p in $ports) {
  ...
  if (-not $pids) {
    Write-Host "[D-$p] LISTEN found but PID parse failed"
    continue
  }

  foreach ($procId in $pids) {
    $proc = Get-Process -Id $procId -ErrorAction SilentlyContinue
    $pname = if ($proc) { $proc.ProcessName } else { "(not found)" }
    Write-Host "[D-$p] LISTEN: PID=$procId Name=$pname"
  }
  ...
  try {
    ...
    if ($resp) {
      Write-Host "[D-$p] HTTP reachable (snippet):" ($resp.Substring(0,[Math]::Min(120,$resp.Length)))
    } else {
      Write-Host "[D-$p] HTTP reachable but empty response"
    }
  } catch {
    Write-Host "[D-$p] HTTP probe failed"
  }
}
