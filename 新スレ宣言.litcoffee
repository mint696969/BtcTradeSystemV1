...
※C:\BtcTradeSystem\tmp\gpt_room\reference\_generatedにはGPTが作業しやすくなるための便利ファイルが入っています。
--------------------------------------------
...
初めての任務としてgpt_room直下に配置してあるファイルを番号順に読み進め運用ルールや実作業においての指示の出し方などを把握してください。

--------------------------------------------

私の希望した仕様書作成などの成果物は作業台としての./tmpへ作成、作業していくうえでの一時メモなどは./tmp/gpt_room/memoryなど、記憶に関するものは部屋内の適した場所を利用してください。 

それではhandoffや関連する書類を読んで前GPTからの引継ぎを行い現状をとロードマップ・次に行うタスクなどを把握してください。

--------------------------------------------

この後、作業を始めるにあたって以下に配置してある仕様書も確認してください。 
わたしは責務分離を徹底したファイル構造を希望します。 
C:\BtcTradeSystem\docs\architecture
他に質問やすり合わせたいことはないですか？
あと、現在「CCよろ。」はよほどのトラブルがない限りレスポンスが悪くなるのとGPTに負担がかかるので使っていません。
代わりに画像のように修正指示の最後にPowerShellの簡単なコマンドでミスがないか確認しています。
どんなコマンドで確認するかは君に任せます。

--------------------------------------------

最新の差分を仕様書に反映させたいので、C:\BtcTradeSystem\docs\architectureにある仕様書を元にして差分を吸収した最新の仕様書を./tmp直下に書き出してください。
書き出された新しい仕様書を私が元の仕様書と差し替えます。
重複情報はなるべく減らし、まとめられる情報はまとめ、分けるべき情報は分け、最適化した読みやすい仕様書にしてくださいね。
gpt_roomも、あなたが利用しやすいよう、過去の情報で寄せられるものは寄せ最適化してください。
