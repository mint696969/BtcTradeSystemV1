...
私の希望した仕様書作成などの成果物は作業台としての./tmpへ作成、作業していくうえでの一時メモなどは./tmp/gpt_room/memoryなど、記憶に関するものは部屋内の適した場所を利用してください。 それではhandoffや関連する書類を読んで前GPTからの引継ぎを行い現状をとロードマップ・次に行うタスクなどを把握してください。

--------------------------------------------

うん
この後repo 側の現行実装位置を最小確認し作業を始めるにあたって以下に配置してある仕様書も確認してください。 
