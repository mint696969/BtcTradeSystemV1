...
前GPT同様に君にもgpt_room内に専用のフォルダを作ってほしい。 
...
次は作業方針のすり合わせだ。 わたしはチャット進行の作業を好みます。 相棒と共同作業というかパーティープレイをしてるようで楽しいからねｗ 画像のような作業指示でお願いしたい。 ただし、ヒューマンエラーをできるだけ未然に防ぐため以下の注意点を守ってほしい。 ①VSVのコード検索で修正ブロックを見つけコピペで修正できるようにインデントの考慮を忘れないでほしい。 ②どこを修正しているのか間違えないよう画像のようにナンバリングしてほしい。