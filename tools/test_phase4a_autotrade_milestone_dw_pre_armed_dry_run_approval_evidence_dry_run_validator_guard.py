# path: ./tools/test_phase4a_autotrade_milestone_dw_pre_armed_dry_run_approval_evidence_dry_run_validator_guard.py
# desc: Guard S57 Pre-Armed Dry Run approval evidence validator remains dry-run only and non-authorizing.
...
TMP_ROOT = REPO_ROOT / "tmp" / "_s57_pre_armed_dry_run_approval_evidence_validator_guard"
...
def _evidence(path: Path, *, valid: bool) -> Path:
    payload = {
        "evidence_id": "approval_evidence_guard_valid_001" if valid else "bad_evidence_guard_001",
        "approval_scope": "PRE_ARMED_DRY_RUN_REVIEW_ONLY" if valid else "LIVE",
        "target_mode": "PRE_ARMED_DRY_RUN" if valid else "LIVE_MIN_SIZE",
        "requested_by": "guard_operator" if valid else "",
        "requested_at": "2026-06-17T00:00:20Z" if valid else "",
        "operator_identity": "guard_human_operator" if valid else "",
        "human_review_packet_ready": True if valid else False,
        "approval_recording_requested": False,
        "command_ledger_append_requested": False,
        "mode_change_requested": False,
        "reason_codes": ["operator_final_human_review", "pre_armed_dry_run_review_only"] if valid else ["invalid"],
        "acknowledgements": list(REQUIRED_ACKS) if valid else ["review_all_runtime_control_evidence"],
    }
...
def main() -> int:
    ...
    cv = clear_valid["validator"]; ci = clear_invalid["validator"]; bv = blocked_valid["validator"]
    checks = {
        "clear_valid_pipeline_returncode_zero": all(clear_valid[name] == 0 for name in ("wrapper_returncode", "evidence_returncode", "clearance_returncode", "rollup_returncode", "packet_returncode", "status_returncode", "validator_returncode")),
        "clear_valid_evidence_valid_not_recorded": cv.get("approval_evidence_valid") is True and cv.get("decision") == "approval_evidence_dry_run_valid_not_recorded" and cv.get("human_approval_recorded") is False,
        ...
        "clear_invalid_pipeline_returncode_zero": all(clear_invalid[name] == 0 for name in ("wrapper_returncode", "evidence_returncode", "clearance_returncode", "rollup_returncode", "packet_returncode", "status_returncode", "validator_returncode")),
        "clear_invalid_evidence_blocked_visible": ci.get("approval_evidence_valid") is False and ci.get("validation_blockers") and any("missing_ack" in item for item in ci.get("validation_blockers", [])),
        "blocked_valid_pipeline_returncode_zero": all(blocked_valid[name] == 0 for name in ("wrapper_returncode", "evidence_returncode", "clearance_returncode", "rollup_returncode", "packet_returncode", "status_returncode", "validator_returncode")),
        "blocked_valid_evidence_blocked_by_source_status": bv.get("approval_evidence_valid") is False and "approval_request_status_not_ready_for_human_review" in (bv.get("validation_blockers") or []),
    ...
    failures.extend(f"protected lower-layer dirty during milestone DW: {hit}" for hit in protected_dirty_hits)
    payload = {"ok": not failures, "phase": "phase4a_autotrade_milestone_dw_pre_armed_dry_run_approval_evidence_dry_run_validator_guard", "status": "closed" if not failures else "open", "contract": {"approval_evidence_validator_present": VALIDATOR.exists(), "clear_valid_evidence_dry_run_valid_without_recording": checks.get("clear_valid_evidence_valid_not_recorded", False), "clear_invalid_evidence_blocked_visible": checks.get("clear_invalid_evidence_blocked_visible", False), "blocked_source_status_blocks_evidence": checks.get("blocked_valid_evidence_blocked_by_source_status", False), "no_ledger_append_no_mode_request": checks.get("clear_valid_no_ledger_or_mode_request", False) and checks.get("blocked_valid_no_ledger_or_mode_request", False), "read_only_no_broker_non_authorizing": checks.get("clear_valid_not_authorizing", False) and checks.get("blocked_valid_not_authorizing", False), "protected_lower_layers_untouched": not protected_dirty_hits}, "checks": checks, "cases": {"clear_valid": {"ok": cv.get("ok"), "decision": cv.get("decision"), "approval_evidence_valid": cv.get("approval_evidence_valid"), "human_approval_recorded": cv.get("human_approval_recorded"), "approval_ledger_appended": cv.get("approval_ledger_appended"), "command_ledger_appended": cv.get("command_ledger_appended"), "mode_change_requested": cv.get("mode_change_requested")}, "clear_invalid": {"ok": ci.get("ok"), "decision": ci.get("decision"), "approval_evidence_valid": ci.get("approval_evidence_valid"), "validation_blockers": ci.get("validation_blockers")}, "blocked_valid": {"ok": bv.get("ok"), "decision": bv.get("decision"), "approval_evidence_valid": bv.get("approval_evidence_valid"), "validation_blockers": bv.get("validation_blockers"), "human_approval_recorded": bv.get("human_approval_recorded")}}, "protected_dirty_hits": protected_dirty_hits, "failures": failures}
