# path: ./tools/run_sr_fx_pre_armed_dry_run_approval_record_append_execution_authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_status.py
# desc: Broker-free Pre-Armed Dry Run authorization request record execution authorization request record execution gate dry-run/status from S68 plan. Gate/status only; non-recording; non-executing; non-authorizing.
...
TOOL = "run_sr_fx_pre_armed_dry_run_approval_record_append_execution_authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_status"
REPORT_VERSION = "pre_armed_dry_run_approval_record_append_execution_authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_status.v1"
REQUIRED_GATE_ACKS = (
    "confirm_s68_record_execution_authorization_request_record_dry_run_plan_status_reviewed",
    "confirm_record_execution_authorization_request_record_execution_gate_is_review_only",
...
FALSE_FIELDS = (
    ...
    "approval_record_append_request_submitted",
    "approval_record_append_request_persisted",
    "approval_record_persisted_by_this_tool",
...
def build_pre_armed_dry_run_authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_status(*, record_dry_run_plan_status: Mapping[str, Any], record_execution_gate_review: Mapping[str, Any], generated_at: str | None = None) -> dict[str, Any]:
    plan = dict(record_dry_run_plan_status)
    review = dict(record_execution_gate_review)
    plan_ok = plan.get("ok") is True
    plan_ready = plan.get("authorization_request_record_execution_authorization_request_record_dry_run_plan_ready") is True
    ...
    plan_safety_ok = (
        ...
        and all(plan.get(name) is False for name in FALSE_FIELDS)
    )
    ...
    false_inputs = {
        ...
        "authorization_request_record_execution_authorization_request_recorded": "record_execution_authorization_request_recorded_must_be_false_in_dry_run_gate",
        "authorization_request_record_execution_authorized": "record_execution_authorization_grant_must_be_false_in_dry_run_gate",
        "authorization_request_record_execution_requested": "authorization_request_record_execution_request_must_be_false_in_dry_run_gate",
        "authorization_request_record_executed": "authorization_request_record_execution_must_be_false_in_dry_run_gate",
    ...
    if draft.get("record_kind") != "pre_armed_dry_run_append_execution_authorization_request_record_execution_authorization_request_record_draft":
        gate_blockers.append("record_execution_authorization_request_record_draft_missing_or_invalid")
    if draft.get("recorded") is not False or draft.get("persisted") is not False or draft.get("authorized") is not False or draft.get("executed") is not False:
        gate_blockers.append("record_execution_authorization_request_record_draft_not_dry_run_safe")
    ...
    decision = "authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_ready_not_executed" if gate_ready else "authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_blocked_not_executed"
    payload = {
        ...
        "decision": decision,
        "authorization_request_record_execution_authorization_request_record_execution_gate_ready": gate_ready,
        ...
        "warnings": [
            "authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_status_is_informational_only",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build broker-free Pre-Armed Dry Run authorization request record execution authorization request record execution gate dry-run/status from S68 JSON.")
    parser.add_argument("--record-dry-run-plan-status", required=True, help="Path to S68 dry-run plan/status JSON.")
    parser.add_argument("--record-execution-gate-review", required=True, help="Path to gate review JSON. This slice records and executes nothing.")
    parser.add_argument("--out", default="", help="Optional gate/status output JSON path.")
    return parser
...
def main() -> int:
    ...
    try:
        ...
        payload = build_pre_armed_dry_run_authorization_request_record_execution_authorization_request_record_execution_gate_dry_run_status(record_dry_run_plan_status=plan, record_execution_gate_review=review)
