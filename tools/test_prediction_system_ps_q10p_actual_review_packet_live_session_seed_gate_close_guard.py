# path: ./tools/test_prediction_system_ps_q10p_actual_review_packet_live_session_seed_gate_close_guard.py
# desc: Close guard for PS-Q10P live-session seed gate.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_actual_review_packet_live_session_seed_gate.py",
...
def _run(path: Path, failures: list[str]) -> None:
    proc = subprocess.run([sys.executable, str(path)], cwd=REPO_ROOT, text=True, capture_output=True, timeout=180)
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q10P actual review-packet live-session seed gate close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q10P actual review-packet live-session seed gate close guard")
    return 0


def test_prediction_system_ps_q10p_actual_review_packet_live_session_seed_gate_close_guard() -> None:
    assert main() == 0
