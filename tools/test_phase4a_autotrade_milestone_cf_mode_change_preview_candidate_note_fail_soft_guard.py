# path: ./tools/test_phase4a_autotrade_milestone_cf_mode_change_preview_candidate_note_fail_soft_guard.py
# desc: Guard Mode Change Apply Preview candidate readiness note parsing is fail-soft. Read-only; no append/runner/broker.
...
def command_row(command_id: str, note: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "confirmation": True,
            "reason_codes": ["guard", "candidate_note_fail_soft"],
            "note": note,
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_mode_change_preview_candidate_note_fail_soft_hot"
    ...
    try:
        ...
        write_jsonl(
            command_path,
            [
                command_row("cmd_cf_candidate_malformed_note", "{not-json"),
                command_row("cmd_cf_candidate_wrong_kind_note", json.dumps({"kind": "not-readiness", "observer_latest_run_id": "should_not_win"}, sort_keys=True)),
                command_row("cmd_cf_candidate_array_note", json.dumps(["not", "object"])),
            ],
        )
    ...
    checks = {
        ...
        "preview_ignores_malformed_or_wrong_kind_candidate_note": preview_data.get("command_id") == "cmd_cf_candidate_array_note" and preview_data.get("candidate_readiness_note_present") is False and preview_data.get("candidate_readiness_ready") is None and preview_data.get("candidate_readiness_current_mode") is None and tuple(preview_data.get("candidate_readiness_blocked_by") or ()) == () and tuple(preview_data.get("candidate_readiness_warnings") or ()) == () and preview_data.get("candidate_readiness_observer_latest_run_id") is None,
        "preview_recheck_still_runs_with_bad_candidate_note": preview_data.get("readiness") is not None and preview_data.get("would_reject_by_readiness") is True and preview_data.get("readiness_ready") is False and "readiness_recheck_not_ready" in tuple(preview_data.get("blocked_by") or ()),
        ...
        "preview_did_not_append_command_or_mode_state": before_command_count == 3 and after_preview_command_count == before_command_count and before_mode_count == 0 and after_preview_mode_count == before_mode_count,
        "render_did_not_append_command_or_mode_state": before_command_count == 3 and after_render_command_count == before_command_count and before_mode_count == 0 and after_render_mode_count == before_mode_count,
    ...
    failures.extend(f"protected lower-layer dirty during milestone CF: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_cf_mode_change_preview_candidate_note_fail_soft_guard",
        ...
        "contract": {
            ...
            "preview_ignores_malformed_or_wrong_kind_candidate_note": checks["preview_ignores_malformed_or_wrong_kind_candidate_note"],
            "preview_recheck_still_runs_with_bad_candidate_note": checks["preview_recheck_still_runs_with_bad_candidate_note"],
