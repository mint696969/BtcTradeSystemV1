# path: ./tools/test_phase4a_autotrade_milestone_hj_decision_policy_gate_rendering_plan_design_close_guard.py
# desc: Close guard for S147 decision policy gate rendering plan design packet. Syntax-checks touched guard files and reruns focused HJ guard.
...
TOUCHED = (
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hj_decision_policy_gate_rendering_plan_design_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hj_decision_policy_gate_rendering_plan_design_close_guard.py",
)
DOC = REPO_ROOT / "docs/architecture/OPERATOR_UI_DECISION_POLICY_GATE_READ_ONLY_RENDERING_PLAN_2026-06-18.md"
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hj_decision_policy_gate_rendering_plan_design_guard.py"
...
def main() -> int:
    ...
    if not DOC.exists():
        failures.append(f"missing design doc: {DOC.relative_to(REPO_ROOT)}")
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HJ decision policy gate rendering plan design guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hj_decision_policy_gate_rendering_plan_design_close_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "syntax_checked_without_pyc": all(item.get("ok") is True for item in syntax.values()),
            "design_doc_present": DOC.exists(),
...
def test_decision_policy_gate_rendering_plan_design_close_guard_closes() -> None:
    assert main() == 0
