# path: ./tools/test_phase4a_prediction_system_ps_q18e_close_guard.py
# desc: Close guard for PS-Q18E latest_prediction_summary_widget props binding preflight.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18e_latest_prediction_summary_widget_props_binding_preflight_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18e_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    if "test_ps_q18e_validates_props_preflight_from_q18d_fixture" not in unit_text:
        failures.append("unit test must cover Q18D fixture props preflight")
    if "test_ps_q18e_candidate_has_all_required_component_props" not in unit_text:
        failures.append("unit test must cover required component props")
    ...
    if report.get("props_validation_failures"):
        failures.append(f"props validation failures: {report.get('props_validation_failures')}")
    ...
    if report.get("schema_probe_row_count") != 4:
        failures.append("schema row count should be 4")
    if report.get("missing_required_schema_keys") != []:
        failures.append("missing required schema keys should be empty")
    if int(report.get("props_candidate_key_count") or 0) < 10:
        failures.append("props candidate key count unexpectedly small")
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_props_binding_preflight_guard":
    ...
    if blocked.get("props_candidate_ready") is not False:
        failures.append("blocked report should not be props candidate ready")
    if blocked.get("widget_props_bound_to_component") is not False:
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18e_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_props_binding_preflight_closed_before_component_binding_real_widget_rendering_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18e_closed": not failures,
            "latest_prediction_summary_widget_props_binding_preflight_only": True,
            "props_candidate_ready": True,
            "props_contract_complete": True,
            "missing_required_component_props": list(report.get("missing_required_component_props") or []),
            "props_value_binding_deferred": True,
            "real_payload_values_bound": False,
            "widget_props_binding_allowed": False,
            "widget_props_bound_to_component": False,
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18F latest_prediction_summary_widget props candidate status row mount or first render-disabled component packet validation",
        },
...
def test_ps_q18e_close_guard() -> None:
    assert main_guard() == 0
