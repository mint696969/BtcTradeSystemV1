# path: ./tools/test_phase4a_prediction_system_ps_q18u_latest_prediction_summary_widget_one_source_no_read_existence_check_gate_open_contract.py
# desc: Unit tests for PS-Q18U latest_prediction_summary_widget one-source no-read existence-check gate-open contract.
...
def test_ps_q18u_validates_gate_open_contract_from_q18t_fixture() -> None:
    report = build_report(use_observed_fixture=True)
    assert report["ok"] is True
    assert report["checker_version"] == CHECKER_VERSION
    assert report["one_source_no_read_existence_check_gate_open_contract_check_version"] == ONE_SOURCE_NO_READ_EXISTENCE_CHECK_GATE_OPEN_CONTRACT_CHECK_VERSION
    assert report["one_source_no_read_existence_check_gate_open_contract_ack"] == ONE_SOURCE_NO_READ_EXISTENCE_CHECK_GATE_OPEN_CONTRACT_ACK
    assert report["source_q18t_report_valid"] is True
    assert report["gate_open_contract_packet_valid"] is True
    assert report["gate_open_contract_row_count"] == 14
    assert report["source_candidate_count"] == 1
    assert report["gate_open_contract_candidate_ready"] is True
    assert report["existence_gate_open_contract_kind"] == EXISTENCE_GATE_OPEN_CONTRACT_KIND
    assert report["existence_gate_open_contract_state"] == EXISTENCE_GATE_OPEN_CONTRACT_STATE
    assert report["existence_check_execution_gate_open_allowed"] is False
    assert report["existence_check_execution_gate_opened"] is False
...
def test_ps_q18u_packet_without_source_is_contract_only_but_candidate_not_ready() -> None:
    packet = build_latest_prediction_summary_widget_one_source_no_read_existence_check_gate_open_contract_packet()
    assert packet["ok"] is True
    assert packet["gate_open_contract_row_count"] == 14
    assert packet["source_candidate_count"] == 1
    assert packet["one_source_no_read_existence_check_gate_open_contract_ack"] == ONE_SOURCE_NO_READ_EXISTENCE_CHECK_GATE_OPEN_CONTRACT_ACK
    assert packet["supplied_execution_preflight_report"] is False
    assert packet["gate_open_contract_candidate_ready"] is False
    assert packet["existence_check_execution_gate_open_allowed"] is False
    assert packet["existence_check_execution_gate_opened"] is False
...
def test_ps_q18u_blocks_missing_q18t_source() -> None:
    blocked = build_report()
    assert blocked["ok"] is False
    assert blocked["gate_open_contract_row_count"] == 0
    assert blocked["source_q18t_report_valid"] is False
    assert blocked["existence_check_execution_gate_opened"] is False
    assert blocked["source_artifact_exists_checked"] is False
    assert blocked["actual_source_read_invoked"] is False


def test_ps_q18u_observed_fixture_cli_and_main(capsys) -> None:
    assert main(["--use-observed-fixture"]) == 0
    printed = json.loads(capsys.readouterr().out)
    assert printed["ok"] is True
    assert printed["use_observed_fixture"] is True
    assert printed["stage"] == "latest_prediction_summary_widget_one_source_no_read_existence_check_gate_open_contract_before_gate_open_exists_schema_read_render_refresh_and_writes"
    assert printed["recommended_next_slice"].startswith("PS-Q18V")
