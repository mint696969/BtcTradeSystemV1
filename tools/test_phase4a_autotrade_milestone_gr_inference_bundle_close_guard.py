# path: ./tools/test_phase4a_autotrade_milestone_gr_inference_bundle_close_guard.py
# desc: Close guard for S129 inference bundle assembly. Syntax-checks touched files and reruns focused GR guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/bundle_assembly.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gr_inference_bundle_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gr_inference_bundle_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gr_inference_bundle_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GR inference bundle guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gr_inference_bundle_close_guard",
