# path: ./tools/test_phase4a_autotrade_milestone_fo_pre_armed_dry_run_auth_request_dry_run_plan_status_close_guard.py
# desc: Close guard for S100 Pre-Armed Dry Run authorization request dry-run plan/status. Broker-free; non-recording; non-executing; non-authorizing. Short path avoids Windows MAX_PATH.
...
SELF_PATH = "tools/test_phase4a_autotrade_milestone_fo_pre_armed_dry_run_auth_request_dry_run_plan_status_close_guard.py"
S100_RUN_PATH = "tools/run_sr_fx_pre_armed_dry_run_auth_request_dry_run_plan_status_s100.py"
S100_GUARD_PATH = "tools/test_phase4a_autotrade_milestone_fo_pre_armed_dry_run_auth_request_dry_run_plan_status_guard.py"
...
EXPECTED_SLICE_FILES = {S100_RUN_PATH, S100_GUARD_PATH, SELF_PATH}
...
def _run_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"S100 run missing required fragment: {frag}" for frag in missing)
    failures.extend(f"S100 run contains forbidden token: {tok}" for tok in forbidden)
    failures.extend(f"S100 run missing safety literal: {frag}" for frag in false_missing)
...
def _git_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"unexpected dirty file during S100 close: {line}" for line in unexpected)
    return {"lines": lines, "unexpected": unexpected, "expected_slice_files": sorted(EXPECTED_SLICE_FILES)}


def main() -> int:
    failures: list[str] = []
    syntax = {p: _syntax(p, failures) for p in (SELF_PATH, S100_RUN_PATH, S100_GUARD_PATH, S99_GUARD_PATH, COMMAND_STATUS_PATH)}
    run_boundary = _run_boundary(failures)
    s100_guard = _run_json_guard(S100_GUARD_PATH, failures)
    s99_guard = _run_json_guard(S99_GUARD_PATH, failures)
    git_boundary = _git_boundary(failures)
    payload = {"ok": not failures, "phase": "phase4a_autotrade_milestone_fo_pre_armed_dry_run_authorization_request_execution_authorization_request_execution_authorization_request_execution_authorization_request_execution_authorization_request_dry_run_plan_status_close_guard", "status": "closed" if not failures else "open", "contract": {"s100_authorization_request_dry_run_plan_status_present": (REPO_ROOT / S100_RUN_PATH).exists(), "s100_focused_guard_closed": s100_guard.get("ok") is True, "s99_fn_guard_closed": s99_guard.get("ok") is True, "syntax_checked_without_pyc": all(item.get("ok") is True for item in syntax.values()), "broker_free_non_recording_non_executing_non_authorizing": not run_boundary.get("forbidden"), "expected_git_boundary_only": not git_boundary.get("unexpected")}, "syntax": syntax, "run_boundary": run_boundary, "guards": {"s100_fo_guard": s100_guard, "s99_fn_guard": s99_guard}, "git_boundary": git_boundary, "failures": failures}
