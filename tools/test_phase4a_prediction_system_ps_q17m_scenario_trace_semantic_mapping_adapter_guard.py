# path: ./tools/test_phase4a_prediction_system_ps_q17m_scenario_trace_semantic_mapping_adapter_guard.py
# desc: Focused guard for PS-Q17M scenario-trace semantic mapping adapter.
...
from check_phase4a_prediction_system_ps_q17m_scenario_trace_semantic_mapping_adapter import ADAPTER_VERSION, CHECKER_VERSION, TRACE_PACKET_VERSION, build_report, main
...
def main_guard() -> int:
    ...
    for key in ("scenario_trace", "warroom_scenario_trace_release_gate", "contract_completeness", "warroom_scenario_trace_widget"):
        if key not in packet:
            failures.append(f"adapted packet missing: {key}")
    semantic = packet.get("scenario_trace", {}).get("semantic_mapping", {})
    ...
    if packet.get("warroom_scenario_trace_widget", {}).get("render_allowed") is not False:
    ...
    result = {"ok": not failures, "guard": "ps_q17m_scenario_trace_semantic_mapping_adapter_guard", "dirty_paths": sorted(dirty), "unexpected_dirty": sorted(unexpected), "failures": failures}
    print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if not failures else 1


def test_ps_q17m_scenario_trace_semantic_mapping_adapter_guard() -> None:
    assert main_guard() == 0
