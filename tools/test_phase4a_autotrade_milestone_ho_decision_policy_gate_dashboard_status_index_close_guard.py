# path: ./tools/test_phase4a_autotrade_milestone_ho_decision_policy_gate_dashboard_status_index_close_guard.py
# desc: Close guard for S152 decision policy gate dashboard status index packet. Syntax-checks touched files and reruns focused HO guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ho_decision_policy_gate_dashboard_status_index_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ho_decision_policy_gate_dashboard_status_index_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ho_decision_policy_gate_dashboard_status_index_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HO decision policy gate dashboard status index guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_ho_decision_policy_gate_dashboard_status_index_close_guard",
...
def test_decision_policy_gate_dashboard_status_index_close_guard_closes() -> None:
    assert main() == 0
