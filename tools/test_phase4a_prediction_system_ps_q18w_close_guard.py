# path: ./tools/test_phase4a_prediction_system_ps_q18w_close_guard.py
# desc: Close guard for PS-Q18W latest_prediction_summary_widget one-source no-read filesystem existence-check dry-run packet.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18w_latest_prediction_summary_widget_one_source_no_read_filesystem_existence_check_dry_run_packet_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18w_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    if "test_ps_q18w_validates_dry_run_packet_from_q18v_fixture" not in unit_text:
        failures.append("unit test must cover Q18V fixture dry-run packet")
    if "test_ps_q18w_packet_without_source_is_packet_only_but_candidate_not_ready" not in unit_text:
        failures.append("unit test must cover dry-run packet without candidate")
    ...
    if report.get("dry_run_packet_validation_failures"):
        failures.append(f"dry-run packet validation failures: {report.get('dry_run_packet_validation_failures')}")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_one_source_no_read_filesystem_existence_check_dry_run_packet_guard":
    ...
    if blocked.get("dry_run_packet_row_count") != 0:
        failures.append("blocked report should not emit observed dry-run packet rows")
    ...
    for forbidden in (
        ...
        "q18v_validation_invoked_by_mount=true",
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18w_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_one_source_no_read_filesystem_existence_check_dry_run_packet_before_exists_schema_read_render_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18w_closed": not failures,
            "latest_prediction_summary_widget_one_source_no_read_filesystem_existence_check_dry_run_packet_only": True,
            "one_source_no_read_filesystem_existence_check_dry_run_packet_ready": True,
            "filesystem_existence_check_dry_run_packet_declared": True,
            "filesystem_existence_check_dry_run_execution_allowed": False,
            ...
            "dry_run_packet_row_count": int(report.get("dry_run_packet_row_count") or 0),
            "source_candidate_count": int(report.get("source_candidate_count") or 0),
            "dry_run_packet_candidate_ready": bool(report.get("dry_run_packet_candidate_ready")),
            "filesystem_existence_check_dry_run_packet_kind": report.get("filesystem_existence_check_dry_run_packet_kind"),
            "filesystem_existence_check_dry_run_packet_state": report.get("filesystem_existence_check_dry_run_packet_state"),
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18X explicit one-source no-read filesystem existence-check dry-run result placeholder",
        },
...
def test_ps_q18w_close_guard() -> None:
    assert main_guard() == 0
