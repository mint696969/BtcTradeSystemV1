# path: ./tools/test_prediction_system_ps_e4_tradeflow_opportunity_context_guard.py
# desc: Focused guard for PS-E4 tradeflow/orderbook context on opportunity_participation.
...
def test_static_boundaries_and_ps_e4_markers() -> None:
    ...
    assert "opportunity_participation_feature_depth_context_supplied" in text
    assert "opportunity_tradeflow_context_version" in text
    assert "ps_e4.v1" in text


def test_rule_based_opportunity_accepts_tradeflow_context_only() -> None:
    ...
    opportunity = next(item for item in outputs if item.family == PredictionFamily.OPPORTUNITY_PARTICIPATION)
    data = opportunity.to_dict()
    context = data["values"]["feature_depth_context"]
    assert context["version"] == "ps_e4.v1"
    assert context["target_family"] == "opportunity_participation"
    ...
    assert context["tradeflow_buy_sell_imbalance_ratio"] is not None
    assert "opportunity_participation_feature_depth_context_supplied" in data["drivers"]
...
def test_prediction_system_passes_feature_depth_to_opportunity_context() -> None:
    ...
    data = result.to_dict()
    opportunity_outputs = [item for item in data["outputs"] if item["family"] == "opportunity_participation"]
    assert len(data["outputs"]) == 33
    assert data["forecast_batch"]["record_count"] == 33
    assert len(opportunity_outputs) == 3
    assert all(item["values"]["feature_depth_context"]["version"] == "ps_e4.v1" for item in opportunity_outputs)
    assert all(item["values"]["feature_depth_context"]["target_family"] == "opportunity_participation" for item in opportunity_outputs)
    assert all(item["values"]["feature_depth_context"]["primary_direction_owner"] is False for item in opportunity_outputs)
    assert all(item["values"]["feature_depth_context"]["usable_for_primary_short_horizon"] is False for item in opportunity_outputs)
    assert data["system_input"]["feature_snapshot"]["feature_depth_snapshot_supplied"] is True
    assert data["gpt_review_digest"]["opportunity_tradeflow_context_version"] == "ps_e4.v1"
...
def main() -> int:
    test_static_boundaries_and_ps_e4_markers()
    test_rule_based_opportunity_accepts_tradeflow_context_only()
    test_prediction_system_passes_feature_depth_to_opportunity_context()
    print("[OK] Prediction System PS-E4 tradeflow opportunity context guard passed")
    return 0
