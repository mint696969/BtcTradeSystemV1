# path: ./tools/test_phase4a_prediction_system_ps_q24n_autotrade_read_only_status_page_actual_page_wiring_gate_awaiting_human_decision.py
# desc: Focused pytest guard for PS-Q24N gate awaiting human decision before page wiring.
...
from tools.diagnose_phase4a_prediction_system_ps_q24n_autotrade_read_only_status_page_actual_page_wiring_gate_awaiting_human_decision import (  # noqa: E402
    DIAGNOSTIC_VERSION,
    run_autotrade_read_only_status_page_actual_page_wiring_gate_awaiting_human_decision,
)

SPEC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q24N_AUTOTRADE_READ_ONLY_STATUS_PAGE_ACTUAL_PAGE_WIRING_GATE_AWAITING_HUMAN_DECISION_2026-06-29.md"
MODULE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/autotrade_prediction_status_page_gate_awaiting_human_decision.py"
DIAG = REPO_ROOT / "tools/diagnose_phase4a_prediction_system_ps_q24n_autotrade_read_only_status_page_actual_page_wiring_gate_awaiting_human_decision.py"
AUTOTRADE_PAGE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/views/autotrade_page.py"


def test_spec_declares_q24n_gate_awaiting_human_decision_contract() -> None:
...
def test_live_q24n_gate_awaiting_human_decision_ready() -> None:
    ...
    assert result["q24m_explicit_human_gate_decision_required"]["ready"] is True
    packet = result["gate_awaiting_human_decision"]
    assert packet["awaiting_gate_type"] == "autotrade_prediction_status_page_actual_page_wiring_gate_awaiting_human_decision_packet"
    assert packet["gate_awaiting_human_decision_ready"] is True
    assert packet["gate_awaiting_human_decision_packet_available"] is True
    ...
    assert packet["gate_awaiting_human_decision"] is True
    ...
    assert packet["gate_awaiting_human_decision_packet_only"] is True
    ...
    assert safety["gate_awaiting_human_decision_packet_only"] is True
...
def test_autotrade_page_is_not_modified_or_wired_by_q24n_slice() -> None:
    ...
    assert "autotrade_prediction_status_page_gate_awaiting_human_decision" not in page_text
    assert "build_autotrade_prediction_status_page_gate_awaiting_human_decision_packet" not in page_text
...
if __name__ == "__main__":
    test_spec_declares_q24n_gate_awaiting_human_decision_contract()
    test_live_q24n_gate_awaiting_human_decision_ready()
    test_autotrade_page_is_not_modified_or_wired_by_q24n_slice()
