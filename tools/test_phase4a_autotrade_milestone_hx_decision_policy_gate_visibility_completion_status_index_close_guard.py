# path: ./tools/test_phase4a_autotrade_milestone_hx_decision_policy_gate_visibility_completion_status_index_close_guard.py
# desc: Close guard for S161 decision policy gate visibility completion status index packet. Syntax-checks touched files and reruns focused HX guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hx_decision_policy_gate_visibility_completion_status_index_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hx_decision_policy_gate_visibility_completion_status_index_close_guard.py",
)
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HX decision policy gate visibility completion status index guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hx_decision_policy_gate_visibility_completion_status_index_close_guard",
...
def test_decision_policy_gate_visibility_completion_status_index_close_guard_closes() -> None:
    assert main() == 0
