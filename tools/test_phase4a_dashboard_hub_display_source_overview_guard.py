# path: ./tools/test_phase4a_dashboard_hub_display_source_overview_guard.py
# desc: Guard dashboard hub display source overview presenter. No app.py/Streamlit/layout/runtime/broker wiring.
...
REGISTRY_CLOSE_GUARD = "tools/test_phase4a_dashboard_hub_display_source_registry_close_guard.py"
FILES = [
    "btcts_next/src/btcts/apps/operator_ui/hub/display_source_overview.py",
    "btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_overview.py",
]
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_overview"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    for fragment in missing:
        failures.append(f"dashboard hub display source overview missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub display source overview contains forbidden token: {token}")
...
def _check_app_py_untouched_boundary(failures: list[str]) -> dict[str, object]:
    ...
    forbidden = [
        "display_source_overview",
        "dashboard_hub_display_source_overview",
        "DASHBOARD_HUB_SOURCE_OVERVIEW_CONTRACT",
    ]
    hits = [token for token in forbidden if token in app_text]
    for token in hits:
        failures.append(f"app.py must not be wired to dashboard hub source overview in this slice: {token}")
    return {"hits": hits}
...
def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_overview_guard.py", failures),
        "compile_files": {rel: _compile(rel, failures) for rel in FILES},
        "registry_close_guard": _run_json_guard(REGISTRY_CLOSE_GUARD, failures),
        "plain_test": _run_plain_ok("btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_overview.py", failures),
        "source": _check_source(failures),
        "app_py_untouched_boundary": _check_app_py_untouched_boundary(failures),
    }
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_overview_guard",
        "failures": failures,
        "checks": checks,
    }
