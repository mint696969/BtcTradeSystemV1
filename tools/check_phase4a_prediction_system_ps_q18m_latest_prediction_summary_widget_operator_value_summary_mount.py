# path: ./tools/check_phase4a_prediction_system_ps_q18m_latest_prediction_summary_widget_operator_value_summary_mount.py
# desc: PS-Q18M checker for WarRoom latest_prediction_summary_widget operator-readable mapped value summary mount. It validates display-only rows and page mount; it never invokes Q18J, component packet builders, Streamlit render, source reads, D-hot discovery, refresh, writes, AutoTrade, or broker APIs from WarRoom.
...
from btcts.apps.operator_ui.components.prediction_warroom_latest_prediction_summary_widget_operator_value_summary_panel import LATEST_PREDICTION_SUMMARY_WIDGET_OPERATOR_VALUE_SUMMARY_PANEL_VERSION, build_latest_prediction_summary_widget_operator_value_summary_packet
...
SECTION_TITLE = "Prediction WarRoom latest summary operator value summary"
RENDER_FUNCTION = "_render_prediction_warroom_latest_prediction_summary_widget_operator_value_summary_section"
EXPECTED_VALUES = {
...
EXPECTED_COMPACT_LINE = "latest_prediction_summary_widget: run=ps_q18i_fixture_run / market=BTC-USD / generated_at=2026-06-22T00:00:00Z / source=fixture://ps_q18i/latest_prediction.json"
...
def _safe_q18j_boundary(report: Mapping[str, Any]) -> tuple[bool, list[str]]:
    ...
    for key, value in {
        ...
        if report.get(key) != value:
            failures.append(f"q18j_value_mismatch:{key}")
    for key in ("latest_prediction_summary_widget_mapped_payload_render_disabled_packet_validation_only", "real_payload_values_visible_in_component_packet"):
        if report.get(key) is not True:
            failures.append(f"q18j_true_boundary_missing:{key}")
    for key in ("streamlit_render_invoked", "actual_source_read_invoked_by_validation", "payload_reparse_allowed", "d_hot_directory_scan_allowed", "refresh_invocation_allowed", "runtime_artifact_write_allowed", "parameter_apply_allowed", "broker_private_api_allowed"):
        if report.get(key) is not False:
            failures.append(f"q18j_boundary_not_false:{key}")
    return not failures, failures


def _safe_summary_packet(packet: Mapping[str, Any]) -> tuple[bool, list[str]]:
    failures: list[str] = []
    if packet.get("panel_version") != LATEST_PREDICTION_SUMMARY_WIDGET_OPERATOR_VALUE_SUMMARY_PANEL_VERSION:
        failures.append("panel_version_mismatch")
    if packet.get("ok") is not True:
        failures.append("summary_packet_not_ok")
    if packet.get("summary_row_count") != 7:
        failures.append("summary_row_count_mismatch")
    for key in ("latest_prediction_summary_widget_operator_value_summary_mount_only", "warroom_operator_summary_rows_ready", "operator_summary_display_only", "mapped_payload_values_display_only"):
        if packet.get(key) is not True:
            failures.append(f"summary_true_boundary_missing:{key}")
    for key in ("q18j_validation_invoked_by_mount", "component_packet_builder_invoked_by_mount", "component_packet_builder_allowed_by_mount", "streamlit_render_invoked", "real_prediction_widget_rendering_allowed", "actual_source_read_invoked_by_mount", "actual_source_read_allowed_by_mount", "payload_reparse_allowed", "d_hot_directory_scan_allowed", "refresh_invocation_allowed", "runtime_artifact_write_allowed", "parameter_apply_allowed", "broker_private_api_allowed"):
        if packet.get(key) is not False:
            failures.append(f"summary_boundary_not_false:{key}")
    return not failures, failures
...
def _page_validation(page_text: str) -> tuple[bool, list[str]]:
    ...
    if page_text.count(f"{RENDER_FUNCTION}(") != 2:
        failures.append("operator_value_summary_render_function_should_have_definition_and_page_body_call_only")
...
def build_report(*, supplied_q18j_report: Mapping[str, Any] | Any | None = None, page_text: str | None = None, use_observed_fixture: bool = False) -> dict[str, Any]:
    ...
    summary_packet = build_latest_prediction_summary_widget_operator_value_summary_packet(supplied_validation_report=q18j_report) if safe_q18j else {}
    safe_summary, summary_failures = _safe_summary_packet(summary_packet) if summary_packet else (False, [])
    page_summary_packet = build_latest_prediction_summary_widget_operator_value_summary_packet()
    safe_page_summary, page_summary_failures = _safe_summary_packet(page_summary_packet)
    actual_page_text = page_text if page_text is not None else (_read_warroom_page() if safe_summary and safe_page_summary else "")
    page_valid, page_failures = _page_validation(actual_page_text) if safe_summary and safe_page_summary else (False, [])
    ok = bool(safe_q18j and safe_summary and safe_page_summary and page_valid)
    return {
        "ok": ok,
        "checker": CHECKER,
        "checker_version": CHECKER_VERSION,
        "operator_value_summary_mount_version": OPERATOR_VALUE_SUMMARY_MOUNT_VERSION,
        "stage": "latest_prediction_summary_widget_operator_value_summary_mount_before_real_rendering_refresh_and_writes",
        ...
        "summary_packet_valid": safe_summary,
        "summary_validation_failures": summary_failures,
        "page_summary_packet_valid": safe_page_summary,
        "page_summary_validation_failures": page_summary_failures,
        ...
        "summary_row_count": int(summary_packet.get("summary_row_count") or 0) if summary_packet else 0,
        "page_summary_row_count": int(page_summary_packet.get("summary_row_count") or 0),
        "values_supplied": bool(summary_packet.get("values_supplied")) if summary_packet else False,
        "page_values_supplied": bool(page_summary_packet.get("values_supplied")),
        "compact_line_ready": bool(summary_packet.get("compact_line_ready")) if summary_packet else False,
        "page_compact_line_ready": bool(page_summary_packet.get("compact_line_ready")),
        "compact_line": str(summary_packet.get("compact_line") or "") if summary_packet else "",
        "recommended_first_validation": "latest_prediction_summary_widget_operator_value_summary_mount_guard" if ok else "",
        "recommended_next_slice": "PS-Q18N latest summary operator value summary close guard or first real source handoff preflight; real widget rendering, refresh invocation, confidence increase, and parameter staging/apply remain deferred.",
        "human_interpretation": "PS-Q18M mounts WarRoom operator-readable value summary rows for latest_prediction_summary_widget mapped payload values only. The page mount never runs Q18J, never invokes the component packet builder, never invokes Streamlit rendering, never reads sources, never discovers D-hot, never refreshes, writes, stages/applies parameters, appends ledgers, triggers AutoTrade, or calls broker APIs.",
        **EXPECTED_VALUES,
        "read_only": True,
        "non_executing": True,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="PS-Q18M latest prediction summary widget operator value summary mount")
