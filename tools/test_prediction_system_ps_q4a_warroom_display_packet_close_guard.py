# path: ./tools/test_prediction_system_ps_q4a_warroom_display_packet_close_guard.py
# desc: Close guard for PS-Q4A WarRoom display packet contract.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/prediction/warroom_display_packet.py",
...
GUARDS = (
    ...
    REPO_ROOT / "tools/test_prediction_system_ps_q3c_signal_strength_bands_close_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q4A WarRoom display packet close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q4A WarRoom display packet close guard")
    return 0


def test_prediction_system_ps_q4a_warroom_display_packet_close_guard() -> None:
    assert main() == 0
