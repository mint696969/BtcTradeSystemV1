# path: ./tools/test_phase4a_autotrade_milestone_gf_authorization_grant_status_close_guard.py
# desc: Close guard for S117 authorization grant/status packet. Syntax-checks touched files and reruns focused GF guard.
...
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gf_authorization_grant_status_guard.py"

SELF = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gf_authorization_grant_status_close_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        ...
        failures.append("focused GF authorization grant/status guard did not close")

    payload = {

        "ok": not failures,

        "phase": "phase4a_autotrade_milestone_gf_authorization_grant_status_close_guard",
