# path: ./tools/test_prediction_system_ps_q8d_guarded_warroom_page_insertion_close_guard.py
# desc: Close guard for PS-Q8D guarded folded WarRoom page insertion.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_prediction_system_ps_q8d_guarded_warroom_page_insertion_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q8d_guarded_warroom_page_insertion_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q8c_warroom_page_insertion_contract_guard.py",
...
def main() -> int:
    ...
    failures.extend(f"protected runtime dirty: {line}" for line in proc.stdout.splitlines() if any(prefix in line for prefix in PROTECTED_PREFIXES))
    if failures:
        print("[FAIL] Prediction System PS-Q8D guarded WarRoom page insertion close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q8D guarded WarRoom page insertion close guard")
    return 0


def test_prediction_system_ps_q8d_guarded_warroom_page_insertion_close_guard() -> None:
    assert main() == 0
