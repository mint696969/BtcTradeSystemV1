# path: ./tools/test_prediction_system_ps_q9z_hot_source_probe_close_guard.py
# desc: Close guard for PS-Q9Z hot source probe.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_hot_source_probe.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9z_hot_source_probe_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9z_hot_source_probe_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9z_hot_source_probe_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9Z hot source probe close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9Z hot source probe close guard")
    return 0


def test_prediction_system_ps_q9z_hot_source_probe_close_guard() -> None:
    assert main() == 0
