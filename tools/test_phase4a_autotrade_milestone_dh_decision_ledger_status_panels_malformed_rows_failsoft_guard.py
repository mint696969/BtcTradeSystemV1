# path: ./tools/test_phase4a_autotrade_milestone_dh_decision_ledger_status_panels_malformed_rows_failsoft_guard.py
# desc: Guard Observer/Shadow/Forecast status panels render fail-soft with malformed decision ledger rows and remain read-only.
...
OBSERVER_PANEL_FIELDS = (
    "skipped_rows",
    "latest_run_id",
    "latest_finished_at",
    "latest_completed_cycles",
    "latest_appended_shadow_decision_count",
    "latest_appended_forecast_outcome_count",
    "latest_duplicate_snapshot_skipped_count",
    "blocked_by_counts",
    "error_samples",
...
SHADOW_PANEL_FIELDS = (
    ...
    "latest_forecast_id",
    "latest_action",
    "latest_forecast_direction",
    "latest_forecast_confidence",
    "latest_risk_allowed",
    "latest_executable",
    "action_counts",
    "forecast_confidence_counts",
    "blocked_by_counts",
    "reason_code_counts",
    "error_samples",
    "would_send_to_broker",
...
FORECAST_PANEL_FIELDS = (
    ...
    "latest_forecast_direction",
    "latest_forecast_confidence",
    "latest_actual_snapshot_id",
    "latest_divergence_reasons",
...
def shadow_row(decision_id: str, action: str, blocker: str = "") -> dict[str, Any]:
    return {
        ...
        "final_action": action,
        "reason_codes": ["guard", "decision_ledger_status_panels_malformed_rows", decision_id],
        "blocked_by": [blocker] if blocker else [],
        "would_order": None,
    }
...
def import_and_render_panels(fake_st: FakeStreamlit) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        module._render_observer_run_status()
        module._render_shadow_decision_status()
        module._render_forecast_calibration_status()
        return None, None
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_decision_ledger_status_panels_malformed_rows_hot"
    ...
    try:
        ...
        for path in (command_path, mode_state_path, observer_path, shadow_path, outcome_path):
            if path.exists():
                path.unlink()
        write_jsonl_with_malformed(observer_path, [observer_row("obs_dh_old", -20, "dh_old_observer_blocker"), observer_row("obs_dh_latest", -5, "dh_latest_observer_blocker")])
        write_jsonl_with_malformed(shadow_path, [shadow_row("dh_old", "WAIT", "dh_old_shadow_blocker"), shadow_row("dh_latest", "WAIT", "dh_latest_shadow_blocker")])
        write_jsonl_with_malformed(outcome_path, [outcome_row("fcst_dh_old", "hit", "medium"), outcome_row("fcst_dh_latest", "miss", "high")])
        ...
        _module, ui_exception = import_and_render_panels(fake_st)
    ...
    observer_panel_source = function_source(UI_FILE, "_render_observer_run_status")
    shadow_panel_source = function_source(UI_FILE, "_render_shadow_decision_status")
    forecast_panel_source = function_source(UI_FILE, "_render_forecast_calibration_status")
    ...
    checks = {
        "observer_panel_summary_skips_malformed_and_keeps_latest_valid_row": observer_before.get("total_rows") == 2 and observer_before.get("skipped_rows") == 1 and tuple(observer_before.get("error_samples") or ()) != () and observer_before.get("latest_run_id") == "obs_dh_latest" and "dh_latest_observer_blocker" in tuple(observer_before.get("latest_blocked_by") or ()),
        "shadow_panel_summary_skips_malformed_and_keeps_latest_valid_row": shadow_before.get("total_rows") == 2 and shadow_before.get("skipped_rows") == 1 and tuple(shadow_before.get("error_samples") or ()) != () and shadow_before.get("latest_decision_id") == "dh_latest" and shadow_before.get("latest_action") == "WAIT" and shadow_before.get("blocked_by_counts", {}).get("dh_latest_shadow_blocker") == 1,
        "forecast_panel_summary_ignores_malformed_and_keeps_latest_valid_calibration": outcome_before.get("total_rows") == 2 and outcome_before.get("latest_forecast_id") == "fcst_dh_latest" and outcome_before.get("latest_result") == "miss" and (outcome_before.get("calibration") or {}).get("total_forecast_count") == 2 and (outcome_before.get("calibration") or {}).get("hit_rate") == 0.5,
        "ui_decision_status_panels_render_with_malformed_rows": ui_exception is None and fake_st.subheaders == ["Observer Runs", "Shadow Decision Ledger", "Forecast Outcomes / Calibration"] and fake_st.metric_call_count >= 15 and fake_st.markdown_call_count >= 3,
        "decision_status_panels_do_not_append_any_ledgers": after_command_count == before_command_count == 0 and after_mode_count == before_mode_count == 0 and after_observer_line_count == before_observer_line_count == 3 and after_shadow_line_count == before_shadow_line_count == 3 and after_outcome_line_count == before_outcome_line_count == 3,
        "decision_status_summaries_after_render_are_unchanged": observer_after == observer_before and shadow_after == shadow_before and outcome_after == outcome_before,
        "observer_shadow_forecast_sources_are_failsoft_readonly": bool(observer_summary_source) and "skipped += 1" in observer_summary_source and "read_only=True" in observer_summary_source and bool(shadow_read_source) and "skipped += 1" in shadow_read_source and bool(shadow_summary_source) and "read_shadow_decision_rows" in shadow_summary_source and "read_only=True" in shadow_summary_source and bool(forecast_read_source) and "except Exception" in forecast_read_source and bool(forecast_summary_source) and "read_forecast_outcome_links" in forecast_summary_source and "read_only=True" in forecast_summary_source,
        "ui_observer_panel_displays_fields_and_no_runner_broker": bool(observer_panel_source) and all(field in observer_panel_source for field in OBSERVER_PANEL_FIELDS) and not any(token in observer_panel_source for token in FORBIDDEN_PANEL_TOKENS),
        "ui_shadow_panel_displays_fields_and_no_runner_broker": bool(shadow_panel_source) and all(field in shadow_panel_source for field in SHADOW_PANEL_FIELDS) and not any(token in shadow_panel_source for token in FORBIDDEN_PANEL_TOKENS),
        "ui_forecast_panel_displays_fields_and_no_runner_broker": bool(forecast_panel_source) and all(field in forecast_panel_source for field in FORECAST_PANEL_FIELDS) and not any(token in forecast_panel_source for token in FORBIDDEN_PANEL_TOKENS),
    }
    ...
    failures.extend(f"protected lower-layer dirty during milestone DH: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_dh_decision_ledger_status_panels_malformed_rows_failsoft_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "observer_panel_summary_skips_malformed_and_keeps_latest_valid_row": checks["observer_panel_summary_skips_malformed_and_keeps_latest_valid_row"],
            "shadow_panel_summary_skips_malformed_and_keeps_latest_valid_row": checks["shadow_panel_summary_skips_malformed_and_keeps_latest_valid_row"],
            "forecast_panel_summary_ignores_malformed_and_keeps_latest_valid_calibration": checks["forecast_panel_summary_ignores_malformed_and_keeps_latest_valid_calibration"],
            "ui_decision_status_panels_render_with_malformed_rows": checks["ui_decision_status_panels_render_with_malformed_rows"],
            "decision_status_panels_do_not_append_any_ledgers": checks["decision_status_panels_do_not_append_any_ledgers"],
            "decision_status_summaries_after_render_are_unchanged": checks["decision_status_summaries_after_render_are_unchanged"],
            "observer_shadow_forecast_sources_are_failsoft_readonly": checks["observer_shadow_forecast_sources_are_failsoft_readonly"],
            "ui_observer_panel_displays_fields_and_no_runner_broker": checks["ui_observer_panel_displays_fields_and_no_runner_broker"],
            "ui_shadow_panel_displays_fields_and_no_runner_broker": checks["ui_shadow_panel_displays_fields_and_no_runner_broker"],
            "ui_forecast_panel_displays_fields_and_no_runner_broker": checks["ui_forecast_panel_displays_fields_and_no_runner_broker"],
        ...
        "before_counts": {
            "command": before_command_count,
            "mode_state": before_mode_count,
            "observer_lines": before_observer_line_count,
            "shadow_lines": before_shadow_line_count,
            "outcome_lines": before_outcome_line_count,
        },
        "after_counts": {
            "command": after_command_count,
            "mode_state": after_mode_count,
            "observer_lines": after_observer_line_count,
            "shadow_lines": after_shadow_line_count,
            "outcome_lines": after_outcome_line_count,
        },
