# path: ./tools/test_phase4a_execution_review_hint_replay_artifact_snapshot_guard.py
# desc: Guard Execution review hint read-only replay artifact snapshot entry. No UI/runtime/broker wiring.
...
SELF_PATH = "tools/test_phase4a_execution_review_hint_replay_artifact_snapshot_guard.py"
EXECUTION_HELPER_GUARD = "tools/test_phase4a_execution_review_hint_builder_skeleton_guard.py"
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "execution_review_hint_replay_artifact_snapshot"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    required_artifacts = [
        ...
        "execution_review_hint_read_only_snapshot",
    ...
    test_required = [
        "prediction_execution_review_hint_snapshot",
        "execution_context.review_only.row_1",
        "position_review_hint_snapshot.row_1",
        "broker_link_free",
        "account_side_effect_free",
    ]
...
def main() -> int:
    ...
    checks = {
        ...
        "execution_helper_guard": _run_json_guard(EXECUTION_HELPER_GUARD, failures),
        "plain_test": _run_plain_ok(TEST_PATH, failures),
        "source": _check_source(failures),
    }
    payload = {
        "ok": not failures,
        "phase": "phase4a_execution_review_hint_replay_artifact_snapshot_guard",
        "failures": failures,
        "checks": checks,
    }
