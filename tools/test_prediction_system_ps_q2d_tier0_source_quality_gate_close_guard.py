# path: ./tools/test_prediction_system_ps_q2d_tier0_source_quality_gate_close_guard.py
# desc: Close guard for PS-Q2D/PS-Q3-entry Tier 0 source quality gate effects.
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q2D Tier 0 source quality gate close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q2D Tier 0 source quality gate close guard")
    return 0


def test_prediction_system_ps_q2d_tier0_source_quality_gate_close_guard() -> None:
    assert main() == 0
