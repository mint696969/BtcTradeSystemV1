# path: ./tools/test_phase4a_prediction_system_ps_q18ad_close_guard.py
# desc: Close guard for PS-Q18AD latest_prediction_summary_widget schema validation gate blocked by missing source.
...
def main_guard() -> int:
    ...
    if packet.get("ok") is not True:
        failures.append(f"Q18AD packet must be ok: {packet}")
    if packet.get("schema_validation_blocked") is not True:
        failures.append("schema validation blocked must be true")
    if packet.get("schema_validation_gate_row_count") != 12:
        failures.append("schema validation gate row count must be 12")
    ...
    for key in ("filesystem_exists_check_reexecuted", "source_artifact_schema_checked", "actual_source_read_invoked", "payload_parse_allowed", "real_prediction_widget_rendering_allowed", "refresh_invocation_allowed", "runtime_artifact_write_allowed", "autotrade_trigger_allowed", "broker_private_api_allowed"):
        if packet.get(key) is not False:
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18ad_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_schema_validation_gate_blocked_missing_source_before_actual_read_refresh_and_writes",
        "contract": {
            "ps_q18ad_closed": not failures,
            "schema_validation_blocked": True,
            "schema_validation_block_reason": packet.get("schema_validation_block_reason"),
            ...
            "next_slice": "source availability repair or candidate resolver refresh",
        },
...
def test_ps_q18ad_close_guard() -> None:
    assert main_guard() == 0
