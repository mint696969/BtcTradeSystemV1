# path: ./tools/test_phase4a_autotrade_milestone_gv_shadow_adapter_close_guard.py
# desc: Close guard for S133 Shadow adapter preview contract. Syntax-checks touched files and reruns focused GV guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/shadow_adapter.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gv_shadow_adapter_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gv_shadow_adapter_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gv_shadow_adapter_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GV shadow adapter guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gv_shadow_adapter_close_guard",
