# path: ./tools/test_prediction_system_ps_q9g_guarded_lowered_display_packet_ui_mount_close_guard.py
# desc: Close guard for PS-Q9G guarded lowered display-packet UI mount/review panel.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_prediction_system_ps_q9g_guarded_lowered_display_packet_ui_mount_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9g_guarded_lowered_display_packet_ui_mount_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9G guarded lowered display-packet UI mount close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9G guarded lowered display-packet UI mount close guard")
    return 0


def test_prediction_system_ps_q9g_guarded_lowered_display_packet_ui_mount_close_guard() -> None:
    assert main() == 0
