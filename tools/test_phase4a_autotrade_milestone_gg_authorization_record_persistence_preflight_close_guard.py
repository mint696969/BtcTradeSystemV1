# path: ./tools/test_phase4a_autotrade_milestone_gg_authorization_record_persistence_preflight_close_guard.py
# desc: Close guard for S118 authorization record persistence schema/preflight. Syntax-checks touched files and reruns focused GG guard.
...
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gg_authorization_record_persistence_preflight_guard.py"
SELF = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gg_authorization_record_persistence_preflight_close_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GG authorization record persistence preflight guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gg_authorization_record_persistence_preflight_close_guard",
