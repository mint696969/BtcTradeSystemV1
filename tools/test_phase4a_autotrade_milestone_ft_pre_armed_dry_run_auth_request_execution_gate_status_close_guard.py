# path: ./tools/test_phase4a_autotrade_milestone_ft_pre_armed_dry_run_auth_request_execution_gate_status_close_guard.py
# desc: Close guard for S105 Pre-Armed Dry Run authorization request execution gate dry-run/status. Broker-free; non-recording; non-executing; non-authorizing. Short path avoids Windows MAX_PATH.
...
SELF_PATH = "tools/test_phase4a_autotrade_milestone_ft_pre_armed_dry_run_auth_request_execution_gate_status_close_guard.py"
S105_RUN_PATH = "tools/run_sr_fx_pre_armed_dry_run_auth_request_execution_gate_dry_run_status_s105.py"
...
EXPECTED_SLICE_FILES = {S105_RUN_PATH, S105_GUARD_PATH, SELF_PATH}
...
def _run_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"S105 run missing required fragment: {frag}" for frag in missing)
    failures.extend(f"S105 run contains forbidden token: {tok}" for tok in forbidden)
    failures.extend(f"S105 run missing safety literal: {frag}" for frag in false_missing)
...
def _git_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"unexpected dirty file during S105 close: {line}" for line in unexpected)
    return {"lines": lines, "unexpected": unexpected, "expected_slice_files": sorted(EXPECTED_SLICE_FILES)}


def main() -> int:
    failures: list[str] = []
    syntax = {p: _syntax(p, failures) for p in (SELF_PATH, S105_RUN_PATH, S105_GUARD_PATH, S104_GUARD_PATH, COMMAND_STATUS_PATH)}
    ...
    payload = {"ok": not failures, "phase": "phase4a_autotrade_milestone_ft_pre_armed_dry_run_authorization_request_execution_authorization_request_execution_authorization_request_execution_authorization_request_execution_gate_dry_run_status_close_guard", "status": "closed" if not failures else "open", "contract": {"s105_authorization_request_execution_gate_status_present": (REPO_ROOT / S105_RUN_PATH).exists(), "s105_focused_guard_closed": s105_guard.get("ok") is True, "s104_fs_guard_closed": s104_guard.get("ok") is True, "syntax_checked_without_pyc": all(item.get("ok") is True for item in syntax.values()), "broker_free_non_recording_non_executing_non_authorizing": not run_boundary.get("forbidden"), "expected_git_boundary_only": not git_boundary.get("unexpected")}, "syntax": syntax, "run_boundary": run_boundary, "guards": {"s105_ft_guard": s105_guard, "s104_fs_guard": s104_guard}, "git_boundary": git_boundary, "failures": failures}
