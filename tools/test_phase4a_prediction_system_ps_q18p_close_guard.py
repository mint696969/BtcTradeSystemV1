# path: ./tools/test_phase4a_prediction_system_ps_q18p_close_guard.py
# desc: Close guard for PS-Q18P latest_prediction_summary_widget explicit one-source resolver contract preflight.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18p_latest_prediction_summary_widget_one_source_resolver_contract_preflight_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18p_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    if "test_ps_q18p_validates_resolver_contract_from_q18o_fixture" not in unit_text:
        failures.append("unit test must cover Q18O fixture resolver contract")
    if "test_ps_q18p_packet_without_source_is_contract_only_but_candidate_not_ready" not in unit_text:
        failures.append("unit test must cover resolver contract without candidate")
    ...
    if report.get("resolver_contract_validation_failures"):
        failures.append(f"resolver contract validation failures: {report.get('resolver_contract_validation_failures')}")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_one_source_resolver_contract_preflight_guard":
    ...
    if blocked.get("resolver_contract_row_count") != 0:
        failures.append("blocked report should not emit observed resolver contract rows")
    if blocked.get("source_artifact_resolver_invoked") is not False:
        failures.append("blocked report should not invoke source artifact resolver")
    ...
    for forbidden in (
        ...
        "q18o_validation_invoked_by_mount=true",
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18p_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_one_source_resolver_contract_preflight_closed_before_resolution_path_materialization_read_render_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18p_closed": not failures,
            "latest_prediction_summary_widget_one_source_resolver_contract_preflight_only": True,
            "one_source_resolver_contract_preflight_ready": True,
            "resolver_contract_declared": True,
            "one_source_candidate_preserved": True,
            ...
            "resolver_contract_row_count": int(report.get("resolver_contract_row_count") or 0),
            "source_candidate_count": int(report.get("source_candidate_count") or 0),
            "resolver_contract_candidate_ready": bool(report.get("resolver_contract_candidate_ready")),
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18Q explicit one-source resolver dry-run path-shape preflight",
        },
...
def test_ps_q18p_close_guard() -> None:
    assert main_guard() == 0
