# path: ./tools/test_prediction_system_ps_q9b_latest_payload_read_only_loader_close_guard.py
# desc: Close guard for PS-Q9B minimal read-only latest payload loader.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_payload_read_only_loader.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9b_latest_payload_read_only_loader_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9b_latest_payload_read_only_loader_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9b_latest_payload_read_only_loader_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9B latest payload read-only loader close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9B latest payload read-only loader close guard")
    return 0


def test_prediction_system_ps_q9b_latest_payload_read_only_loader_close_guard() -> None:
    assert main() == 0
