# path: ./tools/run_sr_fx_pre_armed_dry_run_approval_record_append_execution_authorization_request_record_execution_gate_dry_run_status.py
# desc: Broker-free Pre-Armed Dry Run authorization request record execution gate dry-run/status from S64 plan. Gate/status only; non-recording; non-authorizing.
...
TOOL = "run_sr_fx_pre_armed_dry_run_approval_record_append_execution_authorization_request_record_execution_gate_dry_run_status"
REPORT_VERSION = "pre_armed_dry_run_approval_record_append_execution_authorization_request_record_execution_gate_dry_run_status.v1"
RECORD_KIND = "pre_armed_dry_run_append_execution_authorization_request_record_draft"
REQUIRED_GATE_ACKS = (
    "confirm_s64_record_dry_run_plan_status_reviewed",
    "confirm_record_execution_gate_is_review_only",
    "confirm_this_gate_does_not_record_authorization_request",
...
def build_pre_armed_dry_run_authorization_request_record_execution_gate_dry_run_status(*, record_dry_run_plan_status: Mapping[str, Any], record_execution_gate_review: Mapping[str, Any], generated_at: str | None = None) -> dict[str, Any]:
    plan = dict(record_dry_run_plan_status)
    review = dict(record_execution_gate_review)
    plan_ok = plan.get("ok") is True
    plan_ready = plan.get("authorization_request_record_dry_run_plan_ready") is True
    plan_request_observed = plan.get("authorization_request_record_dry_run_plan_requested_observed") is True
    plan_blockers = _as_list(plan.get("record_dry_run_plan_blockers"))
    draft = _as_dict(plan.get("authorization_request_record_draft"))
    ...
    plan_safety_ok = (
        ...
        and plan.get("approval_record_append_execution_authorization_request_recorded") is False
        and plan.get("authorization_request_recorded") is False
        and plan.get("approval_record_append_execution_authorized") is False
        and plan.get("approval_record_append_execution_requested") is False
        and plan.get("approval_record_append_executed") is False
    ...
    if review.get("authorization_request_record_execution_requested") is not False:
        review_blockers.append("authorization_request_record_execution_request_must_be_false_in_dry_run_gate")
    if review.get("authorization_request_recorded") is not False:
        review_blockers.append("authorization_request_recorded_must_be_false_in_dry_run_gate")
    if review.get("approval_record_append_execution_authorized") is not False:
        review_blockers.append("authorization_grant_must_be_false_in_dry_run_gate")
    if review.get("approval_record_append_execution_requested") is not False:
        review_blockers.append("append_execution_request_must_be_false_in_dry_run_gate")
    if review.get("approval_record_append_executed") is not False:
        review_blockers.append("append_execution_must_be_false_in_dry_run_gate")
    if review.get("approval_ledger_append_requested") is not False:
        review_blockers.append("approval_ledger_append_request_must_be_false_in_dry_run_gate")
    ...
    if not plan_ok:
        gate_blockers.append("authorization_request_record_dry_run_plan_status_not_ok")
    if not plan_ready:
        gate_blockers.append("authorization_request_record_dry_run_plan_not_ready")
        gate_blockers.extend(plan_blockers)
    if not plan_request_observed:
        gate_blockers.append("authorization_request_record_dry_run_plan_request_not_observed")
    if not plan_safety_ok:
        gate_blockers.append("authorization_request_record_dry_run_plan_safety_contract_not_clear")
    if draft.get("record_kind") != RECORD_KIND:
        gate_blockers.append("authorization_request_record_draft_missing_or_invalid")
    if draft.get("recorded") is not False or draft.get("persisted") is not False or draft.get("authorized") is not False:
        gate_blockers.append("authorization_request_record_draft_not_dry_run_safe")
    if not str(draft.get("evidence_id") or "").startswith("approval_evidence_"):
        gate_blockers.append("invalid_or_missing_evidence_id")
    if draft.get("operator_identity_present") is not True:
        gate_blockers.append("operator_identity_not_present_in_record_draft")
    ...
    decision = "authorization_request_record_execution_gate_dry_run_ready_not_executed" if gate_ready else "authorization_request_record_execution_gate_dry_run_blocked_not_executed"
    return {
        ...
        "decision": decision,
        "authorization_request_record_execution_gate_ready": gate_ready,
        ...
        "record_execution_gate_review_summary": {
            ...
            "approval_ledger_append_request_input": review.get("approval_ledger_append_requested"),
            "command_ledger_append_request_input": review.get("command_ledger_append_requested"),
            "mode_change_request_input": review.get("mode_change_requested"),
        },
        "authorization_request_record_draft": {
            ...
            "source_authorization_requested_by": draft.get("source_authorization_requested_by"),
            "source_authorization_requested_at": draft.get("source_authorization_requested_at"),
            "planned_by": draft.get("planned_by"),
            "planned_at": draft.get("planned_at"),
        ...
        "checks": {
            ...
            "record_execution_gate_blockers_visible_when_blocked": bool(gate_blockers) if not gate_ready else True,
        ...
        "warnings": [
            "authorization_request_record_execution_gate_dry_run_status_is_informational_only",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build broker-free Pre-Armed Dry Run authorization request record execution gate dry-run/status from S64 JSON.")
    parser.add_argument("--record-dry-run-plan-status", required=True, help="Path to S64 authorization request record dry-run plan/status JSON.")
    parser.add_argument("--record-execution-gate-review", required=True, help="Path to record execution gate review JSON. This slice executes and records nothing.")
    parser.add_argument("--out", default="", help="Optional record execution gate/status output JSON path.")
    return parser
...
def main() -> int:
    ...
    try:
        ...
        payload = build_pre_armed_dry_run_authorization_request_record_execution_gate_dry_run_status(record_dry_run_plan_status=plan, record_execution_gate_review=review)
        ...
        payload = {
            ...
            "blocked_by": ["pre_armed_dry_run_authorization_request_record_execution_gate_dry_run_status_failed"],
            "read_only": True,
            "would_send_to_broker": False,
            "mode_changed": False,
            "autotrade_resume_authorized": False,
