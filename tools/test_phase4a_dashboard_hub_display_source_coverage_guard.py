# path: ./tools/test_phase4a_dashboard_hub_display_source_coverage_guard.py
# desc: Guard dashboard hub display source coverage summary read model. No app.py/Streamlit/layout/runtime/broker wiring.
...
MATRIX_CLOSE_GUARD = "tools/test_phase4a_dashboard_hub_display_source_matrix_close_guard.py"
FILES = [
    "btcts_next/src/btcts/apps/operator_ui/hub/display_source_coverage.py",
    "btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_coverage.py",
]
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_coverage"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    required = [
        ...
        "page_source_count_range",
        "source_page_count_range",
        "coverage_ok",
        "summary_widget",
        "review_hint_display",
        "read_only_contract",
    ...
    for fragment in missing:
        failures.append(f"dashboard hub display source coverage missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub display source coverage contains forbidden token: {token}")
...
def _check_app_py_untouched_boundary(failures: list[str]) -> dict[str, object]:
    ...
    for token in hits:
        failures.append(f"app.py must not be wired to dashboard hub source coverage in this slice: {token}")
    return {"hits": hits}


def _run_matrix_close_guard(failures: list[str]) -> dict[str, object]:
    if os.environ.get("BTCTS_DASHBOARD_HUB_SKIP_NESTED_CLOSE_GUARD") == "1":
        return {
            "ok": True,
            "skipped": True,
            "reason": "verified_by_separate_primary_total_guard",
            "path": MATRIX_CLOSE_GUARD,
        }
    return _run_json_guard(MATRIX_CLOSE_GUARD, failures)

def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_coverage_guard.py", failures),
        "compile_files": {rel: _compile(rel, failures) for rel in FILES},
        "matrix_close_guard": _run_matrix_close_guard(failures),
        "plain_test": _run_plain_ok("btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_coverage.py", failures),
        "source": _check_source(failures),
        "app_py_untouched_boundary": _check_app_py_untouched_boundary(failures),
    }
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_coverage_guard",
        "failures": failures,
        "checks": checks,
    }
