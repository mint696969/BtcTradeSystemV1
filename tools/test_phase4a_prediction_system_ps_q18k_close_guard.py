# path: ./tools/test_phase4a_prediction_system_ps_q18k_close_guard.py
# desc: Close guard for PS-Q18K latest_prediction_summary_widget mapped-payload render-disabled packet status row mount.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18k_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_row_mount_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18k_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    if page_text.count("_render_prediction_warroom_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_section(") != 2:
        failures.append("mapped-payload status render function should have definition and page-body call only")
    for forbidden in (
    ...
    if "test_ps_q18k_validates_mapped_payload_packet_status_row_mount_from_q18j_fixture" not in unit_text:
        failures.append("unit test must cover Q18J fixture status row mount")
    if "test_ps_q18k_page_safe_status_packet_does_not_invoke_validation_or_builder" not in unit_text:
        failures.append("unit test must cover page-safe packet no validation/builder")
    ...
    if report.get("status_row_count") != 12:
        failures.append("expected 12 observed status rows")
    ...
    if report.get("observed_component_packet_builder_invoked") is not True:
        failures.append("observed component builder should be true in supplied report path")
    ...
    if report.get("observed_component_packet_state") != "read_only_component_skeleton_render_disabled":
        failures.append("observed component state mismatch")
    if report.get("observed_component_missing_props") != []:
        failures.append("observed component missing props should be empty")
    if report.get("observed_component_source_generated_at") != "2026-06-22T00:00:00Z":
        failures.append("observed component generated_at mismatch")
    if report.get("observed_component_source_artifact_ref") != "fixture://ps_q18i/latest_prediction.json":
        failures.append("observed component artifact ref mismatch")
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_row_mount_guard":
    ...
    for marker in (
        ...
        "observed_component_packet_valid=true",
        "observed_component_packet_state=read_only_component_skeleton_render_disabled",
        "observed_component_missing_props=[]",
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18k_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_row_mount_closed_before_real_rendering_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18k_closed": not failures,
            "latest_prediction_summary_widget_mapped_payload_render_disabled_packet_status_row_mount_only": True,
            "warroom_status_rows_ready": True,
            "validation_report_display_only": True,
            "mapped_payload_render_disabled_packet_status_display_only": True,
            ...
            "observed_component_packet_builder_invoked": bool(report.get("observed_component_packet_builder_invoked")),
            "observed_component_packet_valid": bool(report.get("observed_component_packet_valid")),
            "observed_component_packet_state": report.get("observed_component_packet_state"),
            "observed_component_missing_props": list(report.get("observed_component_missing_props") or []),
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18L first operator-visible latest summary value panel or WarRoom mapped payload value rows",
        },
...
def test_ps_q18k_close_guard() -> None:
    assert main_guard() == 0
