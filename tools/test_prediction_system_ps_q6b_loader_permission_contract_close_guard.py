# path: ./tools/test_prediction_system_ps_q6b_loader_permission_contract_close_guard.py
# desc: Close guard for PS-Q6B latest payload loader dry-run permission contract.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_payload_loader_permission_contract.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q6b_loader_permission_contract_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q6b_loader_permission_contract_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q6b_loader_permission_contract_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q6B loader permission contract close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q6B loader permission contract close guard")
    return 0


def test_prediction_system_ps_q6b_loader_permission_contract_close_guard() -> None:
    assert main() == 0
