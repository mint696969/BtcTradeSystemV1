...
# desc: Phase 4-A Health evidence presentation page wiring guard.
...
SPEC_PATH = "tmp/docs/architecture/PHASE4A_HEALTH_EVIDENCE_PRESENTATION_PAGE_WIRING_2026-05-30.md"
SHARED_GUARD_PATH = "tools/test_phase4a_health_warroom_evidence_consumption_shared_ui_rendering_component_guard.py"
...
TEST_PATH = "btcts_next/src/btcts/apps/operator_ui/tests/test_health_page_evidence_presentation_wiring.py"
...
PATTERNS = [
    ...
    "_snapshot_evidence_presentation_payload",
]
...
def _compile(rel_path: str, failures: List[str]) -> Dict[str, Any]:
    ...
    if not path.exists():
        ...
        return {"ok": False}
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "health_evidence_page_wiring"
...
def _check_docs(failures: List[str]) -> Dict[str, Any]:
    required = {
        SPEC_PATH: [
            "Health page evidence presentation wiring only",
            "WarRoom page はまだ触らない",
            'snapshot["current_state_bundle"]["evidence_presentation_payload"]',
            "tools/test_phase4a_health_evidence_presentation_page_wiring_guard.py",
        ],
        HEALTH_PAGE_PATH: [
        ...
        PRIMARY_GUARD_PATH: [
            "tools/test_phase4a_health_evidence_presentation_page_wiring_guard.py",
            "health_evidence_presentation_page_wiring_guard",
        ],
    }
...
def _check_wiring_boundaries(failures: List[str]) -> Dict[str, Any]:
    ...
    for token in health_hits:
        failures.append(f"health_page contains forbidden evidence wiring token: {token}")
    # WarRoom evidence presentation wiring is allowed after the Health page wiring close.
    warroom_hits = []

    health_slot_pos = health.find('health_widget_slot("evidence_presentation_panel")')
    section_def_pos = health.find("def _render_evidence_presentation_section()")
    if section_def_pos < 0 or health_slot_pos < section_def_pos:
        failures.append("health_page evidence presentation slot must be after section function definition")

    return {"health_hits": health_hits, "warroom_hits": warroom_hits, "section_def_pos": section_def_pos, "health_slot_pos": health_slot_pos}
    ...
    for rel_root in FORBIDDEN_RUNTIME_ROOTS:
        ...
        for path in root.rglob("*.py"):
            ...
            if any(pattern in text for pattern in PATTERNS):
                ...
                hits.append(rel)
                failures.append(f"runtime path references evidence presentation wiring: {rel}")
...
def main() -> int:
    ...
    checks = {
        ...
        "compile_self": _compile("tools/test_phase4a_health_evidence_presentation_page_wiring_guard.py", failures),
        "shared_guard": _run_json_guard(SHARED_GUARD_PATH, failures),
    ...
    summary = {
        "phase": "phase4a_health_evidence_presentation_page_wiring_guard",
        "checks": checks,
        "failures": failures,
        "ok": len(failures) == 0,
    }
