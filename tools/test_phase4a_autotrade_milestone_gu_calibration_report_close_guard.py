# path: ./tools/test_phase4a_autotrade_milestone_gu_calibration_report_close_guard.py
# desc: Close guard for S132 calibration/missed-opportunity report contracts. Syntax-checks touched files and reruns focused GU guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/calibration.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gu_calibration_report_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gu_calibration_report_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gu_calibration_report_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GU calibration report guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gu_calibration_report_close_guard",
