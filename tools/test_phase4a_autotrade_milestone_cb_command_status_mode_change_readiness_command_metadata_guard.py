# path: ./tools/test_phase4a_autotrade_milestone_cb_command_status_mode_change_readiness_command_metadata_guard.py
# desc: Guard Command Requests status surfaces latest mode-change readiness command metadata. Read-only; no append/runner/broker.
    ...
    return json.dumps(
        {
            ...
            "observer_latest_run_id": "obs_cb_blocked_off",
            "observer_latest_blocked_by": EXPECTED_NOTE_BLOCKED_BY[1:],
...
def command_row(*, command_id: str, command_type: str, accepted: bool, requested_by: str, requested_at: str, current_mode: str, target: str | None, blocked_by: list[str], note: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "target": target,
            "confirmation": True,
            "reason_codes": ["guard", "readiness_command_metadata"],
            "note": note,
...
def write_command_rows(path: Path) -> None:
    rows = [
        command_row(
            command_id="cmd_cb_live_request",
            ...
            blocked_by=EXPECTED_COMMAND_BLOCKED_BY,
            note=readiness_note(),
        ),
        command_row(
            command_id="cmd_cb_halt_after_live_request",
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_command_status_mode_change_readiness_command_metadata_hot"
    ...
    checks = {
        "summary_dataclass_has_readiness_command_metadata_fields": all(field in status_text for field in METADATA_FIELDS),
        "summary_preserves_latest_mode_change_readiness_command_metadata": summary_data.get("latest_mode_change_readiness_command_id") == "cmd_cb_live_request" and summary_data.get("latest_mode_change_readiness_requested_by") == "operator_ui" and isinstance(summary_data.get("latest_mode_change_readiness_requested_at"), str) and summary_data.get("latest_mode_change_readiness_accepted") is False and tuple(summary_data.get("latest_mode_change_readiness_command_blocked_by") or ()) == tuple(EXPECTED_COMMAND_BLOCKED_BY),
        "summary_preserves_note_context_separately_from_command_metadata": tuple(summary_data.get("latest_mode_change_readiness_blocked_by") or ()) == tuple(EXPECTED_NOTE_BLOCKED_BY) and summary_data.get("latest_mode_change_readiness_observer_run_id") == "obs_cb_blocked_off",
        "ui_command_requests_displays_latest_mode_change_readiness_command_metadata": all(field in ui_source for field in METADATA_FIELDS),
        "summary_latest_command_is_newer_non_mode": summary_data.get("latest_command_id") == "cmd_cb_halt_after_live_request" and summary_data.get("latest_command_type") == "REQUEST_HALT_NEW",
    ...
    failures.extend(f"protected lower-layer dirty during milestone CB: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_cb_command_status_mode_change_readiness_command_metadata_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "summary_dataclass_has_readiness_command_metadata_fields": checks["summary_dataclass_has_readiness_command_metadata_fields"],
            "summary_preserves_latest_mode_change_readiness_command_metadata": checks["summary_preserves_latest_mode_change_readiness_command_metadata"],
            "summary_preserves_note_context_separately_from_command_metadata": checks["summary_preserves_note_context_separately_from_command_metadata"],
            "ui_command_requests_displays_latest_mode_change_readiness_command_metadata": checks["ui_command_requests_displays_latest_mode_change_readiness_command_metadata"],
            "summary_latest_command_is_newer_non_mode": checks["summary_latest_command_is_newer_non_mode"],
