# path: ./tools/test_phase4a_prediction_system_ps_q18u_close_guard.py
# desc: Close guard for PS-Q18U latest_prediction_summary_widget one-source no-read existence-check gate-open contract.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18u_latest_prediction_summary_widget_one_source_no_read_existence_check_gate_open_contract_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18u_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    for marker in (
        ...
        "one_source_no_read_existence_check_gate_open_contract_ready",
    ...
    if "test_ps_q18u_validates_gate_open_contract_from_q18t_fixture" not in unit_text:
        failures.append("unit test must cover Q18T fixture gate-open contract")
    if "test_ps_q18u_packet_without_source_is_contract_only_but_candidate_not_ready" not in unit_text:
        failures.append("unit test must cover gate-open contract packet without candidate")
    ...
    if report.get("gate_open_contract_validation_failures"):
        failures.append(f"gate-open contract validation failures: {report.get('gate_open_contract_validation_failures')}")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_one_source_no_read_existence_check_gate_open_contract_guard":
    ...
    if blocked.get("gate_open_contract_row_count") != 0:
        failures.append("blocked report should not emit observed gate-open contract rows")
    if blocked.get("existence_check_execution_gate_open_allowed") is not False:
        failures.append("blocked report should not allow gate opening")
    if blocked.get("existence_check_execution_gate_opened") is not False:
    ...
    for forbidden in (
        "existence_check_execution_gate_open_allowed=true",
        "existence_check_execution_gate_opened=true",
        ...
        "q18t_validation_invoked_by_mount=true",
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18u_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_one_source_no_read_existence_check_gate_open_contract_before_gate_open_exists_schema_read_render_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18u_closed": not failures,
            "latest_prediction_summary_widget_one_source_no_read_existence_check_gate_open_contract_only": True,
            "one_source_no_read_existence_check_gate_open_contract_ready": True,
            "existence_check_gate_open_contract_declared": True,
            "existence_check_execution_gate_open_allowed": False,
            ...
            "gate_open_contract_row_count": int(report.get("gate_open_contract_row_count") or 0),
            "source_candidate_count": int(report.get("source_candidate_count") or 0),
            "gate_open_contract_candidate_ready": bool(report.get("gate_open_contract_candidate_ready")),
            "existence_gate_open_contract_kind": report.get("existence_gate_open_contract_kind"),
            "existence_gate_open_contract_state": report.get("existence_gate_open_contract_state"),
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18V explicit one-source no-read filesystem existence check dry-run plan",
        },
...
def test_ps_q18u_close_guard() -> None:
    assert main_guard() == 0
