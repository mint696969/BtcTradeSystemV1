# path: ./tools/test_prediction_system_ps_e4_tradeflow_opportunity_context_close_guard.py
# desc: Close guard for PS-E4 tradeflow opportunity context syntax and importability.
...
FILES = [
    ...
    ROOT / "tools" / "test_prediction_system_ps_e4_tradeflow_opportunity_context_close_guard.py",
]


def test_ps_e4_files_compile_and_import() -> None:
    for path in FILES:
        if not path.exists():
    ...
    assert hasattr(rule, "_opportunity_participation")
    assert hasattr(system, "build_prediction_system_result")


def main() -> int:
    test_ps_e4_files_compile_and_import()
    print("[OK] Prediction System PS-E4 tradeflow opportunity context close guard passed")
    return 0
