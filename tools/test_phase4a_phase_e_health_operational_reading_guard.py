...
# desc: Phase 4-A Phase E Health operational reading surfacing guard.
...
def _check_health_operational_reading_shape(failures: List[str]) -> Dict[str, Any]:
    rel_path = "btcts_next/src/btcts/apps/operator_ui/components/health_top_panels.py"
    path = REPO_ROOT / rel_path
    ...
    for fragment in required_fragments:
        if fragment not in text:
            ...
            failures.append(f"Health operational reading fragment missing: {fragment}")

    for fragment in forbidden_fragments:
        if fragment in text:
            forbidden.append(fragment)
            failures.append(f"Health operational reading must remain review-only: {fragment}")
...
def _check_health_page_wiring(failures: List[str]) -> Dict[str, Any]:
    rel_path = "btcts_next/src/btcts/apps/operator_ui/views/health_page.py"
    path = REPO_ROOT / rel_path
    text = path.read_text(encoding="utf-8") if path.exists() else ""
    ...
    for fragment in required_fragments:
        if fragment not in text:
            missing.append(fragment)
            failures.append(f"Health page operational reading wiring missing: {fragment}")
...
def _check_test_contract(failures: List[str]) -> Dict[str, Any]:
    rel_path = "btcts_next/src/btcts/apps/operator_ui/tests/test_health_top_panels_digest_caption.py"
    path = REPO_ROOT / rel_path
    text = path.read_text(encoding="utf-8") if path.exists() else ""
    ...
    required_fragments = [
        ...
        "health_operational_reading=allow_structural_use",
        "trust=trusted",
        ...
        "review_mode=operator_review_only",
        "execution=not_instruction",
        "health_operational_reading unavailable",
    ]
    ...
    for fragment in required_fragments:
        if fragment not in text:
            missing.append(fragment)
            failures.append(f"Health operational reading test fragment missing: {fragment}")
...
def main() -> int:
    ...
    operational_shape = _check_health_operational_reading_shape(failures)
    page_wiring = _check_health_page_wiring(failures)
    test_contract = _check_test_contract(failures)

    summary = {
        "phase": "phase4a_phase_e_health_operational_reading_guard",
        "checks": {
            "compile": compile_result,
            "plain_tests": plain_test_results,
            "operational_shape": operational_shape,
            "page_wiring": page_wiring,
