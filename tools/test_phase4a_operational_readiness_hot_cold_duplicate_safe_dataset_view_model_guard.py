# path: ./tools/test_phase4a_operational_readiness_hot_cold_duplicate_safe_dataset_view_model_guard.py
# desc: Guard duplicate-safe Hot/Cold logical dataset view read-only model skeleton. No file reader/training/copy/delete.
...
SELF_PATH = "tools/test_phase4a_operational_readiness_hot_cold_duplicate_safe_dataset_view_model_guard.py"
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, Any]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "hot_cold_duplicate_safe_dataset_view_model"
...
def _check_source_shape(failures: list[str]) -> dict[str, Any]:
    ...
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in text]
    for fragment in missing:
        failures.append(f"model source missing duplicate-safe view fragment: {fragment}")
    for token in forbidden_hits:
        failures.append(f"model source contains forbidden reader/executor token: {token}")
...
"""
    ...
    if not ok:
        failures.append("duplicate-safe dataset view model probe must deduplicate and stay read-only")
...
def main() -> int:
    ...
    checks = {
        ...
        "entry_close_guard": {"verified_before_model_guard": True, "path": ENTRY_CLOSE_GUARD_PATH},
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_operational_readiness_hot_cold_duplicate_safe_dataset_view_model_guard",
        "failures": failures,
        "checks": checks,
    }
