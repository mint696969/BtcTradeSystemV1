# path: ./tools/diagnose_phase4a_prediction_system_ps_q24k_autotrade_read_only_status_page_actual_page_wiring_human_gate_decision.py
# desc: No-write diagnostic for AutoTrade prediction status page human-gate decision. Gate remains not granted.
...
from btcts.apps.operator_ui.components.autotrade_prediction_status_page_human_gate_decision import (  # noqa: E402
...
DIAGNOSTIC_VERSION = "prediction_warroom.ps_q24k_autotrade_read_only_status_page_actual_page_wiring_human_gate_decision.v1"
AUTOTRADE_PAGE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/views/autotrade_page.py"
DECISION_MODULE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/autotrade_prediction_status_page_human_gate_decision.py"
DECISION_MODULE_REF = "autotrade_prediction_status_page_human_gate_decision"
DECISION_BUILDER_REF = "build_autotrade_prediction_status_page_human_gate_decision_packet"
...
def _sample_status() -> AutoTradePredictionPreviewStatus:
    return AutoTradePredictionPreviewStatus(
        status_id="ps_q24k_human_gate_decision_sample_status",
        generated_at="2026-06-29T00:00:00Z",
        status_state="ok",
        preview_id="ps_q24k_preview",
        readiness_id="ps_q24k_readiness",
...
def run_autotrade_read_only_status_page_actual_page_wiring_human_gate_decision() -> dict[str, Any]:
    branch = _git(["rev-parse", "--abbrev-ref", "HEAD"])
    head = _git(["rev-parse", "--short", "HEAD"])
    status_short = _git(["status", "--short", "--untracked-files=all"])
    q24j = run_autotrade_read_only_status_page_actual_page_wiring_explicit_gate_required()
    ...
    packet = build_autotrade_prediction_status_page_human_gate_decision_packet(_sample_status(), autotrade_page_text=page_text)
    ...
    blockers: list[str] = []
    if q24j.get("ready") is not True:
        blockers.append("q24j_actual_page_wiring_patch_plan_ready_required")
    for item in list(q24j.get("blockers") or []):
        blockers.append(f"q24j:{item}")
    if AUTOTRADE_PREDICTION_STATUS_PAGE_HUMAN_GATE_DECISION_CONTRACT.get("decision_type") != "autotrade_prediction_status_page_actual_page_wiring_human_gate_decision_packet":
        blockers.append("human_gate_decision_contract_type_required")
    if packet.get("human_gate_decision_ready") is not True:
        blockers.append("human_gate_decision_ready_required")
    for marker in ("human_gate_decision_packet_only=true", "explicit_page_change_gate_required=true", "human_gate_decision=not_granted", "human_gate_granted=false", "page_change_authorized=false", "actual_page_wiring_allowed=false", "blocked_until_human_gate=true", "not_page_wiring=true", "not_runtime_wiring=true", "not_ui_rendering=true", "no_command_buttons=true", "no_forms=true", "no_" + "session" + "_state=true", "no_callbacks=true"):
        if marker not in tuple(packet.get("snapshot_lines") or ()): 
            blockers.append(f"human_gate_decision_snapshot_marker_required:{marker}")
    ...
    if packet.get("human_gate_decision_packet_only") is not True or packet.get("not_page_wiring") is not True:
        blockers.append("human_gate_decision_not_page_wiring_required")
    if packet.get("not_runtime_wiring") is not True or packet.get("not_ui_rendering") is not True:
        blockers.append("human_gate_decision_no_runtime_ui_required")
    if packet.get("no_command_buttons") is not True or packet.get("no_forms") is not True or packet.get("no_" + "session" + "_state") is not True or packet.get("no_callbacks") is not True:
        blockers.append("human_gate_decision_no_controls_required")
    if packet.get("autotrade_page_edit_performed_by_this_slice") is not False or packet.get("page_runtime_mount_performed_by_this_slice") is not False:
        blockers.append("human_gate_decision_page_edit_or_mount_must_be_false")
    if packet.get("actual_ui_rendering_performed_by_this_slice") is not False or packet.get("command_surface_changed_by_this_slice") is not False:
        blockers.append("human_gate_decision_render_or_command_change_must_be_false")
    if not _all_false(packet, execution_flags):
        blockers.append("human_gate_decision_execution_flags_false_required")
    if page_wired:
        blockers.append("autotrade_page_human_gate_decision_wiring_unexpected")
    for token in forbidden_module_tokens:
        if token in module_text:
            blockers.append(f"human_gate_decision_module_forbidden_token:{token}")
    ...
    return {
        ...
        "ready": ready,
        "state": "ps_q24k_autotrade_status_page_human_gate_decision_not_granted_ready" if ready else "ps_q24k_autotrade_status_page_human_gate_decision_blocked",
        "blockers": blockers,
        "q24j_actual_page_wiring_patch_plan": {
            "ready": q24j.get("ready"),
            "diagnostic_version": q24j.get("diagnostic_version"),
            "actual_page_wiring_patch_plan": q24j.get("actual_page_wiring_patch_plan"),
            "safety": q24j.get("safety"),
        },
        "human_gate_decision": {
            "decision_type": packet.get("decision_type"),
            "human_gate_decision_ready": packet.get("human_gate_decision_ready"),
            "human_gate_decision_packet_available": packet.get("human_gate_decision_packet_available"),
            ...
            "autotrade_page_contains_decision_module": packet.get("autotrade_page_contains_decision_module"),
            ...
            "human_gate_decision_packet_only": packet.get("human_gate_decision_packet_only"),
...
def main() -> int:
    result = run_autotrade_read_only_status_page_actual_page_wiring_human_gate_decision()
