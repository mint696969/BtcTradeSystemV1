# path: ./tools/test_phase4a_prediction_system_ps_q18r_close_guard.py
# desc: Close guard for PS-Q18R latest_prediction_summary_widget one-source existence-check contract preflight.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18r_latest_prediction_summary_widget_one_source_existence_check_contract_preflight_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18r_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    for marker in (
        ...
        "one_source_existence_check_contract_preflight_ready",
    ...
    if "test_ps_q18r_validates_existence_contract_from_q18q_fixture" not in unit_text:
        failures.append("unit test must cover Q18Q fixture existence-check contract")
    if "test_ps_q18r_packet_without_source_is_contract_only_but_candidate_not_ready" not in unit_text:
        failures.append("unit test must cover existence-check packet without candidate")
    ...
    if report.get("existence_contract_validation_failures"):
        failures.append(f"existence contract validation failures: {report.get('existence_contract_validation_failures')}")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_one_source_existence_check_contract_preflight_guard":
    ...
    if blocked.get("existence_contract_row_count") != 0:
        failures.append("blocked report should not emit observed existence-check rows")
    ...
    for forbidden in (
        ...
        "q18q_validation_invoked_by_mount=true",
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18r_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_one_source_existence_check_contract_preflight_closed_before_exists_schema_read_render_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18r_closed": not failures,
            "latest_prediction_summary_widget_one_source_existence_check_contract_preflight_only": True,
            "one_source_existence_check_contract_preflight_ready": True,
            "existence_check_contract_declared": True,
            "path_shape_preview_string_only": True,
            ...
            "existence_contract_row_count": int(report.get("existence_contract_row_count") or 0),
            "source_candidate_count": int(report.get("source_candidate_count") or 0),
            "existence_contract_candidate_ready": bool(report.get("existence_contract_candidate_ready")),
            "existence_check_kind": report.get("existence_check_kind"),
            "existence_result_state": report.get("existence_result_state"),
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18S explicit one-source no-read existence check execution gate",
        },
...
def test_ps_q18r_close_guard() -> None:
    assert main_guard() == 0
