# path: ./tools/test_phase4a_prediction_system_ps_q17u_warroom_prediction_widget_page_import_mount_preflight_guard.py
# desc: Focused guard for PS-Q17U WarRoom prediction widget page import/mount implementation preflight.
...
def main_guard() -> int:
    ...
    for widget_id in WIDGET_FAMILY_ORDER:
        if not any(f"render_{widget_id}" in line for line in report.get("future_import_block", [])):
            failures.append(f"missing future import for {widget_id}")
    for row in report.get("mount_invocation_rows", []):
        for key in ("page_import_patch_allowed", "page_body_call_patch_allowed", "warroom_mount_patch_allowed", "streamlit_render_allowed", "actual_source_read_allowed", "refresh_invocation_allowed"):
    ...
    result = {"ok": not failures, "guard": "ps_q17u_warroom_prediction_widget_page_import_mount_preflight_guard", "dirty_paths": sorted(dirty), "unexpected_dirty": sorted(unexpected), "failures": failures}
    print(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True))
    return 0 if not failures else 1


def test_ps_q17u_warroom_prediction_widget_page_import_mount_preflight_guard() -> None:
    assert main_guard() == 0
