# path: ./tools/test_prediction_system_ps_q5b_explanation_widget_groups_close_guard.py
# desc: Close guard for PS-Q5B supplemental explanation widget groups.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_explanation_widget_groups.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q5b_explanation_widget_groups_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q5b_explanation_widget_groups_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q5b_explanation_widget_groups_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q5B explanation widget groups close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q5B explanation widget groups close guard")
    return 0


def test_prediction_system_ps_q5b_explanation_widget_groups_close_guard() -> None:
    assert main() == 0
