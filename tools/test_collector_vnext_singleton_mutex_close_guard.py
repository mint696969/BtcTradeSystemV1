# path: ./tools/test_collector_vnext_singleton_mutex_close_guard.py
# desc: Close guard for collector_vnext singleton mutex hotfix.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/collector_vnext/lock.py",
    REPO_ROOT / "btcts_next/src/btcts/collector_vnext/unified_watchdog.py",
    REPO_ROOT / "btcts_next/src/btcts/collector_vnext/unified_daemon.py",
    REPO_ROOT / "btcts_next/src/btcts/collector_vnext/archive/worker.py",
    REPO_ROOT / "tools/test_collector_vnext_singleton_mutex_guard.py",
    REPO_ROOT / "tools/test_collector_vnext_singleton_mutex_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_collector_vnext_singleton_mutex_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused collector_vnext singleton mutex guard did not close")
    payload = {
        "ok": not failures,
        "phase": "collector_vnext_singleton_mutex_close_guard",
...
def test_collector_vnext_singleton_mutex_close_guard_closes() -> None:
    assert main() == 0
