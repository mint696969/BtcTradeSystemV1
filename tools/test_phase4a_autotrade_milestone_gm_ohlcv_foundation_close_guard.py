# path: ./tools/test_phase4a_autotrade_milestone_gm_ohlcv_foundation_close_guard.py
# desc: Close guard for S124 OHLCV foundation. Syntax-checks touched files and reruns focused GM guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/ohlcv.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gm_ohlcv_foundation_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gm_ohlcv_foundation_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gm_ohlcv_foundation_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GM OHLCV foundation guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gm_ohlcv_foundation_close_guard",
