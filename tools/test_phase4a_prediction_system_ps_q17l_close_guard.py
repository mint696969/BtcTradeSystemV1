# path: ./tools/test_phase4a_prediction_system_ps_q17l_close_guard.py
# desc: Close guard for PS-Q17L scenario-trace semantic mapping contract.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q17l_scenario_trace_semantic_mapping_contract_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    for marker in (
        ...
        "warroom_scenario_trace_release_gate_contract",
        "operator_explanation_trace_taxonomy_contract",
        "blocks_scenario_trace_reliability_claim",
        "blocks_warroom_scenario_trace_widget_reliability",
        "blocks_warroom_widget_rendering",
    ...
    if "test_ps_q17l_builds_scenario_trace_semantic_contract_from_q17b_gap" not in unit_text:
        failures.append("unit test must cover Q17B scenario gap to contract")
    if "test_ps_q17l_blocks_invalid_or_wrong_priority_q17b_report" not in unit_text:
    ...
    if report.get("stage") != "scenario_trace_semantic_mapping_contract_before_reliability_and_widget_rendering":
    ...
    if report.get("source_gap_id") != "scenario_trace_confirmation":
    ...
    for contract_id in EXPECTED_CONTRACTS:
        ...
        if contract_id not in report.get("blocks_scenario_trace_reliability_claim", []):
            failures.append(f"expected scenario reliability blocker: {contract_id}")
        if contract_id not in report.get("blocks_warroom_scenario_trace_widget_reliability", []):
            failures.append(f"expected WarRoom scenario widget reliability blocker: {contract_id}")
    if report.get("blocks_warroom_widget_rendering") != ["warroom_scenario_trace_release_gate_contract"]:
        failures.append("only release gate contract should block widget rendering")
    for contract_id in EXPECTED_P0:
        if priorities.get(contract_id) != "P0":
            failures.append(f"expected P0 contract: {contract_id}")
    if priorities.get("operator_explanation_trace_taxonomy_contract") != "P1":
        failures.append("expected operator explanation taxonomy to be P1")
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q17l_close_guard",
        "phase": "phase3_scenario_trace_semantic_mapping_contract_closed_before_reliability_and_widget_rendering",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q17l_closed": not failures,
            "contract_only": True,
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q17M scenario-trace semantic mapping adapter or parameter-candidate evidence contract",
        },
...
def test_ps_q17l_close_guard() -> None:
    assert main_guard() == 0
