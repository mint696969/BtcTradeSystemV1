# path: ./tools/test_phase4a_autotrade_milestone_id_decision_policy_gate_visibility_registry_chain_completion_final_status_index_close_guard.py
# desc: Close guard for S167 decision policy gate visibility registry chain completion final status index packet. Syntax-checks touched files and reruns focused ID guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_id_decision_policy_gate_visibility_registry_chain_completion_final_status_index_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_id_decision_policy_gate_visibility_registry_chain_completion_final_status_index_close_guard.py",
)
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused ID decision policy gate visibility registry chain completion final status index guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_id_decision_policy_gate_visibility_registry_chain_completion_final_status_index_close_guard",
...
def test_decision_policy_gate_visibility_registry_chain_completion_final_status_index_close_guard_closes() -> None:
    assert main() == 0
