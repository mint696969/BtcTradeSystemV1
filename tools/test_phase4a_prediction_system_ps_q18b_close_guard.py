# path: ./tools/test_phase4a_prediction_system_ps_q18b_close_guard.py
# desc: Close guard for PS-Q18B WarRoom prediction widget bounded actual-source read probe.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18b_warroom_prediction_widget_bounded_actual_source_read_probe_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18b_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    if "test_ps_q18b_probe_reads_one_explicit_fixture_json_only" not in unit_text:
        failures.append("unit test must cover one explicit JSON read")
    if "test_ps_q18b_blocks_without_ack_or_allow" not in unit_text:
        failures.append("unit test must cover ack/allow blocking")
    ...
    if report.get("payload_preview_key_count", 0) <= 0:
        failures.append("payload preview key count should be positive")
    if report.get("probe_validation_failures"):
        failures.append(f"probe validation failures: {report.get('probe_validation_failures')}")
    _assert_boundary(report, failures)

    blocked = build_report()
    if blocked.get("ok") is not False:
        failures.append("missing source should block")
    if blocked.get("actual_file_read_attempted") is not False:
        failures.append("blocked report should not attempt read")
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18b_close_guard",
        "phase": "phase3_warroom_prediction_widget_bounded_actual_source_read_probe_closed_before_warroom_binding_real_widget_rendering_and_refresh",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18b_closed": not failures,
            "bounded_actual_source_read_probe_only": True,
            "single_file_probe_only": True,
            "actual_source_read_allowed": True,
            "actual_file_read_attempted": True,
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18C WarRoom source read probe status row mount or bounded schema-specific probe",
        },
...
def test_ps_q18b_close_guard() -> None:
    assert main_guard() == 0
