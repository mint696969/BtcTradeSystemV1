# path: ./tools/test_prediction_system_ps_q9n_actual_read_to_review_handoff_preflight_close_guard.py
# desc: Close guard for PS-Q9N actual-read-to-review handoff preflight contract.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_actual_read_to_review_packet_handoff_preflight_contract.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9n_actual_read_to_review_handoff_preflight_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9n_actual_read_to_review_handoff_preflight_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9n_actual_read_to_review_handoff_preflight_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9N actual-read-to-review handoff preflight close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9N actual-read-to-review handoff preflight close guard")
    return 0


def test_prediction_system_ps_q9n_actual_read_to_review_handoff_preflight_close_guard() -> None:
    assert main() == 0
