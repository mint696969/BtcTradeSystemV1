# path: ./tools/test_phase4a_dashboard_hub_display_source_diagnostics_close_guard.py
# desc: Close guard for dashboard hub display source read-only diagnostics. No app.py/Streamlit/layout/runtime/broker wiring.
...
FOCUSED_GUARDS = [
    "tools/test_phase4a_dashboard_hub_display_source_diagnostics_guard.py",
]
COMPILE_FILES = [
    "btcts_next/src/btcts/apps/operator_ui/hub/display_source_diagnostics.py",
    "btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_diagnostics.py",
    "tools/test_phase4a_dashboard_hub_display_source_diagnostics_guard.py",
]
SOURCE_SHAPE_FILES = [
    "btcts_next/src/btcts/apps/operator_ui/hub/display_source_diagnostics.py",
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_diagnostics_close"
...
def _check_source_shape(failures: list[str]) -> dict[str, object]:
    ...
    required = [
        ...
        "dashboard_hub_source_diagnostics=",
        "hot_cold_summary",
        "hot_cold_status_label",
        "hot_cold_metadata_detail_status",
        "metadata_only_no_payload_reader_rendering_or_executor_opened",
    ...
    for fragment in missing:
        failures.append(f"dashboard hub source diagnostics bundle missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub source diagnostics bundle contains forbidden token: {token}")
...
def _check_app_py_untouched_boundary(failures: list[str]) -> dict[str, object]:
    ...
    for token in hits:
        failures.append(f"app.py must not be wired to dashboard hub source diagnostics in this slice: {token}")
    return {"hits": hits}
...
def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_diagnostics_close_guard.py", failures),
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_diagnostics_close_guard",
