# path: ./tools/test_phase4a_prediction_system_ps_q18t_close_guard.py
# desc: Close guard for PS-Q18T latest_prediction_summary_widget one-source no-read existence-check execution preflight.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18t_latest_prediction_summary_widget_one_source_no_read_existence_check_execution_preflight_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18t_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    if "test_ps_q18t_validates_execution_preflight_from_q18s_fixture" not in unit_text:
        failures.append("unit test must cover Q18S fixture no-read execution preflight")
    if "test_ps_q18t_packet_without_source_is_preflight_only_but_candidate_not_ready" not in unit_text:
        failures.append("unit test must cover no-read execution preflight packet without candidate")
    ...
    if report.get("ok") is not True:
        failures.append(f"observed fixture no-read execution preflight should be ok: {report}")
    ...
    if report.get("execution_preflight_validation_failures"):
        failures.append(f"execution preflight validation failures: {report.get('execution_preflight_validation_failures')}")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_one_source_no_read_existence_check_execution_preflight_guard":
    ...
    if blocked.get("execution_preflight_row_count") != 0:
        failures.append("blocked report should not emit observed execution preflight rows")
    if blocked.get("existence_check_execution_preflight_would_open_gate") is not False:
        failures.append("blocked report should not open gate")
    ...
    for forbidden in (
        "existence_check_execution_preflight_would_open_gate=true",
        ...
        "q18s_validation_invoked_by_mount=true",
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18t_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_one_source_no_read_existence_check_execution_preflight_closed_before_gate_open_exists_schema_read_render_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18t_closed": not failures,
            "latest_prediction_summary_widget_one_source_no_read_existence_check_execution_preflight_only": True,
            "one_source_no_read_existence_check_execution_preflight_ready": True,
            "existence_check_execution_preflight_declared": True,
            "existence_check_execution_preflight_would_open_gate": False,
            ...
            "execution_preflight_row_count": int(report.get("execution_preflight_row_count") or 0),
            "source_candidate_count": int(report.get("source_candidate_count") or 0),
            "execution_preflight_candidate_ready": bool(report.get("execution_preflight_candidate_ready")),
            "existence_execution_preflight_kind": report.get("existence_execution_preflight_kind"),
            "existence_execution_preflight_state": report.get("existence_execution_preflight_state"),
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18U explicit one-source no-read existence check execution gate-open contract",
        },
...
def test_ps_q18t_close_guard() -> None:
    assert main_guard() == 0
