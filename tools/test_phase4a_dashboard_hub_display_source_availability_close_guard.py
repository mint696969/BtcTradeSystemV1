# path: ./tools/test_phase4a_dashboard_hub_display_source_availability_close_guard.py
# desc: Close guard for dashboard hub display source availability/status read model. No app.py/Streamlit/layout/runtime/broker wiring.
...
FOCUSED_GUARDS = [
    "tools/test_phase4a_dashboard_hub_display_source_availability_guard.py",
]
COMPILE_FILES = [
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_availability_close"
...
def _check_source_shape(failures: list[str]) -> dict[str, object]:
    ...
    for fragment in missing:
        failures.append(f"dashboard hub source availability bundle missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub source availability bundle contains forbidden token: {token}")
...
def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_availability_close_guard.py", failures),
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_availability_close_guard",
