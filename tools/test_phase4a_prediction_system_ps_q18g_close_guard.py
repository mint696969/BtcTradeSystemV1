# path: ./tools/test_phase4a_prediction_system_ps_q18g_close_guard.py
# desc: Close guard for PS-Q18G latest_prediction_summary_widget render-disabled component packet validation.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18g_latest_prediction_summary_widget_render_disabled_packet_validation_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18g_close_guard.py"
EXPECTED_DIRTY = {
...
def main_guard() -> int:
    ...
    if "test_ps_q18g_validates_render_disabled_component_packet_from_q18e_fixture" not in unit_text:
        failures.append("unit test must cover Q18E fixture render-disabled packet")
    if "test_ps_q18g_component_validation_blocks_missing_candidate" not in unit_text:
        failures.append("unit test must cover missing candidate block")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_render_disabled_component_packet_validation_guard":
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18g_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_render_disabled_packet_validation_closed_before_real_rendering_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18g_closed": not failures,
            "latest_prediction_summary_widget_render_disabled_packet_validation_only": True,
            "render_disabled_component_packet_validation_only": True,
            "component_skeleton_packet_only": True,
            ...
            "props_candidate_supplied_to_packet_builder": bool(report.get("props_candidate_supplied_to_packet_builder")),
            "props_value_binding_deferred": True,
            "real_payload_values_bound": False,
            "streamlit_render_allowed": False,
            "streamlit_render_invoked": False,
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18H WarRoom render-disabled latest_prediction_summary_widget packet status row mount or first real payload value mapping preflight",
        },
...
def test_ps_q18g_close_guard() -> None:
    assert main_guard() == 0
