# path: ./tools/test_prediction_system_ps_q9j_guarded_local_observation_hook_close_guard.py
# desc: Close guard for PS-Q9J guarded local WarRoom observation hook.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_local_observation_hook.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9j_guarded_local_observation_hook_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9j_guarded_local_observation_hook_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9j_guarded_local_observation_hook_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9J guarded local observation hook close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9J guarded local observation hook close guard")
    return 0


def test_prediction_system_ps_q9j_guarded_local_observation_hook_close_guard() -> None:
    assert main() == 0
