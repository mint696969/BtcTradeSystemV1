# path: ./tools/test_phase4a_prediction_system_ps_q18h_close_guard.py
# desc: Close guard for PS-Q18H latest_prediction_summary_widget render-disabled packet status row mount.
...
from check_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount import CHECKER_VERSION, RENDER_DISABLED_PACKET_STATUS_ROW_MOUNT_VERSION, WARROOM_PAGE_TARGET, build_report, main

REPO_ROOT = Path(__file__).resolve().parents[1]
WARROOM_PAGE = REPO_ROOT / WARROOM_PAGE_TARGET
COMPONENT = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_prediction_summary_widget_render_disabled_packet_status_panel.py"
TOOL = REPO_ROOT / "tools/check_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount.py"
UNIT = REPO_ROOT / "tools/test_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount.py"
DOC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q18H_LATEST_PREDICTION_SUMMARY_WIDGET_RENDER_DISABLED_PACKET_STATUS_ROW_MOUNT_2026-06-22.md"
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18h_close_guard.py"
EXPECTED_DIRTY = {
    "btcts_next/src/btcts/apps/operator_ui/views/warroom_page.py",
    "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_prediction_summary_widget_render_disabled_packet_status_panel.py",
    "tools/check_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount.py",
    "tools/test_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount.py",
    "docs/strategy/PREDICTION_SYSTEM_PS_Q18H_LATEST_PREDICTION_SUMMARY_WIDGET_RENDER_DISABLED_PACKET_STATUS_ROW_MOUNT_2026-06-22.md",
    "tools/test_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount_guard.py",
    "tools/test_phase4a_prediction_system_ps_q18h_close_guard.py",
}
...
def _assert_boundary(report: dict, failures: list[str]) -> None:
    for key in (
        "read_only",
        "non_executing",
        "latest_prediction_summary_widget_render_disabled_packet_status_row_mount_only",
        "warroom_status_rows_ready",
        "validation_report_display_only",
        "render_disabled_packet_status_display_only",
...
def main_guard() -> int:
    ...
    for marker in (
        "LATEST_PREDICTION_SUMMARY_WIDGET_RENDER_DISABLED_PACKET_STATUS_PANEL_VERSION",
        "build_latest_prediction_summary_widget_render_disabled_packet_status_rows",
    ...
    for marker in (
        ...
        "def _render_prediction_warroom_latest_prediction_summary_widget_render_disabled_packet_status_section() -> None:",
    ...
    if page_text.count("_render_prediction_warroom_latest_prediction_summary_widget_render_disabled_packet_status_section(") != 2:
        failures.append("render-disabled packet status render function should have definition and page-body call only")
    for forbidden in (
    ...
    for marker in (
        ...
        "component_packet_builder_allowed_by_mount",
        "streamlit_render_invoked",
        "PS-Q18I first real payload value mapping preflight",
    ):
        if marker not in tool_text:
            failures.append(f"missing tool marker: {marker}")
    if "test_ps_q18h_validates_render_disabled_packet_status_row_mount_from_q18g_fixture" not in unit_text:
        failures.append("unit test must cover Q18G fixture status row mount")
    if "test_ps_q18h_page_safe_status_packet_does_not_invoke_validation_or_render" not in unit_text:
        failures.append("unit test must cover page-safe packet no validation/render")
    if CLOSE_REL not in focused_text:
        failures.append("focused guard expected dirty set must include close guard")
    if CHECKER_VERSION != "check_phase4a_prediction_system_ps_q18h_latest_prediction_summary_widget_render_disabled_packet_status_row_mount.v1":
        failures.append("checker version mismatch")
    if RENDER_DISABLED_PACKET_STATUS_ROW_MOUNT_VERSION != "latest_prediction_summary_widget_render_disabled_packet_status_row_mount.v1":
    ...
    if report.get("source_q18g_report_valid") is not True:
        failures.append("source Q18G report should validate")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_render_disabled_packet_status_row_mount_guard":
    ...
    if blocked.get("q18g_validation_invoked_by_mount") is not False:
        failures.append("blocked report should not invoke Q18G")
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18h_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_render_disabled_packet_status_row_mount_closed_before_real_rendering_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18h_closed": not failures,
            "latest_prediction_summary_widget_render_disabled_packet_status_row_mount_only": True,
            "warroom_status_rows_ready": True,
            "validation_report_display_only": True,
            "render_disabled_packet_status_display_only": True,
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18I first real payload value mapping preflight for latest_prediction_summary_widget",
        },
...
def test_ps_q18h_close_guard() -> None:
    assert main_guard() == 0
