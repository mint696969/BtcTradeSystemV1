# path: ./tools/test_prediction_system_ps_q5c_payload_schema_validator_close_guard.py
# desc: Close guard for PS-Q5C Prediction WarRoom payload schema validator.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_payload_schema_validator.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q5c_payload_schema_validator_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q5c_payload_schema_validator_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q5c_payload_schema_validator_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q5C payload schema validator close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q5C payload schema validator close guard")
    return 0


def test_prediction_system_ps_q5c_payload_schema_validator_close_guard() -> None:
    assert main() == 0
