# path: ./tools/test_phase4a_autotrade_milestone_hn_decision_policy_gate_static_section_registry_visibility_close_guard.py
# desc: Close guard for S151 decision policy gate static section registry visibility packet. Syntax-checks touched files and reruns focused HN guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hn_decision_policy_gate_static_section_registry_visibility_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hn_decision_policy_gate_static_section_registry_visibility_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hn_decision_policy_gate_static_section_registry_visibility_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HN decision policy gate static section registry visibility guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hn_decision_policy_gate_static_section_registry_visibility_close_guard",
...
def test_decision_policy_gate_static_section_registry_visibility_close_guard_closes() -> None:
    assert main() == 0
