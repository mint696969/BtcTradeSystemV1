# path: ./tools/test_phase4a_autotrade_milestone_fv_pre_armed_dry_run_auth_request_preflight_status_close_guard.py
# desc: Close guard for S107 Pre-Armed Dry Run authorization request preflight/status. Broker-free; non-recording; non-executing; non-authorizing. Short path avoids Windows MAX_PATH.
...
SELF_PATH = "tools/test_phase4a_autotrade_milestone_fv_pre_armed_dry_run_auth_request_preflight_status_close_guard.py"
S107_RUN_PATH = "tools/run_sr_fx_pre_armed_dry_run_auth_request_preflight_status_s107.py"
...
EXPECTED_SLICE_FILES = {S107_RUN_PATH, S107_GUARD_PATH, SELF_PATH}
...
def _run_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"S107 run missing required fragment: {frag}" for frag in missing)
    failures.extend(f"S107 run contains forbidden token: {tok}" for tok in forbidden)
    failures.extend(f"S107 run missing safety literal: {frag}" for frag in false_missing)
...
def _git_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"unexpected dirty file during S107 close: {line}" for line in unexpected)
    return {"lines": lines, "unexpected": unexpected, "expected_slice_files": sorted(EXPECTED_SLICE_FILES)}


def main() -> int:
    failures: list[str] = []
    syntax = {p: _syntax(p, failures) for p in (SELF_PATH, S107_RUN_PATH, S107_GUARD_PATH, S106_GUARD_PATH, COMMAND_STATUS_PATH)}
    ...
    payload = {"ok": not failures, "phase": "phase4a_autotrade_milestone_fv_pre_armed_dry_run_authorization_request_execution_authorization_request_execution_authorization_request_execution_authorization_request_execution_authorization_request_preflight_status_close_guard", "status": "closed" if not failures else "open", "contract": {"s107_authorization_request_preflight_status_present": (REPO_ROOT / S107_RUN_PATH).exists(), "s107_focused_guard_closed": s107_guard.get("ok") is True, "s106_fu_guard_closed": s106_guard.get("ok") is True, "syntax_checked_without_pyc": all(item.get("ok") is True for item in syntax.values()), "broker_free_non_recording_non_executing_non_authorizing": not run_boundary.get("forbidden"), "expected_git_boundary_only": not git_boundary.get("unexpected")}, "syntax": syntax, "run_boundary": run_boundary, "guards": {"s107_fv_guard": s107_guard, "s106_fu_guard": s106_guard}, "git_boundary": git_boundary, "failures": failures}
