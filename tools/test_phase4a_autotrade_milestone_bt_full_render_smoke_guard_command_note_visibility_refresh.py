# path: ./tools/test_phase4a_autotrade_milestone_bt_full_render_smoke_guard_command_note_visibility_refresh.py
# desc: Full AutoTrade tab render smoke after command note observer visibility. No command/mode/observer append; no runner/broker.
...
def main() -> int:
    ...
    try:
        ...
        write_jsonl(command_path, [
            command_row("cmd_full_render_halt_bp", command_type="REQUEST_HALT_NEW", accepted=True, current_mode="ARMED_DRY_RUN", target="halt_new", blocked_by=[]),
            command_row("cmd_full_render_live_bp", command_type="REQUEST_MODE_CHANGE", accepted=True, current_mode="ARMED_DRY_RUN", target="LIVE_MIN_SIZE", blocked_by=[]),
        ])
    ...
    failures.extend(f"protected lower-layer dirty during milestone BT: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_bt_full_render_smoke_guard_command_note_visibility_refresh",
