# path: ./tools/test_phase4a_autotrade_milestone_bw_command_status_readiness_note_fail_soft_guard.py
# desc: Guard command status readiness note parsing is fail-soft and preserves latest valid mode-change readiness note. Read-only; no append/runner/broker.
    ...
    return json.dumps(
        {
            ...
            "warnings": ["guard_valid"],
            "health_state": "warn",
            "observer_run_fresh": True,
            "observer_latest_run_id": "obs_bw_valid_blocked_off",
...
def command_row(*, command_id: str, command_type: str, accepted: bool, requested_at: str, current_mode: str, target: str | None, blocked_by: list[str], note: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "target": target,
            "confirmation": True,
            "reason_codes": ["guard", "readiness_note_fail_soft"],
            "note": note,
...
def write_command_rows(path: Path) -> None:
    rows = [
        command_row(
            command_id="cmd_bw_valid_live_request",
            ...
            note=valid_readiness_note(),
        ),
        command_row(
            command_id="cmd_bw_malformed_live_request",
            command_type="REQUEST_MODE_CHANGE",
            accepted=False,
        ...
        command_row(
            command_id="cmd_bw_halt_after_malformed",
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_command_status_readiness_note_fail_soft_hot"
    ...
    note_payload_source = function_source(STATUS_FILE, "_readiness_note_payload")
    ...
    checks = {
        "malformed_note_does_not_skip_command_rows": summary_data.get("total_rows") == 3 and summary_data.get("skipped_rows") == 0,
        "latest_command_is_newer_non_mode_without_note": summary_data.get("latest_command_id") == "cmd_bw_halt_after_malformed" and summary_data.get("latest_command_type") == "REQUEST_HALT_NEW",
        "latest_row_readiness_fields_empty": summary_data.get("latest_readiness_observer_run_id") is None and tuple(summary_data.get("latest_readiness_observer_blocked_by") or ()) == (),
        "malformed_newer_mode_change_note_ignored_fail_soft": summary_data.get("latest_mode_change_readiness_command_id") == "cmd_bw_valid_live_request" and summary_data.get("latest_mode_change_readiness_observer_run_id") == "obs_bw_valid_blocked_off" and tuple(summary_data.get("latest_mode_change_readiness_observer_blocked_by") or ()) == tuple(EXPECTED_BLOCKED_BY),
        "readiness_note_payload_fail_soft_source": bool(note_payload_source) and "except Exception" in note_payload_source and "return {}" in note_payload_source,
        "ui_command_requests_displays_latest_mode_change_readiness_fields": "latest_mode_change_readiness_observer_blocked_by" in ui_source,
        ...
        "summary_did_not_append_command": before_count == 3 and after_summary_count == before_count,
        "render_did_not_append_command": before_count == 3 and after_render_count == before_count,
    ...
    failures.extend(f"protected lower-layer dirty during milestone BW: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_bw_command_status_readiness_note_fail_soft_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "malformed_note_does_not_skip_command_rows": checks["malformed_note_does_not_skip_command_rows"],
            "latest_command_is_newer_non_mode_without_note": checks["latest_command_is_newer_non_mode_without_note"],
            "latest_row_readiness_fields_empty": checks["latest_row_readiness_fields_empty"],
            "malformed_newer_mode_change_note_ignored_fail_soft": checks["malformed_newer_mode_change_note_ignored_fail_soft"],
            "readiness_note_payload_fail_soft_source": checks["readiness_note_payload_fail_soft_source"],
