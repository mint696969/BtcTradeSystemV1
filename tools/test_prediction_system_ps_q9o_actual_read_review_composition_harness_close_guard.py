# path: ./tools/test_prediction_system_ps_q9o_actual_read_review_composition_harness_close_guard.py
# desc: Close guard for PS-Q9O non-UI actual-read review composition harness.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_actual_read_review_composition_harness.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9o_actual_read_review_composition_harness_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9o_actual_read_review_composition_harness_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9o_actual_read_review_composition_harness_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9O actual-read review composition harness close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9O actual-read review composition harness close guard")
    return 0


def test_prediction_system_ps_q9o_actual_read_review_composition_harness_close_guard() -> None:
    assert main() == 0
