# path: ./tools/test_phase4a_autotrade_milestone_bl_full_render_smoke_guard_observer_blocked_refresh.py
# desc: Full AutoTrade tab render smoke after observer latest blocked visibility. No command/mode/observer append; no runner/broker.
...
def command_row(command_id: str, *, command_type: str, accepted: bool, current_mode: str, target: str | None, blocked_by: list[str]) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "requested_by": "guard",
            "requested_at": "2026-06-13T08:20:00Z",
            "current_mode": current_mode,
            "target": target,
            "confirmation": True,
            "reason_codes": ["guard", "full_render_observer_blocked_refresh"],
            "note": "{}",
...
def mode_state_rejection_row() -> dict[str, Any]:
    return {
        "current_mode": "OFF",
        "previous_mode": "OFF",
        "changed_at": "2026-06-13T08:19:00Z",
        "source_command_id": "cmd_render_mode_state_rejected_bl",
        "requested_by": "guard",
        "accepted": False,
        "mode_changed": False,
...
def observer_blocked_row(finished_at: str) -> dict[str, Any]:
    return {
        "run_id": "obs_full_render_observer_blocked_refresh",
...
def shadow_row() -> dict[str, Any]:
    return {
        "decision_id": "dec_full_render_observer_blocked_refresh",
        "mode": "SHADOW",
        "snapshot_id": "snap_full_render_observer_blocked_refresh",
        "forecast_id": "fcst_full_render_observer_blocked_refresh",
        "parameter_set_id": "params_fx_balanced_v0_1",
        "logic_version": "autotrade_logic_v0_1",
        "forecast_5m": {"forecast_id": "fcst_full_render_observer_blocked_refresh", "forecast_direction": "down", "confidence": "medium"},
        "candidate": {"action": "NO_NEW_ENTRY"},
        "risk_gate": {"allowed": False, "executable": False},
        "final_action": "WAIT",
        "reason_codes": ["full_render_observer_blocked_refresh_guard"],
        "blocked_by": [],
        "would_order": None,
    }
...
def outcome_row() -> dict[str, Any]:
    return {
        ...
        "parameter_set_id": "params_fx_balanced_v0_1",
        "logic_version": "autotrade_logic_v0_1",
        "source_snapshot_id": "snap_full_render_observer_blocked_refresh",
        "target_ts": "2026-06-13T02:00:00Z",
        "actual_snapshot_id": "actual_full_render_observer_blocked_refresh",
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_autotrade_ui_full_smoke_observer_blocked_refresh_hot"
    ...
    try:
        ...
        write_jsonl(command_path, [
            command_row("cmd_full_render_halt_bl", command_type="REQUEST_HALT_NEW", accepted=True, current_mode="OFF", target="halt_new", blocked_by=[]),
            command_row("cmd_full_render_live_bl", command_type="REQUEST_MODE_CHANGE", accepted=True, current_mode="OFF", target="PAPER_OR_REPLAY", blocked_by=[]),
        ])
    ...
    failures.extend(f"protected lower-layer dirty during milestone BL: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_bl_full_render_smoke_guard_observer_blocked_refresh",
