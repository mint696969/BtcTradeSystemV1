# path: ./tools/test_phase4a_prediction_system_ps_q17d_tier0_source_quality_gate_coverage_contract_guard.py
# desc: Focused guard for PS-Q17D tier0 source-quality gate coverage contract.
...
def main_guard() -> int:
    ...
    if report.get("gate_state_enum") != ["pass", "warn", "fail", "unknown"]:
        failures.append("gate state enum mismatch")
    if report.get("reason_severity_enum") != ["blocking", "warning", "context_only"]:
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q17d_tier0_source_quality_gate_coverage_contract_guard",
        "phase": "phase3_tier0_source_quality_gate_contract_before_confidence_increase",
        "contract": {
            "contract_only": True,
...
def test_ps_q17d_tier0_source_quality_gate_coverage_contract_guard() -> None:
    assert main_guard() == 0
