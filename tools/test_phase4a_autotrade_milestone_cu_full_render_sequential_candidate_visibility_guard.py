# path: ./tools/test_phase4a_autotrade_milestone_cu_full_render_sequential_candidate_visibility_guard.py
# desc: Full AutoTrade render with sequential candidate state. Preview should show latest unapplied candidate after mode_state marks latest applied; render is read-only.
    ...
    return json.dumps(
        {
            ...
            "warnings": ["full_render_sequential_candidate_visibility_snapshot"],
            "health_state": "warn",
...
def command_row(command_id: str, requested_at: str, blocker: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "confirmation": True,
            "reason_codes": ["guard", "full_render_sequential_candidate_visibility", command_id],
...
def mode_state_rejection_row(command_id: str, blocker: str) -> dict[str, Any]:
    return {
        ...
        "accepted": False,
        "mode_changed": False,
        "reason_codes": ["guard", "full_render_sequential_candidate_visibility", command_id, "readiness_recheck"],
        "blocked_by": [
...
def shadow_row() -> dict[str, Any]:
    return {
        "decision_id": "dec_cu_full_render_sequential_candidate_visibility",
        "mode": "SHADOW",
        "snapshot_id": "snap_cu_full_render_sequential_candidate_visibility",
        "forecast_id": "fcst_cu_full_render_sequential_candidate_visibility",
        "parameter_set_id": "params_fx_balanced_v0_1",
        "logic_version": "autotrade_logic_v0_1",
        "forecast_5m": {"forecast_id": "fcst_cu_full_render_sequential_candidate_visibility", "forecast_direction": "down", "confidence": "medium"},
        "candidate": {"action": "NO_NEW_ENTRY"},
        "risk_gate": {"allowed": False, "executable": False},
        "final_action": "WAIT",
        "reason_codes": ["cu_full_render_sequential_candidate_visibility_guard"],
        "blocked_by": [],
        "would_order": None,
    }
...
def outcome_row() -> dict[str, Any]:
    return {
        ...
        "parameter_set_id": "params_fx_balanced_v0_1",
        "logic_version": "autotrade_logic_v0_1",
        "source_snapshot_id": "snap_cu_full_render_sequential_candidate_visibility",
        "target_ts": "2026-06-13T02:00:00Z",
        "actual_snapshot_id": "actual_cu_full_render_sequential_candidate_visibility",
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_full_render_sequential_candidate_visibility_hot"
    ...
    try:
        ...
        write_jsonl(
            command_path,
            [
                command_row("cmd_cu_oldest_live_request", now_z(-30), "cu_oldest_candidate_blocker"),
                command_row("cmd_cu_middle_live_request", now_z(-20), "cu_middle_candidate_blocker"),
                command_row("cmd_cu_latest_live_request", now_z(-10), "cu_latest_candidate_blocker"),
            ],
        )
        write_jsonl(mode_state_path, [mode_state_rejection_row("cmd_cu_latest_live_request", "cu_latest_candidate_blocker")])
    ...
    checks = {
        "preview_before_render_selects_middle_unapplied_candidate": preview_before_render.get("command_id") == "cmd_cu_middle_live_request" and preview_before_render.get("candidate_command_count") == 2 and tuple(preview_before_render.get("already_applied_command_ids") or ()) == ("cmd_cu_latest_live_request",) and preview_before_render.get("would_reject_by_readiness") is True and preview_before_render.get("candidate_command_type") == "REQUEST_MODE_CHANGE" and preview_before_render.get("candidate_readiness_note_present") is True,
        "preview_after_render_still_selects_middle_unapplied_candidate": preview_after_render.get("command_id") == "cmd_cu_middle_live_request" and preview_after_render.get("candidate_command_count") == 2 and tuple(preview_after_render.get("already_applied_command_ids") or ()) == ("cmd_cu_latest_live_request",) and preview_after_render.get("would_reject_by_readiness") is True and preview_after_render.get("candidate_command_type") == "REQUEST_MODE_CHANGE" and preview_after_render.get("candidate_readiness_note_present") is True,
        "mode_state_summary_surfaces_latest_rejection_before_and_after_render": mode_summary_before_render.get("latest_source_command_id") == "cmd_cu_latest_live_request" and mode_summary_after_render.get("latest_source_command_id") == "cmd_cu_latest_live_request" and mode_summary_after_render.get("latest_ledger_event") == "autotrade.mode_state_readiness_recheck_rejected" and "readiness_recheck_not_ready" in tuple(mode_summary_after_render.get("latest_blocked_by") or ()) and mode_summary_after_render.get("latest_would_send_to_broker") is False,
        "mode_state_ui_displays_rejection_fields": all(field in mode_state_source for field in MODE_STATE_REJECTION_FIELDS),
        "apply_preview_ui_keeps_candidate_context_fields": all(field in apply_preview_source for field in PREVIEW_CANDIDATE_FIELDS),
        ...
        "full_render_did_not_append_mode_state": after_render_mode_count == before_mode_count == 1,
    ...
    failures.extend(f"protected lower-layer dirty during milestone CU: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_cu_full_render_sequential_candidate_visibility_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "preview_before_render_selects_middle_unapplied_candidate": checks["preview_before_render_selects_middle_unapplied_candidate"],
            "preview_after_render_still_selects_middle_unapplied_candidate": checks["preview_after_render_still_selects_middle_unapplied_candidate"],
