# path: ./tools/test_prediction_system_ps_q2c_context_evidence_profiles_close_guard.py
# desc: Close guard for PS-Q2C context-specific evidence profile interpretation.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/prediction/system.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q2c_context_evidence_profiles_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q2c_context_evidence_profiles_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q2c_context_evidence_profiles_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q2b_source_artifact_runtime_coverage_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q2b_source_artifact_runtime_coverage_close_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q2C context evidence profiles close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q2C context evidence profiles close guard")
    return 0


def test_prediction_system_ps_q2c_context_evidence_profiles_close_guard() -> None:
    assert main() == 0
