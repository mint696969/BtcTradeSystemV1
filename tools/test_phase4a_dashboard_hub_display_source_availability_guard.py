# path: ./tools/test_phase4a_dashboard_hub_display_source_availability_guard.py
# desc: Guard dashboard hub display source availability/status read model. No app.py/Streamlit/layout/runtime/broker wiring.
...
OVERVIEW_CLOSE_GUARD = "tools/test_phase4a_dashboard_hub_display_source_overview_close_guard.py"
FILES = [
    "btcts_next/src/btcts/apps/operator_ui/hub/display_source_availability.py",
    "btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_availability.py",
]
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_availability"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    required = [
        ...
        "dashboard_hub_source_availability=",
        "source_statuses",
        "missing_references",
        "future_widget_source_keys",
        "referenced_by_page",
        "catalog_only",
        "referenced",
        "summary_widget",
    ...
    for fragment in missing:
        failures.append(f"dashboard hub display source availability missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub display source availability contains forbidden token: {token}")
...
def _check_app_py_untouched_boundary(failures: list[str]) -> dict[str, object]:
    ...
    forbidden = [
        "display_source_availability",
        "dashboard_hub_display_source_availability",
        "DASHBOARD_HUB_SOURCE_AVAILABILITY_CONTRACT",
    ]
    hits = [token for token in forbidden if token in app_text]
    for token in hits:
        failures.append(f"app.py must not be wired to dashboard hub display source availability in this slice: {token}")
    return {"hits": hits}


def _run_overview_close_guard(failures: list[str]) -> dict[str, object]:
    if os.environ.get("BTCTS_DASHBOARD_HUB_SKIP_NESTED_CLOSE_GUARD") == "1":
        return {
            "ok": True,
            "skipped": True,
            "reason": "verified_by_separate_primary_total_guard",
            "path": OVERVIEW_CLOSE_GUARD,
        }
    return _run_json_guard(OVERVIEW_CLOSE_GUARD, failures)

def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_availability_guard.py", failures),
        "compile_files": {rel: _compile(rel, failures) for rel in FILES},
        "overview_close_guard": _run_overview_close_guard(failures),
        "plain_test": _run_plain_ok("btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_availability.py", failures),
        "source": _check_source(failures),
        "app_py_untouched_boundary": _check_app_py_untouched_boundary(failures),
    }
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_availability_guard",
        "failures": failures,
        "checks": checks,
    }
