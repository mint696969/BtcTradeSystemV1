# path: ./tools/test_phase4a_autotrade_milestone_ez_pre_armed_dry_run_authorization_request_execution_authorization_request_execution_gate_dry_run_status_close_guard.py
# desc: Close guard for S85 Pre-Armed Dry Run authorization request execution gate dry-run/status. Broker-free; non-recording; non-executing; non-authorizing.
...
SELF_PATH = "tools/test_phase4a_autotrade_milestone_ez_pre_armed_dry_run_authorization_request_execution_authorization_request_execution_gate_dry_run_status_close_guard.py"
S85_RUN_PATH = "tools/run_sr_fx_pre_armed_dry_run_authorization_request_execution_authorization_request_execution_gate_dry_run_status.py"
...
EXPECTED_SLICE_FILES = {S85_RUN_PATH, S85_GUARD_PATH, SELF_PATH}
...
def _run_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"S85 run missing required fragment: {frag}" for frag in missing)
    failures.extend(f"S85 run contains forbidden token: {tok}" for tok in forbidden)
    failures.extend(f"S85 run missing safety literal: {frag}" for frag in false_missing)
...
def _git_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"unexpected dirty file during S85 close: {line}" for line in unexpected)
    return {"lines": lines, "unexpected": unexpected, "expected_slice_files": sorted(EXPECTED_SLICE_FILES)}


def main() -> int:
    failures: list[str] = []
    syntax = {p: _syntax(p, failures) for p in (SELF_PATH, S85_RUN_PATH, S85_GUARD_PATH, S84_GUARD_PATH, COMMAND_STATUS_PATH)}
    ...
    payload = {"ok": not failures, "phase": "phase4a_autotrade_milestone_ez_pre_armed_dry_run_authorization_request_execution_authorization_request_execution_gate_dry_run_status_close_guard", "status": "closed" if not failures else "open", "contract": {"s85_authorization_request_execution_gate_status_present": (REPO_ROOT / S85_RUN_PATH).exists(), "s85_focused_guard_closed": s85_guard.get("ok") is True, "s84_ey_guard_closed": s84_guard.get("ok") is True, "syntax_checked_without_pyc": all(item.get("ok") is True for item in syntax.values()), "broker_free_non_recording_non_executing_non_authorizing": not run_boundary.get("forbidden"), "expected_git_boundary_only": not git_boundary.get("unexpected")}, "syntax": syntax, "run_boundary": run_boundary, "guards": {"s85_ez_guard": s85_guard, "s84_ey_guard": s84_guard}, "git_boundary": git_boundary, "failures": failures}
