# path: ./tools/test_phase4a_autotrade_milestone_hi_decision_policy_gate_registry_visibility_close_guard.py
# desc: Close guard for S146 decision policy gate dashboard registry visibility packet. Syntax-checks touched files and reruns focused HI guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hi_decision_policy_gate_registry_visibility_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hi_decision_policy_gate_registry_visibility_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hi_decision_policy_gate_registry_visibility_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HI decision policy gate registry visibility guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hi_decision_policy_gate_registry_visibility_close_guard",
...
def test_decision_policy_gate_registry_visibility_close_guard_closes() -> None:
    assert main() == 0
