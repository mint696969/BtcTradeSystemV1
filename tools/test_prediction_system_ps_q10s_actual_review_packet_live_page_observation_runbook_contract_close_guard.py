# path: ./tools/test_prediction_system_ps_q10s_actual_review_packet_live_page_observation_runbook_contract_close_guard.py
# desc: Close guard for PS-Q10S live page observation runbook contract.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_actual_review_packet_live_page_observation_runbook_contract.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q10s_actual_review_packet_live_page_observation_runbook_contract_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q10s_actual_review_packet_live_page_observation_runbook_contract_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q10s_actual_review_packet_live_page_observation_runbook_contract_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q10S actual review-packet live page observation runbook contract close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q10S actual review-packet live page observation runbook contract close guard")
    return 0


def test_prediction_system_ps_q10s_actual_review_packet_live_page_observation_runbook_contract_close_guard() -> None:
    assert main() == 0
