# path: ./tools/test_phase4a_autotrade_milestone_ia_decision_policy_gate_visibility_completion_status_index_registry_visibility_chain_completion_close_guard.py
# desc: Close guard for S164 decision policy gate visibility completion status index registry visibility chain completion packet. Syntax-checks touched files and reruns focused IA guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ia_decision_policy_gate_visibility_completion_status_index_registry_visibility_chain_completion_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ia_decision_policy_gate_visibility_completion_status_index_registry_visibility_chain_completion_close_guard.py",
)
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused IA decision policy gate visibility completion status index registry visibility chain completion guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_ia_decision_policy_gate_visibility_completion_status_index_registry_visibility_chain_completion_close_guard",
...
def test_decision_policy_gate_visibility_completion_status_index_registry_visibility_chain_completion_close_guard_closes() -> None:
    assert main() == 0
