# path: ./tools/test_phase4a_autotrade_milestone_gy_prediction_foundation_index_close_guard.py
# desc: Close guard for S136 prediction foundation integration index. Syntax-checks guards and reruns focused GY guard.
...
TOUCHED = (
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gy_prediction_foundation_index_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gy_prediction_foundation_index_close_guard.py",
)
DOC = REPO_ROOT / "docs/architecture/AUTOTRADE_PREDICTION_FOUNDATION_INTEGRATION_INDEX_2026-06-18.md"
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gy_prediction_foundation_index_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GY prediction foundation index guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gy_prediction_foundation_index_close_guard",
