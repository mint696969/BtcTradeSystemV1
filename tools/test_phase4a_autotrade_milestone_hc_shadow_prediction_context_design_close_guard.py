# path: ./tools/test_phase4a_autotrade_milestone_hc_shadow_prediction_context_design_close_guard.py
# desc: Close guard for S140 Shadow prediction context design packet. Syntax-checks guards and reruns focused HC guard.
...
TOUCHED = (
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hc_shadow_prediction_context_design_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hc_shadow_prediction_context_design_close_guard.py",
)
DOC = REPO_ROOT / "docs/architecture/AUTOTRADE_SHADOW_DECISION_OPTIONAL_PREDICTION_CONTEXT_DESIGN_2026-06-18.md"
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hc_shadow_prediction_context_design_guard.py"
...
def main() -> int:
    ...
    if not DOC.exists():
        failures.append(f"missing doc target: {DOC.relative_to(REPO_ROOT)}")
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HC shadow prediction context design guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hc_shadow_prediction_context_design_close_guard",
...
def test_shadow_prediction_context_design_close_guard_closes() -> None:
    assert main() == 0
