# path: ./tools/test_phase4a_prediction_system_ps_q11c_scenario_switch_trace_guard.py
# desc: Close guard for PS-Q11C Scenario Core scenario switch trace; advisory/read-only only.
...
EXPECTED_DIRTY = {
    ...
    "tools/test_phase4a_prediction_system_ps_q11c_scenario_switch_trace_guard.py",
}
    ...
    return build_market_summary(
        MarketSummaryBuildInput(
            market_state_row={
                ...
                "interpretation_reason": "ps_q11c_close_guard",
...
def main() -> int:
    ...
    required_markers = (
        ...
        "scenario_switch_action_family",
        "scenario_switch_urgency",
    ...
    try:
        ...
        raised_trace = dict(
            raised_output.scenario_trace.get("scenario_switch_trace") or {}
        )
        raised_checks = {
            "raised_trace_type": raised_trace.get("trace_type") == "prediction_scenario_switch_trace",
            "raised_switch_hint_preserved": raised_output.scenario_switch_hint == "tighten_primary_watch",
            "raised_action_family": raised_trace.get("switch_action_family") == "watch",
            "raised_urgency": raised_trace.get("switch_urgency") == "medium",
            "raised_focus_alignment": raised_trace.get("trace_focus_switch_alignment") == "no_focus",
            "raised_diagnostics_action": raised_output.diagnostics.get("scenario_switch_action_family") == "watch",
        ...
        transition_trace = dict(
            transition_output.scenario_trace.get("scenario_switch_trace") or {}
        )
        transition_checks = {
            "transition_switch_hint_preserved": transition_output.scenario_switch_hint == "execute_transition_switch",
            "transition_action_family": transition_trace.get("switch_action_family") == "execute",
            "transition_urgency": transition_trace.get("switch_urgency") == "high",
        }
        ...
        reversal_trace = dict(
            reversal_output.scenario_trace.get("scenario_switch_trace") or {}
        )
        reversal_checks = {
            "reversal_switch_hint_preserved": reversal_output.scenario_switch_hint == "prepare_reversal_switch",
            "reversal_action_family": reversal_trace.get("switch_action_family") == "prepare",
            "reversal_urgency": reversal_trace.get("switch_urgency") == "high",
            "reversal_focus_alignment": reversal_trace.get("trace_focus_switch_alignment") == "different_switch_focus",
        }
        failures.extend(f"check failed: {name}" for name, ok in reversal_checks.items() if not ok)
    except Exception as exc:
        failures.append(f"scenario switch trace smoke failed: {exc}")
    ...
    payload = {
        "ok": not failures,
        "guard": "ps_q11c_scenario_switch_trace",
...
def test_ps_q11c_scenario_switch_trace_guard() -> None:
    assert main() == 0
