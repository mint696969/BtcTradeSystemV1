# path: ./tools/test_prediction_system_ps_q9y_latest_payload_export_runner_close_guard.py
# desc: Close guard for PS-Q9Y non-UI latest payload export runner.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_payload_export_runner.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9y_latest_payload_export_runner_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9y_latest_payload_export_runner_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9y_latest_payload_export_runner_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9Y latest payload export runner close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9Y latest payload export runner close guard")
    return 0


def test_prediction_system_ps_q9y_latest_payload_export_runner_close_guard() -> None:
    assert main() == 0
