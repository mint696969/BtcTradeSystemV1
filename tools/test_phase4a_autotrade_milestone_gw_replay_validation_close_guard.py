# path: ./tools/test_phase4a_autotrade_milestone_gw_replay_validation_close_guard.py
# desc: Close guard for S134 replay validation contract. Syntax-checks touched files and reruns focused GW guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/replay_validation.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gw_replay_validation_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gw_replay_validation_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gw_replay_validation_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GW replay validation guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gw_replay_validation_close_guard",
