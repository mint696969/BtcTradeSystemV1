# path: ./tools/test_prediction_system_ps_q4c_l4_latest_adapter_contract_close_guard.py
# desc: Close guard for PS-Q4C L4/latest adapter contract boundary.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_l4_latest_adapter.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q4c_l4_latest_adapter_contract_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q4c_l4_latest_adapter_contract_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q4c_l4_latest_adapter_contract_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q4b_warroom_widget_groups_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q4b_warroom_widget_groups_close_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q4C L4/latest adapter contract close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q4C L4/latest adapter contract close guard")
    return 0


def test_prediction_system_ps_q4c_l4_latest_adapter_contract_close_guard() -> None:
    assert main() == 0
