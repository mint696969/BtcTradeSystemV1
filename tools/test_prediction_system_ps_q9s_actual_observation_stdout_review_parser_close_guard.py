# path: ./tools/test_prediction_system_ps_q9s_actual_observation_stdout_review_parser_close_guard.py
# desc: Close guard for PS-Q9S actual observation stdout review parser.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_actual_observation_stdout_review_parser.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9s_actual_observation_stdout_review_parser_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9s_actual_observation_stdout_review_parser_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9s_actual_observation_stdout_review_parser_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9S actual observation stdout review parser close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9S actual observation stdout review parser close guard")
    return 0


def test_prediction_system_ps_q9s_actual_observation_stdout_review_parser_close_guard() -> None:
    assert main() == 0
