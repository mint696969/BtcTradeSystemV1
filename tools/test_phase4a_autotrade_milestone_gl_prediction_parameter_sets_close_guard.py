# path: ./tools/test_phase4a_autotrade_milestone_gl_prediction_parameter_sets_close_guard.py
# desc: Close guard for S123 prediction parameter-set skeletons. Syntax-checks touched files and reruns focused GL guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/parameter_sets.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gl_prediction_parameter_sets_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gl_prediction_parameter_sets_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gl_prediction_parameter_sets_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GL prediction parameter sets guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gl_prediction_parameter_sets_close_guard",
