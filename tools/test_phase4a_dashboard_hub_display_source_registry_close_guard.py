...
# desc: Close guard for dashboard hub display source registry bundle. No app.py/Streamlit/layout/runtime/broker wiring.
...
FOCUSED_GUARDS = [
    "tools/test_phase4a_operator_display_source_catalog_guard.py",
    "tools/test_phase4a_dashboard_hub_display_source_registry_guard.py",
]
FILES = [
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_registry_close"
...
def _check_source_shape(failures: list[str]) -> dict[str, object]:
    ...
    for fragment in missing:
        failures.append(f"dashboard hub registry bundle missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub registry bundle contains forbidden token: {token}")
...
def _check_app_py_untouched_boundary(failures: list[str]) -> dict[str, object]:
    ...
    for token in hits:
        failures.append(f"app.py must not be wired to dashboard hub registry in this slice: {token}")
    return {"hits": hits}
...
def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_registry_close_guard.py", failures),
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_registry_close_guard",
