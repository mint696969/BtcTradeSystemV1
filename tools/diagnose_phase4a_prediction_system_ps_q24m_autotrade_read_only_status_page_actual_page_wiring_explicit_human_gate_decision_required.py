# path: ./tools/diagnose_phase4a_prediction_system_ps_q24m_autotrade_read_only_status_page_actual_page_wiring_explicit_human_gate_decision_required.py
# desc: No-write diagnostic for explicit human gate decision required before AutoTrade prediction status page wiring.
...
from btcts.apps.operator_ui.components.autotrade_prediction_status_page_explicit_human_gate_decision_required import (  # noqa: E402
...
from tools.diagnose_phase4a_prediction_system_ps_q24l_autotrade_read_only_status_page_actual_page_wiring_explicit_human_gate_required import (  # noqa: E402
    run_autotrade_read_only_status_page_actual_page_wiring_explicit_human_gate_required,
)

DIAGNOSTIC_VERSION = "prediction_warroom.ps_q24m_autotrade_read_only_status_page_actual_page_wiring_explicit_human_gate_decision_required.v1"
AUTOTRADE_PAGE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/views/autotrade_page.py"
DECISION_MODULE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/autotrade_prediction_status_page_explicit_human_gate_decision_required.py"
DECISION_MODULE_REF = "autotrade_prediction_status_page_explicit_human_gate_decision_required"
DECISION_BUILDER_REF = "build_autotrade_prediction_status_page_explicit_human_gate_decision_required_packet"
...
def _sample_status() -> AutoTradePredictionPreviewStatus:
    return AutoTradePredictionPreviewStatus(
        status_id="ps_q24m_explicit_human_gate_decision_required_sample_status",
        generated_at="2026-06-29T00:00:00Z",
        status_state="ok",
        preview_id="ps_q24m_preview",
        readiness_id="ps_q24m_readiness",
...
def run_autotrade_read_only_status_page_actual_page_wiring_explicit_human_gate_decision_required() -> dict[str, Any]:
    branch = _git(["rev-parse", "--abbrev-ref", "HEAD"])
    head = _git(["rev-parse", "--short", "HEAD"])
    status_short = _git(["status", "--short", "--untracked-files=all"])
    q24l = run_autotrade_read_only_status_page_actual_page_wiring_explicit_human_gate_required()
    page_text = AUTOTRADE_PAGE.read_text(encoding="utf-8") if AUTOTRADE_PAGE.exists() else ""
    module_text = DECISION_MODULE.read_text(encoding="utf-8") if DECISION_MODULE.exists() else ""
    packet = build_autotrade_prediction_status_page_explicit_human_gate_decision_required_packet(_sample_status(), autotrade_page_text=page_text)
    ...
    page_wired = DECISION_MODULE_REF in page_text or DECISION_BUILDER_REF in page_text
    blockers: list[str] = []
    if q24l.get("ready") is not True:
        blockers.append("q24l_explicit_human_gate_required_ready_required")
    for item in list(q24l.get("blockers") or []):
        blockers.append(f"q24l:{item}")
    if AUTOTRADE_PREDICTION_STATUS_PAGE_EXPLICIT_HUMAN_GATE_DECISION_REQUIRED_CONTRACT.get("decision_requirement_type") != "autotrade_prediction_status_page_actual_page_wiring_explicit_human_gate_decision_required_packet":
        blockers.append("explicit_human_gate_decision_required_contract_type_required")
    if packet.get("explicit_human_gate_decision_required_ready") is not True:
        blockers.append("explicit_human_gate_decision_required_ready_required")
    for marker in ("explicit_human_gate_decision_required_packet_only=true", "explicit_human_gate_required=true", "explicit_human_gate_decision_required=true", "human_gate_grant_record_present=false", "human_gate_decision=not_granted", "human_gate_granted=false", "page_change_authorized=false", "actual_page_wiring_allowed=false", "must_stop_before_autotrade_page_edit=true", "blocked_until_human_gate=true", "not_page_wiring=true", "not_runtime_wiring=true", "not_ui_rendering=true", "no_command_buttons=true", "no_forms=true", "no_" + "session" + "_state=true", "no_callbacks=true"):
        if marker not in tuple(packet.get("snapshot_lines") or ()): 
            blockers.append(f"explicit_human_gate_decision_required_snapshot_marker_required:{marker}")
    if packet.get("explicit_human_gate_required") is not True or packet.get("explicit_human_gate_decision_required") is not True:
        blockers.append("explicit_human_gate_decision_required_stop_marker_required")
    ...
    if packet.get("explicit_human_gate_decision_required_packet_only") is not True or packet.get("not_page_wiring") is not True:
        blockers.append("explicit_human_gate_decision_required_not_page_wiring_required")
    if packet.get("not_runtime_wiring") is not True or packet.get("not_ui_rendering") is not True:
        blockers.append("explicit_human_gate_decision_required_no_runtime_ui_required")
    if packet.get("no_command_buttons") is not True or packet.get("no_forms") is not True or packet.get("no_" + "session" + "_state") is not True or packet.get("no_callbacks") is not True:
        blockers.append("explicit_human_gate_decision_required_no_controls_required")
    if packet.get("autotrade_page_edit_performed_by_this_slice") is not False or packet.get("page_runtime_mount_performed_by_this_slice") is not False:
        blockers.append("explicit_human_gate_decision_required_page_edit_or_mount_must_be_false")
    if packet.get("actual_ui_rendering_performed_by_this_slice") is not False or packet.get("command_surface_changed_by_this_slice") is not False:
        blockers.append("explicit_human_gate_decision_required_render_or_command_change_must_be_false")
    if not _all_false(packet, execution_flags):
        blockers.append("explicit_human_gate_decision_required_execution_flags_false_required")
    if page_wired:
        blockers.append("autotrade_page_explicit_human_gate_decision_required_wiring_unexpected")
    for token in forbidden_module_tokens:
        if token in module_text:
            blockers.append(f"explicit_human_gate_decision_required_module_forbidden_token:{token}")
    ...
    return {
        ...
        "ready": ready,
        "state": "ps_q24m_autotrade_status_page_explicit_human_gate_decision_required_ready" if ready else "ps_q24m_autotrade_status_page_explicit_human_gate_decision_required_blocked",
        "blockers": blockers,
        "q24l_explicit_human_gate_required": {
            "ready": q24l.get("ready"),
            "diagnostic_version": q24l.get("diagnostic_version"),
            "explicit_human_gate_required": q24l.get("explicit_human_gate_required"),
            "safety": q24l.get("safety"),
        },
        "explicit_human_gate_decision_required": {
            "decision_requirement_type": packet.get("decision_requirement_type"),
            "explicit_human_gate_decision_required_ready": packet.get("explicit_human_gate_decision_required_ready"),
            "explicit_human_gate_decision_required_packet_available": packet.get("explicit_human_gate_decision_required_packet_available"),
            "explicit_human_gate_required_ready": packet.get("explicit_human_gate_required_ready"),
            ...
            "autotrade_page_contains_decision_required_module": packet.get("autotrade_page_contains_decision_required_module"),
            ...
            "explicit_human_gate_decision_required_packet_only": packet.get("explicit_human_gate_decision_required_packet_only"),
