# path: ./tools/test_prediction_system_ps_q7a_loader_authorization_request_close_guard.py
# desc: Close guard for PS-Q7A latest payload loader authorization request contract.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_payload_loader_authorization_request.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q7a_loader_authorization_request_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q7a_loader_authorization_request_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q7a_loader_authorization_request_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q7A loader authorization request close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q7A loader authorization request close guard")
    return 0


def test_prediction_system_ps_q7a_loader_authorization_request_close_guard() -> None:
    assert main() == 0
