# path: ./tools/test_phase4a_autotrade_milestone_cg_mode_change_preview_candidate_command_metadata_guard.py
# desc: Guard Mode Change Apply Preview surfaces candidate command metadata separately from candidate readiness note and recheck. Read-only; no append/runner/broker.
...
EXPECTED_CANDIDATE_BLOCKED_BY = ["candidate_metadata_guard_blocked_but_accepted"]
    ...
    return json.dumps(
        {
            ...
            "warnings": ["candidate_note_context_is_separate_from_command_metadata"],
            "health_state": "warn",
            "observer_latest_run_id": "obs_cg_candidate_note",
...
def command_row(command_id: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "confirmation": True,
            "reason_codes": ["guard", "candidate_command_metadata"],
            "note": readiness_note(),
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_mode_change_preview_candidate_command_metadata_hot"
    ...
    try:
        ...
        write_jsonl(command_path, [command_row("cmd_cg_candidate_live_request")])
    ...
    checks = {
        "preview_dataclass_has_candidate_command_metadata_fields": all(field in applier_text for field in CANDIDATE_METADATA_FIELDS),
        "preview_surfaces_candidate_command_metadata": preview_data.get("command_id") == "cmd_cg_candidate_live_request" and preview_data.get("candidate_command_type") == "REQUEST_MODE_CHANGE" and preview_data.get("candidate_requested_by") == "operator_ui" and isinstance(preview_data.get("candidate_requested_at"), str) and preview_data.get("candidate_current_mode") == "ARMED_DRY_RUN" and preview_data.get("candidate_target_mode") == "LIVE_MIN_SIZE" and preview_data.get("candidate_accepted") is True and tuple(preview_data.get("candidate_blocked_by") or ()) == tuple(EXPECTED_CANDIDATE_BLOCKED_BY),
        "preview_keeps_candidate_note_context_separate": preview_data.get("candidate_readiness_note_present") is True and tuple(preview_data.get("candidate_readiness_blocked_by") or ()) == tuple(EXPECTED_NOTE_BLOCKED_BY) and preview_data.get("candidate_readiness_observer_latest_run_id") == "obs_cg_candidate_note",
        "preview_recheck_still_runs_separately": preview_data.get("readiness") is not None and preview_data.get("would_reject_by_readiness") is True and preview_data.get("readiness_ready") is False and "readiness_recheck_not_ready" in tuple(preview_data.get("blocked_by") or ()),
        "ui_preview_displays_candidate_command_metadata_fields": all(field in ui_source for field in CANDIDATE_METADATA_FIELDS),
        "ui_preview_still_displays_candidate_note_fields": all(field in ui_source for field in CANDIDATE_NOTE_FIELDS),
    ...
    failures.extend(f"protected lower-layer dirty during milestone CG: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_cg_mode_change_preview_candidate_command_metadata_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "preview_dataclass_has_candidate_command_metadata_fields": checks["preview_dataclass_has_candidate_command_metadata_fields"],
            "preview_surfaces_candidate_command_metadata": checks["preview_surfaces_candidate_command_metadata"],
            "preview_keeps_candidate_note_context_separate": checks["preview_keeps_candidate_note_context_separate"],
            "preview_recheck_still_runs_separately": checks["preview_recheck_still_runs_separately"],
            "ui_preview_displays_candidate_command_metadata_fields": checks["ui_preview_displays_candidate_command_metadata_fields"],
            "ui_preview_still_displays_candidate_note_fields": checks["ui_preview_still_displays_candidate_note_fields"],
