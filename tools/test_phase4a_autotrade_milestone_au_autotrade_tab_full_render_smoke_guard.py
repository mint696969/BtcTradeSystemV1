# path: ./tools/test_phase4a_autotrade_milestone_au_autotrade_tab_full_render_smoke_guard.py
# desc: Full AutoTrade tab render smoke after mode-state/apply-preview surfaces. Fake Streamlit; no command/mode append.
...
def command_row(command_id: str, *, command_type: str, accepted: bool, current_mode: str, target: str | None, blocked_by: list[str]) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "requested_by": "guard",
            "requested_at": "2026-06-13T05:30:00Z",
            "current_mode": current_mode,
            "target": target,
            "confirmation": True,
            "reason_codes": ["guard", "full_render_smoke"],
            "note": "{}",
...
def mode_state_row() -> dict[str, Any]:
    return {
        "current_mode": "SHADOW",
        "previous_mode": "OFF",
        "changed_at": "2026-06-13T05:29:00Z",
        "source_command_id": "cmd_render_mode_state_shadow",
        "requested_by": "guard",
        "accepted": True,
        "mode_changed": True,
        "reason_codes": ["guard"],
...
def shadow_row() -> dict[str, Any]:
    return {
        "decision_id": "dec_full_render_smoke",
        "mode": "SHADOW",
        "snapshot_id": "snap_full_render_smoke",
        "forecast_id": "fcst_full_render_smoke",
        "parameter_set_id": "params_fx_balanced_v0_1",
        "logic_version": "autotrade_logic_v0_1",
        "forecast_5m": {"forecast_id": "fcst_full_render_smoke", "forecast_direction": "down", "confidence": "medium"},
        "candidate": {"action": "NO_NEW_ENTRY"},
        "risk_gate": {"allowed": False, "executable": False},
        "final_action": "WAIT",
        "reason_codes": ["full_render_smoke_guard"],
        "blocked_by": [],
        "would_order": None,
    }
...
def outcome_row() -> dict[str, Any]:
    return {
        ...
        "parameter_set_id": "params_fx_balanced_v0_1",
        "logic_version": "autotrade_logic_v0_1",
        "source_snapshot_id": "snap_full_render_smoke",
        "target_ts": "2026-06-13T02:00:00Z",
        "actual_snapshot_id": "actual_full_render_smoke",
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_autotrade_ui_full_smoke_hot"
    ...
    try:
        ...
        write_jsonl(
            command_path,
            [
                command_row("cmd_full_render_halt", command_type="REQUEST_HALT_NEW", accepted=True, current_mode="SHADOW", target="halt_new", blocked_by=[]),
                command_row("cmd_full_render_live", command_type="REQUEST_MODE_CHANGE", accepted=True, current_mode="SHADOW", target="PAPER_OR_REPLAY", blocked_by=[]),
            ],
        )
        write_jsonl(mode_state_path, [mode_state_row()])
    ...
    failures.extend(f"protected lower-layer dirty during milestone AU: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_au_autotrade_tab_full_render_smoke_guard",
