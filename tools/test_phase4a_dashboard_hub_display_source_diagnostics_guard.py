# path: ./tools/test_phase4a_dashboard_hub_display_source_diagnostics_guard.py
# desc: Guard dashboard hub display source diagnostics read model. No app.py/Streamlit/layout/runtime/broker wiring.
...
BUNDLE_CLOSE_GUARD = "tools/test_phase4a_dashboard_hub_display_source_bundle_close_guard.py"
FILES = [
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_diagnostics"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    for fragment in missing:
        failures.append(f"dashboard hub display source diagnostics missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub display source diagnostics contains forbidden token: {token}")
...
def main() -> int:
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_diagnostics_guard",
        "failures": failures,
        "checks": checks,
    }
