# path: ./tools/test_prediction_system_ps_q7d_authorization_registry_summary_panel_close_guard.py
# desc: Close guard for PS-Q7D authorization registry/catalog summary panel.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_loader_authorization_registry_summary_panel.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q7d_authorization_registry_summary_panel_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q7d_authorization_registry_summary_panel_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q7d_authorization_registry_summary_panel_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q7D authorization registry summary panel close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q7D authorization registry summary panel close guard")
    return 0


def test_prediction_system_ps_q7d_authorization_registry_summary_panel_close_guard() -> None:
    assert main() == 0
