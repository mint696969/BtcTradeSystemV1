# path: ./tools/test_prediction_system_ps_q4d_warroom_sample_packets_close_guard.py
# desc: Close guard for PS-Q4D synthetic WarRoom sample packets.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_sample_packets.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q4D WarRoom sample packets close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q4D WarRoom sample packets close guard")
    return 0


def test_prediction_system_ps_q4d_warroom_sample_packets_close_guard() -> None:
    assert main() == 0
