# path: ./tools/test_phase4a_autotrade_milestone_ck_rechecked_preview_cli_candidate_context_guard.py
# desc: Guard rechecked preview CLI JSON output surfaces candidate context and remains read-only. No command/mode/observer append, runner, or broker.
...
EXPECTED_CANDIDATE_BLOCKED_BY = ["ck_candidate_command_blocker"]
EXPECTED_NOTE_BLOCKED_BY = ["observer_run_latest_blocked_for_live_target", "mode_off"]
EXPECTED_OBSERVER_BLOCKED_BY = ["mode_off"]
EXPECTED_WARNINGS = ["candidate_note_visible_in_preview_cli_output"]
CANDIDATE_FIELDS = (
    ...
    return json.dumps(
        {
            ...
            "health_state": "warn",
            "observer_latest_run_id": "obs_ck_candidate_note",
...
def command_row(command_id: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "confirmation": True,
            "reason_codes": ["guard", "preview_cli_candidate_context"],
            "note": readiness_note(),
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_rechecked_preview_cli_candidate_context_hot"
    ...
    try:
        ...
        write_jsonl(command_path, [command_row("cmd_ck_candidate_live_request")])
        env = os.environ.copy()
        ...
        first_data = parse_stdout("first", first.stdout, failures)
        ...
        second_data = parse_stdout("second", second.stdout, failures)
    ...
    checks = {
        "cli_first_preview_rejection_exit_code_expected": first_returncode == 2 and first_data.get("would_apply") is False and first_data.get("would_reject_by_readiness") is True,
        ...
        "cli_first_stdout_has_candidate_observer_note_details": first_data.get("candidate_readiness_observer_latest_run_id") == "obs_ck_candidate_note" and tuple(first_data.get("candidate_readiness_observer_latest_blocked_by") or ()) == tuple(EXPECTED_OBSERVER_BLOCKED_BY) and first_data.get("candidate_readiness_observer_latest_would_send_to_broker") is False and first_data.get("candidate_readiness_observer_latest_bounded") is True,
        ...
        "cli_preview_does_not_append_anything": before_command_count == 1 and after_first_command_count == before_command_count and after_second_command_count == before_command_count and before_mode_count == 0 and after_first_mode_count == before_mode_count and after_second_mode_count == before_mode_count and observer_count_after_first == observer_count_before and observer_count_after_second == observer_count_before,
        "cli_second_preview_is_same_candidate_not_applied": second_returncode == 2 and second_data.get("command_id") == "cmd_ck_candidate_live_request" and second_data.get("candidate_command_type") == "REQUEST_MODE_CHANGE" and second_data.get("already_applied_command_ids") == [] and second_data.get("candidate_command_count") == 1,
    ...
    failures.extend(f"protected lower-layer dirty during milestone CK: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_ck_rechecked_preview_cli_candidate_context_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "cli_first_preview_rejection_exit_code_expected": checks["cli_first_preview_rejection_exit_code_expected"],
            ...
            "cli_preview_does_not_append_anything": checks["cli_preview_does_not_append_anything"],
            "cli_second_preview_is_same_candidate_not_applied": checks["cli_second_preview_is_same_candidate_not_applied"],
