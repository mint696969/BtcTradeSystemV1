# path: ./tools/test_phase4a_prediction_system_ps_q17f_close_guard.py
# desc: Close guard for PS-Q17F calibration reference contract.
...
from check_phase4a_prediction_system_ps_q17f_calibration_reference_contract import CHECKER_VERSION, CONTRACT_ORDER, REFERENCE_HIT_RATE_BANDS, REQUIRED_CALIBRATION_FIELDS, SIGNAL_BANDS, build_report, main
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q17f_calibration_reference_contract_guard.py"
EXPECTED_DIRTY = {
...
EXPECTED_PARAMETER_TUNING_BLOCKERS = (
    "signal_strength_calibration_reference_contract",
    "reference_hit_rate_calibration_reference_contract",
    "confidence_band_release_contract",
    "parameter_candidate_calibration_dependency_contract",
...
def main_guard() -> int:
    ...
    for marker in (
        ...
        "parameter_candidate_calibration_dependency_contract",
        "warroom_calibration_explanation_contract",
        "calibration_refs_required_before_confidence_claim",
        "signal_reliability_claim_allowed",
        "parameter_tuning_allowed",
    ...
    if "test_ps_q17f_builds_calibration_contract_from_q17b_gap" not in unit_text:
        failures.append("unit test must cover Q17B calibration gap to contract")
    if "test_ps_q17f_blocks_invalid_or_nonblocking_q17b_report" not in unit_text:
    ...
    if report.get("stage") != "calibration_reference_contract_before_confidence_and_parameter_tuning":
    ...
    if report.get("source_gap_id") != "calibration_refs_and_signal_strength_validation":
    ...
    if report.get("calibration_refs_required_before_confidence_claim") is not True:
        failures.append("calibration refs required flag must be true")
    ...
    for contract_id in EXPECTED_PARAMETER_TUNING_BLOCKERS:
    ...
    for marker in (
        ...
        "parameter_tuning_allowed remains false until signal-strength and reference-hit-rate refs",
        "no_parameter_tuning",
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q17f_close_guard",
        "phase": "phase3_calibration_reference_contract_closed_before_confidence_and_parameter_tuning",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q17f_closed": not failures,
            "contract_only": True,
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q17G calibration reference adapter or prediction-delta history contract",
        },
...
def test_ps_q17f_close_guard() -> None:
    assert main_guard() == 0
