# path: ./tools/test_phase4a_autotrade_milestone_hf_decision_ledger_policy_gate_close_guard.py
# desc: Close guard for S143 decision ledger integration policy gate. Syntax-checks touched files and reruns focused HF guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/autotrade/decision_ledger_policy_gate.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hf_decision_ledger_policy_gate_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hf_decision_ledger_policy_gate_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hf_decision_ledger_policy_gate_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HF decision ledger policy gate guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hf_decision_ledger_policy_gate_close_guard",
...
def test_decision_ledger_policy_gate_close_guard_closes() -> None:
    assert main() == 0
