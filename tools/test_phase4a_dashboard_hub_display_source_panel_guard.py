# path: ./tools/test_phase4a_dashboard_hub_display_source_panel_guard.py
# desc: Guard minimal component-only Streamlit panel for dashboard hub display source presenter. No app.py/non-health page routing/runtime/broker wiring.
...
PRESENTER_CLOSE_GUARD = "tools/test_phase4a_dashboard_hub_display_source_presenter_close_guard.py"
FILES = [
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_panel"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    required = [
        ...
        "hot_cold_table_rendered",
        "hot_cold_table_caption",
        "Hot/Cold metadata",
        "hot_cold_status:",
    ...
    for fragment in missing:
        failures.append(f"dashboard hub source panel missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub source panel contains forbidden token: {token}")
...
def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_panel_guard.py", failures),
        "compile_files": {rel: _compile(rel, failures) for rel in FILES},
        "presenter_close_guard": _run_json_guard(PRESENTER_CLOSE_GUARD, failures),
        "plain_test": _run_plain_ok("btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_panel.py", failures),
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_panel_guard",
        "failures": failures,
        "checks": checks,
    }
