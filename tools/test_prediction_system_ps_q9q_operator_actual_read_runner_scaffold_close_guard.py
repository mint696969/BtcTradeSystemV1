# path: ./tools/test_prediction_system_ps_q9q_operator_actual_read_runner_scaffold_close_guard.py
# desc: Close guard for PS-Q9Q non-UI operator actual-read runner scaffold.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_actual_read_operator_runner_scaffold.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9q_operator_actual_read_runner_scaffold_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9q_operator_actual_read_runner_scaffold_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9q_operator_actual_read_runner_scaffold_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9Q operator actual-read runner scaffold close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9Q operator actual-read runner scaffold close guard")
    return 0


def test_prediction_system_ps_q9q_operator_actual_read_runner_scaffold_close_guard() -> None:
    assert main() == 0
