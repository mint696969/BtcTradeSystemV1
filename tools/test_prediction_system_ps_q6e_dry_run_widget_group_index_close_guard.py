# path: ./tools/test_prediction_system_ps_q6e_dry_run_widget_group_index_close_guard.py
# desc: Close guard for PS-Q6E latest payload dry-run widget group index.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_payload_dry_run_widget_groups.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q6e_dry_run_widget_group_index_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q6e_dry_run_widget_group_index_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q6e_dry_run_widget_group_index_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q6E dry-run widget group index close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q6E dry-run widget group index close guard")
    return 0


def test_prediction_system_ps_q6e_dry_run_widget_group_index_close_guard() -> None:
    assert main() == 0
