# path: ./tools/test_phase4a_autotrade_milestone_gz_preview_consumption_design_close_guard.py
# desc: Close guard for S137 preview consumption design packet. Syntax-checks guards and reruns focused GZ guard.
...
TOUCHED = (
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gz_preview_consumption_design_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gz_preview_consumption_design_close_guard.py",
)
DOC = REPO_ROOT / "docs/architecture/AUTOTRADE_PREDICTION_PREVIEW_CONSUMPTION_DESIGN_2026-06-18.md"
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gz_preview_consumption_design_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GZ preview consumption design guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gz_preview_consumption_design_close_guard",
