# path: ./tools/test_phase4a_hot_cold_dashboard_display_source_status_guard.py
# desc: Guard Hot/Cold dashboard display source status model. Metadata-only; no payload loader/rendering/file reader/copy/delete.
...
SELF_PATH = "tools/test_phase4a_hot_cold_dashboard_display_source_status_guard.py"
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, Any]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "hot_cold_dashboard_source_status"
...
def _check_source_shape(failures: list[str]) -> dict[str, Any]:
    ...
    for fragment in missing:
        failures.append(f"hot/cold dashboard source status missing fragment: {fragment}")
    if forbidden_hits:
        failures.append("hot/cold dashboard source status contains forbidden reader/rendering/executor token")
...
"""
    ...
    try:
        ...
        failures.append(f"hot/cold dashboard source status probe did not emit JSON: {exc}")
    ...
    if not ok:
        failures.append("hot/cold dashboard source status probe must remain metadata-only and unopened")
...
def main() -> int:
    ...
    checks = {
        ...
        "catalog_close_guard": {"verified_before_status_guard": True, "path": CATALOG_CLOSE_GUARD_PATH},
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_hot_cold_dashboard_display_source_status_guard",
        "failures": failures,
        "checks": checks,
    }
