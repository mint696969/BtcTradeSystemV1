# path: ./tools/test_phase4a_autotrade_milestone_gn_feature_registry_source_quality_close_guard.py
# desc: Close guard for S125 feature registry/source quality contracts. Syntax-checks touched files and reruns focused GN guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/source_quality.py",
    REPO_ROOT / "btcts_next/src/btcts/prediction/feature_registry.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gn_feature_registry_source_quality_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gn_feature_registry_source_quality_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gn_feature_registry_source_quality_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GN feature registry/source quality guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gn_feature_registry_source_quality_close_guard",
