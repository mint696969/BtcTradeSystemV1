# path: ./tools/test_phase4a_autotrade_milestone_bv_full_render_smoke_guard_latest_mode_change_note_refresh.py
# desc: Full AutoTrade tab render smoke after latest mode-change note visibility. No command/mode/observer append; no runner/broker.
    ...
    return {
        ...
        "command": {
            ...
            "note": json.dumps({
                ...
                "observer_latest_run_id": "obs_full_render_command_note_visibility_refresh",
            ...
            }, ensure_ascii=False, sort_keys=True) if command_type == "REQUEST_MODE_CHANGE" else "{}",
...
def main() -> int:
    ...
    try:
        ...
        write_jsonl(command_path, [
            command_row("cmd_full_render_live_bv", command_type="REQUEST_MODE_CHANGE", accepted=False, current_mode="ARMED_DRY_RUN", target="LIVE_MIN_SIZE", blocked_by=["readiness_preflight_not_ready", "observer_run_latest_blocked_for_live_target"]),
            command_row("cmd_full_render_halt_bv", command_type="REQUEST_HALT_NEW", accepted=True, current_mode="ARMED_DRY_RUN", target="halt_new", blocked_by=[]),
        ])
    ...
    failures.extend(f"protected lower-layer dirty during milestone BV: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_bv_full_render_smoke_guard_latest_mode_change_note_refresh",
