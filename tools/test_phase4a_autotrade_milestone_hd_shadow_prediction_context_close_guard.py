# path: ./tools/test_phase4a_autotrade_milestone_hd_shadow_prediction_context_close_guard.py
# desc: Close guard for S141 Shadow prediction context contract. Syntax-checks touched files and reruns focused HD guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/autotrade/shadow_prediction_context.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hd_shadow_prediction_context_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hd_shadow_prediction_context_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hd_shadow_prediction_context_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HD shadow prediction context guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hd_shadow_prediction_context_close_guard",
...
def test_shadow_prediction_context_close_guard_closes() -> None:
    assert main() == 0
