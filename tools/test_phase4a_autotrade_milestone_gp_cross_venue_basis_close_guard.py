# path: ./tools/test_phase4a_autotrade_milestone_gp_cross_venue_basis_close_guard.py
# desc: Close guard for S127 cross-venue/Spot-FX basis contracts. Syntax-checks touched files and reruns focused GP guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/cross_venue.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gp_cross_venue_basis_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gp_cross_venue_basis_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gp_cross_venue_basis_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GP cross-venue basis guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gp_cross_venue_basis_close_guard",
