# path: ./tools/test_phase4a_autotrade_milestone_eq_pre_armed_dry_run_authorization_request_record_dry_run_plan_status_close_guard.py
# desc: Close guard for S76 Pre-Armed Dry Run authorization request record dry-run plan/status. Broker-free; non-recording; non-executing; non-authorizing.
...
SELF_PATH = "tools/test_phase4a_autotrade_milestone_eq_pre_armed_dry_run_authorization_request_record_dry_run_plan_status_close_guard.py"
S76_RUN_PATH = "tools/run_sr_fx_pre_armed_dry_run_approval_record_append_execution_authorization_request_record_execution_authorization_request_record_execution_authorization_request_record_execution_authorization_request_record_dry_run_plan_status.py"
S76_GUARD_PATH = "tools/test_phase4a_autotrade_milestone_eq_pre_armed_dry_run_authorization_request_record_dry_run_plan_status_guard.py"
...
EXPECTED_SLICE_FILES = {S76_RUN_PATH, S76_GUARD_PATH, SELF_PATH}
...
def _run_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"S76 run missing required fragment: {frag}" for frag in missing)
    failures.extend(f"S76 run contains forbidden token: {tok}" for tok in forbidden)
    failures.extend(f"S76 run missing safety literal: {frag}" for frag in false_missing)
...
def _git_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"unexpected dirty file during S76 close: {line}" for line in unexpected)
    return {"lines": lines, "unexpected": unexpected, "expected_slice_files": sorted(EXPECTED_SLICE_FILES)}


def main() -> int:
    failures: list[str] = []
    syntax = {p: _syntax(p, failures) for p in (SELF_PATH, S76_RUN_PATH, S76_GUARD_PATH, S75_GUARD_PATH, COMMAND_STATUS_PATH)}
    run_boundary = _run_boundary(failures)
    s76_guard = _run_json_guard(S76_GUARD_PATH, failures)
    s75_guard = _run_json_guard(S75_GUARD_PATH, failures)
    git_boundary = _git_boundary(failures)
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_eq_pre_armed_dry_run_authorization_request_record_dry_run_plan_status_close_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "s76_dry_run_plan_status_present": (REPO_ROOT / S76_RUN_PATH).exists(),
            "s76_focused_guard_closed": s76_guard.get("ok") is True,
            "s75_ep_guard_closed": s75_guard.get("ok") is True,
        ...
        "guards": {"s76_eq_guard": s76_guard, "s75_ep_guard": s75_guard},
