# path: ./tools/test_phase4a_autotrade_milestone_cc_full_render_smoke_guard_command_metadata_refresh.py
# desc: Full AutoTrade tab render smoke after readiness command metadata visibility. No command/mode/observer append; no runner/broker.
...
from btcts.autotrade.execution import read_command_ledger, read_mode_state_records, summarize_command_ledger  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        write_jsonl(command_path, [
            command_row("cmd_full_render_live_by", command_type="REQUEST_MODE_CHANGE", accepted=False, current_mode="ARMED_DRY_RUN", target="LIVE_MIN_SIZE", blocked_by=["readiness_preflight_not_ready", "observer_run_latest_blocked_for_live_target"]),
    ...
    checks = {
        ...
        "full_render_summary_mode_change_readiness_metadata_checked": command_summary_data.get("latest_mode_change_readiness_requested_by") == "guard" and isinstance(command_summary_data.get("latest_mode_change_readiness_requested_at"), str) and command_summary_data.get("latest_mode_change_readiness_accepted") is False and "readiness_preflight_not_ready" in tuple(command_summary_data.get("latest_mode_change_readiness_command_blocked_by") or ()),
    ...
    failures.extend(f"protected lower-layer dirty during milestone CC: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_cc_full_render_smoke_guard_command_metadata_refresh",
