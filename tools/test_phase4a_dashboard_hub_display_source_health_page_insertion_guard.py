# path: ./tools/test_phase4a_dashboard_hub_display_source_health_page_insertion_guard.py
# desc: Guard Health page insertion of dashboard hub source panel. No app.py/sidebar/runtime/broker mutation.
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_health_page_insertion"
...
def _check_source_shape(failures: list[str]) -> dict[str, object]:
    ...
    for fragment in missing:
        failures.append(f"health page dashboard hub source panel insertion missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"health page dashboard hub source panel insertion contains forbidden token: {token}")
...
def _check_existing_boundary(failures: list[str]) -> dict[str, object]:
    ...
    app_hits = [token for token in forbidden_app if token in app_text]
    for token in app_hits:
        failures.append(f"app.py must not be wired by health page insertion slice: {token}")
    ...
    for rel, text in page_texts.items():
        for token in forbidden_app:
            if token in text:
                ...
                failures.append(f"non-selected view must not be wired by health page insertion slice: {rel}: {token}")
...
def main() -> int:
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_health_page_insertion_guard",
        "failures": failures,
        "checks": checks,
    }
