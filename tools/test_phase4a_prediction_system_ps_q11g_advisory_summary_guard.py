# path: ./tools/test_phase4a_prediction_system_ps_q11g_advisory_summary_guard.py
# desc: Close guard for PS-Q11G Scenario Core advisory packet summary / next-action labels; read-only/non-executing only.
...
EXPECTED_DIRTY = {
    ...
    "tools/test_phase4a_prediction_system_ps_q11g_advisory_summary_guard.py",
}
    ...
    return build_market_summary(
        MarketSummaryBuildInput(
            market_state_row={
                ...
                "interpretation_reason": "ps_q11g_close_guard",
...
def _check_summary(output, label: str, expected_status: str, expected_route: str, expected_label: str, expected_family: str, failures: list[str]) -> None:
    scenario_trace = dict(output.scenario_trace or {})
    summary = dict(scenario_trace.get("advisory_packet_summary") or {})
    checks = {
        f"{label}_trace_type": summary.get("trace_type") == "prediction_advisory_packet_summary",
        f"{label}_status": summary.get("summary_status") == expected_status,
        f"{label}_route": summary.get("review_route") == expected_route,
        f"{label}_next_action_label": summary.get("next_action_label") == expected_label,
        f"{label}_next_action_family": summary.get("next_action_family") == expected_family,
        f"{label}_manual_review_only": summary.get("manual_review_only") is True,
        f"{label}_advisory_read_only": summary.get("advisory_read_only") is True,
        f"{label}_non_executing": summary.get("non_executing") is True,
        f"{label}_would_send_to_broker_false": summary.get("would_send_to_broker") is False,
        f"{label}_would_append_ledger_false": summary.get("would_append_ledger") is False,
        f"{label}_would_write_runtime_artifact_false": summary.get("would_write_runtime_artifact") is False,
        f"{label}_execution_surface_none": summary.get("execution_surface") == "none",
        f"{label}_runtime_write_surface_none": summary.get("runtime_write_surface") == "none",
        f"{label}_contract_no_auto_trade": "no_auto_trade" in tuple(summary.get("operator_action_contract") or ()),
        f"{label}_contract_no_runtime_write": "no_runtime_write" in tuple(summary.get("operator_action_contract") or ()),
        f"{label}_diag_status": output.diagnostics.get("advisory_packet_summary_status") == expected_status,
        f"{label}_diag_next_label": output.diagnostics.get("advisory_packet_summary_next_action_label") == expected_label,
        f"{label}_diag_next_family": output.diagnostics.get("advisory_packet_summary_next_action_family") == expected_family,
        f"{label}_diag_manual_only": output.diagnostics.get("advisory_packet_summary_manual_only") is True,
        f"{label}_diag_read_only": output.diagnostics.get("advisory_packet_summary_read_only") is True,
        f"{label}_diag_non_executing": output.diagnostics.get("advisory_packet_summary_non_executing") is True,
    }
    summary_lines = tuple(summary.get("summary_lines") or ())
    if not any(str(row).startswith("next_action:") for row in summary_lines):
        checks[f"{label}_summary_lines_next_action"] = False
...
def main() -> int:
    ...
    required_markers = (
        "def _build_advisory_packet_summary(",
        "prediction_advisory_packet_summary",
        "advisory_packet_summary",
        "advisory_packet_summary_status",
    ...
    try:
        ...
        failures.append(f"advisory summary smoke failed: {exc}")
...
def test_ps_q11g_advisory_summary_guard() -> None:
    assert main() == 0
