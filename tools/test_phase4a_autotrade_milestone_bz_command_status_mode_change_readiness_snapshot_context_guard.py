# path: ./tools/test_phase4a_autotrade_milestone_bz_command_status_mode_change_readiness_snapshot_context_guard.py
# desc: Guard Command Requests status surfaces latest mode-change readiness snapshot context from command.note. Read-only; no append/runner/broker.
...
EXPECTED_WARNINGS = ["core_runtime_root_looks_cold_or_archive", "shadow_decision_ledger_empty"]
    ...
    return json.dumps(
        {
            ...
            "observer_latest_run_id": "obs_bz_blocked_off",
            "observer_latest_blocked_by": EXPECTED_BLOCKED_BY[1:],
...
def command_row(*, command_id: str, command_type: str, accepted: bool, requested_at: str, current_mode: str, target: str | None, blocked_by: list[str], note: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "target": target,
            "confirmation": True,
            "reason_codes": ["guard", "readiness_snapshot_context"],
            "note": note,
...
def write_command_rows(path: Path) -> None:
    rows = [
        command_row(
            command_id="cmd_bz_live_request",
            command_type="REQUEST_MODE_CHANGE",
            accepted=False,
            requested_at=now_z(-10),
            current_mode="ARMED_DRY_RUN",
            target="LIVE_MIN_SIZE",
            blocked_by=["readiness_preflight_not_ready", *EXPECTED_BLOCKED_BY],
            note=readiness_note(),
        ),
        command_row(
            command_id="cmd_bz_halt_after_live_request",
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_command_status_mode_change_readiness_snapshot_context_hot"
    ...
    checks = {
        "summary_dataclass_has_readiness_context_fields": all(field in status_text for field in CONTEXT_FIELDS),
        "summary_preserves_latest_mode_change_readiness_context": summary_data.get("latest_mode_change_readiness_command_id") == "cmd_bz_live_request" and summary_data.get("latest_mode_change_readiness_ready") is False and summary_data.get("latest_mode_change_readiness_current_mode") == "ARMED_DRY_RUN" and summary_data.get("latest_mode_change_readiness_target_mode") == "LIVE_MIN_SIZE" and tuple(summary_data.get("latest_mode_change_readiness_blocked_by") or ()) == tuple(EXPECTED_BLOCKED_BY) and tuple(summary_data.get("latest_mode_change_readiness_warnings") or ()) == tuple(EXPECTED_WARNINGS) and summary_data.get("latest_mode_change_readiness_health_state") == "warn",
        "ui_command_requests_displays_latest_mode_change_readiness_context": all(field in ui_source for field in CONTEXT_FIELDS),
        "summary_latest_command_is_newer_non_mode": summary_data.get("latest_command_id") == "cmd_bz_halt_after_live_request" and summary_data.get("latest_command_type") == "REQUEST_HALT_NEW",
    ...
    failures.extend(f"protected lower-layer dirty during milestone BZ: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_bz_command_status_mode_change_readiness_snapshot_context_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "summary_dataclass_has_readiness_context_fields": checks["summary_dataclass_has_readiness_context_fields"],
            "summary_preserves_latest_mode_change_readiness_context": checks["summary_preserves_latest_mode_change_readiness_context"],
            "ui_command_requests_displays_latest_mode_change_readiness_context": checks["ui_command_requests_displays_latest_mode_change_readiness_context"],
