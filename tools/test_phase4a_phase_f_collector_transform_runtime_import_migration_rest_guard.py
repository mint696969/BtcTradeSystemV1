...
FORBIDDEN_WS_FACADE_IMPORT = "from .transforms.facade import"
WS_RUNTIME_FILES = [
...
def _check_ws_lanes_not_migrated(failures: list[str]) -> dict[str, Any]:
    hits: list[dict[str, str]] = []
    for rel_path in WS_RUNTIME_FILES:
        if FORBIDDEN_WS_FACADE_IMPORT in _read(rel_path):
            hits.append({"path": rel_path, "fragment": FORBIDDEN_WS_FACADE_IMPORT})
    for hit in hits:
        failures.append(f"WS lane runtime import migration is not allowed in REST slice: {hit['path']}")
...
def main() -> int:
    ...
    checks = {
        ...
        "ws_lanes_not_migrated": _check_ws_lanes_not_migrated(failures),
