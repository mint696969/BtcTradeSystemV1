# path: ./tools/test_prediction_system_ps_q9v_warroom_top_default_expanded_ui_layout_close_guard.py
# desc: Close guard for PS-Q9V WarRoom top/default-expanded UI-only layout patch.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_prediction_system_ps_q9v_warroom_top_default_expanded_ui_layout_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9v_warroom_top_default_expanded_ui_layout_guard.py",
    # PS-Q9V intentionally supersedes the old folded-position WarRoom page invariants
    # in PS-Q9G/PS-Q9K/PS-Q9L/PS-Q9M. Keep their non-UI ancestors guarded, but do
    # not re-run page-position guards that must fail after the approved top/default-expanded move.
    REPO_ROOT / "tools/test_prediction_system_ps_q9t_actual_observation_ui_handoff_readiness_contract_guard.py",
...
ALLOWED_DIRTY_PATHS = {
    ...
    "tools/test_prediction_system_ps_q9v_warroom_top_default_expanded_ui_layout_guard.py",
    "tools/test_prediction_system_ps_q9v_warroom_top_default_expanded_ui_layout_close_guard.py",
}
...
def main() -> int:
    ...
    for line in dirty_lines:
        dirty = _dirty_path(line)
        if any(dirty.startswith(prefix) for prefix in PROTECTED_DIRTY_PREFIXES):
            failures.append(f"protected dirty path: {line}")
        if dirty not in ALLOWED_DIRTY_PATHS:
            failures.append(f"unexpected dirty path for PS-Q9V: {line}")
    if failures:
        print("[FAIL] Prediction System PS-Q9V WarRoom top/default-expanded UI layout close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9V WarRoom top/default-expanded UI layout close guard")
    return 0


def test_prediction_system_ps_q9v_warroom_top_default_expanded_ui_layout_close_guard() -> None:
    assert main() == 0
