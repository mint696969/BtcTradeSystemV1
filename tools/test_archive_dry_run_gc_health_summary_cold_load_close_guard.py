# path: ./tools/test_archive_dry_run_gc_health_summary_cold_load_close_guard.py
# desc: Close guard for archive dry-run GC cold-load hotfix.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/collector_vnext/archive/health_summary.py",
    REPO_ROOT / "tools/test_archive_dry_run_gc_health_summary_cold_load_guard.py",
    REPO_ROOT / "tools/test_archive_dry_run_gc_health_summary_cold_load_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_archive_dry_run_gc_health_summary_cold_load_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused archive dry-run GC cold-load guard did not close")
    payload = {
        "ok": not failures,
        "phase": "archive_dry_run_gc_health_summary_cold_load_close_guard",
...
def test_archive_dry_run_gc_health_summary_cold_load_close_guard_closes() -> None:
    assert main() == 0
