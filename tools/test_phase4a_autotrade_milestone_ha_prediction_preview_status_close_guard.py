# path: ./tools/test_phase4a_autotrade_milestone_ha_prediction_preview_status_close_guard.py
# desc: Close guard for S138 prediction preview status contract. Syntax-checks touched files and reruns focused HA guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/autotrade/prediction_preview_status.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ha_prediction_preview_status_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ha_prediction_preview_status_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_ha_prediction_preview_status_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HA prediction preview status guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_ha_prediction_preview_status_close_guard",
...
def test_prediction_preview_status_close_guard_closes() -> None:
    assert main() == 0
