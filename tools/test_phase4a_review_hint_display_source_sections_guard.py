# path: ./tools/test_phase4a_review_hint_display_source_sections_guard.py
# desc: Guard review_hint_display sections in AI operator display sources. No Streamlit/UI layout/runtime/broker wiring.
...
COMMON_PRESENTER_GUARD = "tools/test_phase4a_review_hint_common_presenter_guard.py"
FILES = [
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "review_hint_display_source_sections"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    for fragment in missing:
        failures.append(f"display source missing fragment: {fragment}")

    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"display source contains forbidden token: {token}")
...
def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_review_hint_display_source_sections_guard.py", failures),
        "compile_files": {rel: _compile(rel, failures) for rel in FILES},
        "common_presenter_guard": _run_json_guard(COMMON_PRESENTER_GUARD, failures),
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_review_hint_display_source_sections_guard",
        "failures": failures,
        "checks": checks,
    }
