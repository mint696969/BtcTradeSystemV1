# path: ./tools/test_phase4a_dashboard_hub_display_source_bundle_guard.py
# desc: Guard dashboard hub display source bundle facade. No app.py/Streamlit/layout/runtime/broker wiring.
...
COVERAGE_CLOSE_GUARD = "tools/test_phase4a_dashboard_hub_display_source_coverage_close_guard.py"
FILES = [
    "btcts_next/src/btcts/apps/operator_ui/hub/display_source_bundle.py",
    "btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_bundle.py",
]
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_bundle"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    required = [
        ...
        "ready_for_dashboard_hub_display_source_overview",
        "registry",
        "overview",
        "availability",
        "matrix",
    ...
    for fragment in missing:
        failures.append(f"dashboard hub display source bundle missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub display source bundle contains forbidden token: {token}")
...
def _check_app_py_untouched_boundary(failures: list[str]) -> dict[str, object]:
    ...
    forbidden = [
        "display_source_bundle",
        "dashboard_hub_display_source_bundle",
        "DASHBOARD_HUB_SOURCE_BUNDLE_CONTRACT",
    ]
    hits = [token for token in forbidden if token in app_text]
    for token in hits:
        failures.append(f"app.py must not be wired to dashboard hub source bundle in this slice: {token}")
    return {"hits": hits}


def _run_coverage_close_guard(failures: list[str]) -> dict[str, object]:
    if os.environ.get("BTCTS_DASHBOARD_HUB_SKIP_NESTED_CLOSE_GUARD") == "1":
        return {
            "ok": True,
            "skipped": True,
            "reason": "verified_by_separate_primary_total_guard",
            "path": COVERAGE_CLOSE_GUARD,
        }
    return _run_json_guard(COVERAGE_CLOSE_GUARD, failures)

def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_bundle_guard.py", failures),
        "compile_files": {rel: _compile(rel, failures) for rel in FILES},
        "coverage_close_guard": _run_coverage_close_guard(failures),
        "plain_test": _run_plain_ok("btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_bundle.py", failures),
        "source": _check_source(failures),
        "app_py_untouched_boundary": _check_app_py_untouched_boundary(failures),
    }
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_bundle_guard",
        "failures": failures,
        "checks": checks,
    }
