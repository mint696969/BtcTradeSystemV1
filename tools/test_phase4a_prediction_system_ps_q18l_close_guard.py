# path: ./tools/test_phase4a_prediction_system_ps_q18l_close_guard.py
# desc: Close guard for PS-Q18L latest_prediction_summary_widget mapped payload value rows mount.
...
from check_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount import CHECKER_VERSION, MAPPED_PAYLOAD_VALUE_ROWS_MOUNT_VERSION, WARROOM_PAGE_TARGET, build_report, main

REPO_ROOT = Path(__file__).resolve().parents[1]
WARROOM_PAGE = REPO_ROOT / WARROOM_PAGE_TARGET
COMPONENT = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_prediction_summary_widget_mapped_payload_value_rows_panel.py"
TOOL = REPO_ROOT / "tools/check_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount.py"
UNIT = REPO_ROOT / "tools/test_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount.py"
DOC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q18L_LATEST_PREDICTION_SUMMARY_WIDGET_MAPPED_PAYLOAD_VALUE_ROWS_MOUNT_2026-06-22.md"
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q18l_close_guard.py"
EXPECTED_DIRTY = {
    "btcts_next/src/btcts/apps/operator_ui/views/warroom_page.py",
    "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_prediction_summary_widget_mapped_payload_value_rows_panel.py",
    "tools/check_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount.py",
    "tools/test_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount.py",
    "docs/strategy/PREDICTION_SYSTEM_PS_Q18L_LATEST_PREDICTION_SUMMARY_WIDGET_MAPPED_PAYLOAD_VALUE_ROWS_MOUNT_2026-06-22.md",
    "tools/test_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount_guard.py",
    "tools/test_phase4a_prediction_system_ps_q18l_close_guard.py",
}
...
def _assert_boundary(report: dict, failures: list[str]) -> None:
    for key in (
        "read_only",
        "non_executing",
        "latest_prediction_summary_widget_mapped_payload_value_rows_mount_only",
        "warroom_value_rows_ready",
        "value_report_display_only",
...
def main_guard() -> int:
    ...
    for marker in (
        "LATEST_PREDICTION_SUMMARY_WIDGET_MAPPED_PAYLOAD_VALUE_ROWS_PANEL_VERSION",
        "VALUE_ITEMS",
        "build_latest_prediction_summary_widget_mapped_payload_value_rows",
    ...
    for marker in (
        ...
        "build_latest_prediction_summary_widget_mapped_payload_value_rows_packet",
        "def _prediction_warroom_latest_prediction_summary_mapped_payload_value_display_rows(packet: dict) -> list[dict]:",
        "def _render_prediction_warroom_latest_prediction_summary_widget_mapped_payload_value_rows_section() -> None:",
        ...
        "latest summary mapped payload value rows={rows} / values_supplied={values} / q18j_mount=false / render=false / actual_read=false",
        "st.dataframe(value_rows, width=\"stretch\", hide_index=True)",
    ):
        if marker not in page_text:
            failures.append(f"missing page marker: {marker}")
    if page_text.count("_render_prediction_warroom_latest_prediction_summary_widget_mapped_payload_value_rows_section(") != 2:
        failures.append("mapped-payload value rows render function should have definition and page-body call only")
    for forbidden in (
    ...
    for marker in (
        'CHECKER = "ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount"',
        'CHECKER_VERSION = "check_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount.v1"',
        ...
        "q18j_validation_invoked_by_mount",
        "observed_mapped_prediction_run_id",
        "PS-Q18M latest summary value panel close guard",
    ):
        if marker not in tool_text:
            failures.append(f"missing tool marker: {marker}")
    if "test_ps_q18l_validates_mapped_payload_value_rows_from_q18j_fixture" not in unit_text:
        failures.append("unit test must cover Q18J fixture value rows")
    if "test_ps_q18l_page_safe_value_packet_has_no_supplied_values_and_no_invocation" not in unit_text:
        failures.append("unit test must cover page-safe value rows no invocation")
    if CLOSE_REL not in focused_text:
        failures.append("focused guard expected dirty set must include close guard")
    if CHECKER_VERSION != "check_phase4a_prediction_system_ps_q18l_latest_prediction_summary_widget_mapped_payload_value_rows_mount.v1":
        failures.append("checker version mismatch")
    if MAPPED_PAYLOAD_VALUE_ROWS_MOUNT_VERSION != "latest_prediction_summary_widget_mapped_payload_value_rows_mount.v1":
        failures.append("mount version mismatch")

    report = build_report(use_observed_fixture=True)
    if report.get("ok") is not True:
        failures.append(f"observed fixture value rows mount should be ok: {report}")
    ...
    if report.get("value_packet_valid") is not True:
        failures.append("value packet should validate")
    if report.get("page_value_packet_valid") is not True:
        failures.append("page-safe value packet should validate")
    if report.get("value_validation_failures"):
        failures.append(f"value validation failures: {report.get('value_validation_failures')}")
    if report.get("page_value_validation_failures"):
        failures.append(f"page value validation failures: {report.get('page_value_validation_failures')}")
    if report.get("page_validation_failures"):
        failures.append(f"page validation failures: {report.get('page_validation_failures')}")
    if report.get("value_row_count") != 6:
        failures.append("expected 6 observed value rows")
    if report.get("page_value_row_count") != 6:
        failures.append("expected 6 page-safe value rows")
    if report.get("values_supplied") is not True:
        failures.append("observed value rows should have supplied values")
    if report.get("page_values_supplied") is not False:
        failures.append("page-safe value rows should not have supplied values")
    for key, value in {
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_mapped_payload_value_rows_mount_guard":
        failures.append("recommended first validation mismatch")
    _assert_boundary(report, failures)

    page_packet_report = build_report(use_observed_fixture=True)
    if page_packet_report.get("page_values_supplied") is not False:
        failures.append("page-safe path should stay without supplied values")
    blocked = build_report(page_text="")
    if blocked.get("ok") is not False:
        failures.append("missing source should block")
    if blocked.get("value_row_count") != 0:
        failures.append("blocked report should not emit observed value rows")
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q18l_close_guard",
        "phase": "phase3_latest_prediction_summary_widget_mapped_payload_value_rows_mount_closed_before_real_rendering_refresh_and_writes",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q18l_closed": not failures,
            "latest_prediction_summary_widget_mapped_payload_value_rows_mount_only": True,
            "warroom_value_rows_ready": True,
            "value_report_display_only": True,
            "mapped_payload_values_display_only": True,
            "value_row_count": int(report.get("value_row_count") or 0),
            "page_value_row_count": int(report.get("page_value_row_count") or 0),
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q18M latest summary value panel close guard or first real-value display refinement",
        },
...
def test_ps_q18l_close_guard() -> None:
    assert main_guard() == 0
