# path: ./tools/test_phase4a_autotrade_milestone_hl_decision_policy_gate_static_section_model_close_guard.py
# desc: Close guard for S149 decision policy gate static read-only section model. Syntax-checks touched files and reruns focused HL guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hl_decision_policy_gate_static_section_model_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hl_decision_policy_gate_static_section_model_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hl_decision_policy_gate_static_section_model_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HL decision policy gate static section model guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hl_decision_policy_gate_static_section_model_close_guard",
...
def test_decision_policy_gate_static_section_model_close_guard_closes() -> None:
    assert main() == 0
