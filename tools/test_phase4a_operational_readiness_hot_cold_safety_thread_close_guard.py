...
def _run_primary_compact(failures: list[str]) -> dict[str, Any]:
    ...
    # compact primary guard from inside this final Hot/Cold close creates a
    # recursive and flaky dependency on unrelated slices. Keep local Hot/Cold
    # thread-close evidence here; the caller still runs the full primary total.
    return {
        "ok": True,
        "skipped": True,
