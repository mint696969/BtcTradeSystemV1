# path: ./tools/run_sr_fx_pre_armed_dry_run_authorization_request_preflight_status.py
# desc: Broker-free Pre-Armed Dry Run authorization request preflight/status from S78 authorization request/status. Preflight/status only; non-recording; non-executing; non-authorizing.
...
TOOL = "run_sr_fx_pre_armed_dry_run_authorization_request_preflight_status"
REPORT_VERSION = "pre_armed_dry_run_authorization_request_preflight_status.s79.v1"
...
REQUIRED_PREFLIGHT_ACKS = (
    "confirm_s78_authorization_request_status_reviewed",
...
def build_pre_armed_dry_run_authorization_request_preflight_status(*, authorization_request_status: Mapping[str, Any], authorization_request_preflight_review: Mapping[str, Any], generated_at: str | None = None) -> dict[str, Any]:
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build broker-free Pre-Armed Dry Run authorization request preflight/status from S78 status JSON.")
...
def main() -> int:
    ...
    try:
        ...
        payload = build_pre_armed_dry_run_authorization_request_preflight_status(authorization_request_status=status, authorization_request_preflight_review=review)
