# path: ./tools/test_prediction_system_ps_q9h_lowered_display_packet_review_source_handoff_close_guard.py
# desc: Close guard for PS-Q9H lowered display-packet review source handoff.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_lowered_display_packet_visibility_review_source_handoff.py",
    ...
    REPO_ROOT / "tools/test_prediction_system_ps_q9h_lowered_display_packet_review_source_handoff_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9h_lowered_display_packet_review_source_handoff_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9H lowered display-packet review source handoff close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9H lowered display-packet review source handoff close guard")
    return 0


def test_prediction_system_ps_q9h_lowered_display_packet_review_source_handoff_close_guard() -> None:
    assert main() == 0
