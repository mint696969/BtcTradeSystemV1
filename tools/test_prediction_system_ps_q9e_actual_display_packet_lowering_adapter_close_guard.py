# path: ./tools/test_prediction_system_ps_q9e_actual_display_packet_lowering_adapter_close_guard.py
# desc: Close guard for PS-Q9E in-memory actual display-packet lowering adapter.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_actual_display_packet_lowering_adapter.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9e_actual_display_packet_lowering_adapter_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q9e_actual_display_packet_lowering_adapter_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q9e_actual_display_packet_lowering_adapter_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q9E actual display-packet lowering adapter close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q9E actual display-packet lowering adapter close guard")
    return 0


def test_prediction_system_ps_q9e_actual_display_packet_lowering_adapter_close_guard() -> None:
    assert main() == 0
