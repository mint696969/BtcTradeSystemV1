# path: ./tools/test_phase4a_autotrade_milestone_bx_command_status_mode_change_note_filters_command_type_guard.py
# desc: Guard latest_mode_change_readiness_* only uses REQUEST_MODE_CHANGE rows, ignoring non-mode readiness-like notes. Read-only; no append/runner/broker.
...
EXPECTED_MODE_BLOCKED_BY = [
    "mode_off",
    "mode_runtime_gate_blocked_shadow_decision_append",
    "mode_runtime_gate_blocked_forecast_outcome_resolution",
]
EXPECTED_NON_MODE_BLOCKED_BY = ["non_mode_note_should_not_drive_mode_change_readiness"]
...
def readiness_note(*, run_id: str, blocked_by: list[str]) -> str:
    return json.dumps(
        {
            ...
            "blocked_by": ["observer_run_latest_blocked_for_live_target", *blocked_by],
            "warnings": ["guard"],
            "health_state": "warn",
            "observer_run_fresh": True,
            "observer_latest_run_id": run_id,
            "observer_latest_blocked_by": blocked_by,
...
def command_row(*, command_id: str, command_type: str, accepted: bool, requested_at: str, current_mode: str, target: str | None, blocked_by: list[str], note: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "target": target,
            "confirmation": True,
            "reason_codes": ["guard", "mode_change_note_filters_command_type"],
            "note": note,
...
def write_command_rows(path: Path) -> None:
    rows = [
        command_row(
            command_id="cmd_bx_valid_live_request",
            command_type="REQUEST_MODE_CHANGE",
            accepted=False,
            requested_at=now_z(-10),
            current_mode="ARMED_DRY_RUN",
            target="LIVE_MIN_SIZE",
            blocked_by=["readiness_preflight_not_ready", "observer_run_latest_blocked_for_live_target", *EXPECTED_MODE_BLOCKED_BY],
            note=readiness_note(run_id="obs_bx_mode_change_blocked", blocked_by=EXPECTED_MODE_BLOCKED_BY),
        ),
        command_row(
            command_id="cmd_bx_non_mode_with_readiness_like_note",
            ...
            target="halt_new",
            blocked_by=[],
            note=readiness_note(run_id="obs_bx_non_mode_should_not_win", blocked_by=EXPECTED_NON_MODE_BLOCKED_BY),
        ),
    ]
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_command_status_mode_change_note_filters_command_type_hot"
    ...
    checks = {
        "summary_latest_command_is_non_mode_with_readiness_like_note": summary_data.get("latest_command_id") == "cmd_bx_non_mode_with_readiness_like_note" and summary_data.get("latest_command_type") == "REQUEST_HALT_NEW" and summary_data.get("latest_readiness_observer_run_id") == "obs_bx_non_mode_should_not_win",
        "latest_mode_change_readiness_ignores_non_mode_note": summary_data.get("latest_mode_change_readiness_command_id") == "cmd_bx_valid_live_request" and summary_data.get("latest_mode_change_readiness_observer_run_id") == "obs_bx_mode_change_blocked" and tuple(summary_data.get("latest_mode_change_readiness_observer_blocked_by") or ()) == tuple(EXPECTED_MODE_BLOCKED_BY),
        "summarizer_filters_latest_mode_change_by_command_type": "row.command.command_type == CommandType.REQUEST_MODE_CHANGE" in summarize_source,
    ...
    failures.extend(f"protected lower-layer dirty during milestone BX: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_bx_command_status_mode_change_note_filters_command_type_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "summary_latest_command_is_non_mode_with_readiness_like_note": checks["summary_latest_command_is_non_mode_with_readiness_like_note"],
            "latest_mode_change_readiness_ignores_non_mode_note": checks["latest_mode_change_readiness_ignores_non_mode_note"],
            "summarizer_filters_latest_mode_change_by_command_type": checks["summarizer_filters_latest_mode_change_by_command_type"],
