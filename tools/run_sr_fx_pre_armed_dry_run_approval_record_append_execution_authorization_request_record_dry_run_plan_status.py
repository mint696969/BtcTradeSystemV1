# path: ./tools/run_sr_fx_pre_armed_dry_run_approval_record_append_execution_authorization_request_record_dry_run_plan_status.py
# desc: Broker-free Pre-Armed Dry Run authorization request record dry-run plan/status from S63 preflight. Plan/status only; non-recording; non-authorizing.
...
TOOL = "run_sr_fx_pre_armed_dry_run_approval_record_append_execution_authorization_request_record_dry_run_plan_status"
REPORT_VERSION = "pre_armed_dry_run_approval_record_append_execution_authorization_request_record_dry_run_plan_status.v1"
PLAN_SCOPE = "PRE_ARMED_DRY_RUN_AUTHORIZATION_REQUEST_RECORD_PLAN_REVIEW_ONLY"
PLAN_TARGET = "APPROVAL_RECORD_APPEND_EXECUTION_AUTHORIZATION_REQUEST_RECORD_DRY_RUN_PLAN"
RECORD_KIND = "pre_armed_dry_run_append_execution_authorization_request_record_draft"
REQUIRED_PLAN_ACKS = (
    "confirm_s63_record_preflight_status_reviewed",
    "confirm_record_dry_run_plan_is_review_only",
...
def build_pre_armed_dry_run_authorization_request_record_dry_run_plan_status(*, record_preflight_status: Mapping[str, Any], record_plan_review: Mapping[str, Any], generated_at: str | None = None) -> dict[str, Any]:
    preflight = dict(record_preflight_status)
    review = dict(record_plan_review)
    preflight_ok = preflight.get("ok") is True
    preflight_ready = preflight.get("authorization_request_record_preflight_ready") is True
    record_request_observed = preflight.get("authorization_request_recording_requested_observed") is True
    preflight_blockers = _as_list(preflight.get("record_preflight_blockers"))
    source_summary = _as_dict(preflight.get("source_summary"))
    preflight_summary = _as_dict(preflight.get("record_preflight_summary"))
    preflight_safety_ok = (
    ...
    if review.get("authorization_request_record_dry_run_plan_requested") is not True:
        review_blockers.append("authorization_request_record_dry_run_plan_request_required")
    ...
    if review.get("record_dry_run_plan_target") != PLAN_TARGET:
        review_blockers.append("record_dry_run_plan_target_not_authorization_request_record_plan")
    ...
    if review.get("authorization_request_recorded") is not False:
        review_blockers.append("authorization_request_recorded_must_be_false_in_plan")
    if review.get("approval_record_append_execution_authorized") is not False:
        review_blockers.append("authorization_grant_must_be_false_in_plan")
    if review.get("approval_record_append_execution_requested") is not False:
        review_blockers.append("append_execution_request_must_be_false_in_plan")
    if review.get("approval_record_append_executed") is not False:
        review_blockers.append("append_execution_must_be_false_in_plan")
    if review.get("approval_ledger_append_requested") is not False:
        review_blockers.append("approval_ledger_append_request_must_be_false_in_plan")
    if review.get("command_ledger_append_requested") is not False:
        review_blockers.append("command_ledger_append_request_must_be_false_in_plan")
    if review.get("mode_change_requested") is not False:
        review_blockers.append("mode_change_request_must_be_false_in_plan")
    review_blockers.extend(f"missing_record_plan_ack:{ack}" for ack in missing_acks)
    plan_blockers: list[str] = []
    if not preflight_ok:
        plan_blockers.append("authorization_request_record_preflight_status_not_ok")
    if not preflight_ready:
        plan_blockers.append("authorization_request_record_preflight_not_ready")
        plan_blockers.extend(preflight_blockers)
    if not record_request_observed:
        plan_blockers.append("authorization_request_recording_request_not_observed")
    if not preflight_safety_ok:
        plan_blockers.append("authorization_request_record_preflight_safety_contract_not_clear")
    if preflight_summary.get("record_preflight_scope") != "PRE_ARMED_DRY_RUN_AUTHORIZATION_REQUEST_RECORD_REVIEW_ONLY":
        plan_blockers.append("source_record_preflight_scope_not_review_only")
    if preflight_summary.get("record_preflight_target") != "APPROVAL_RECORD_APPEND_EXECUTION_AUTHORIZATION_REQUEST_RECORD":
        plan_blockers.append("source_record_preflight_target_not_authorization_request_record")
    plan_blockers.extend(review_blockers)
    plan_blockers = _dedupe(plan_blockers)
    plan_ready = bool(preflight_ok and preflight_ready and record_request_observed and preflight_safety_ok and not review_blockers and not plan_blockers)
    evidence_id = str(source_summary.get("evidence_id") or "missing_evidence")
    record_id = "authorization_request_record_dry_run_plan_" + evidence_id
    decision = "authorization_request_record_dry_run_plan_ready_not_recorded" if plan_ready else "authorization_request_record_dry_run_plan_blocked_not_recorded"
    return {
        ...
        "decision": decision,
        "authorization_request_record_dry_run_plan_ready": plan_ready,
        "record_dry_run_plan_blockers": plan_blockers,
        "authorization_request_record_dry_run_plan_requested_observed": review.get("authorization_request_record_dry_run_plan_requested") is True,
        ...
        "authorization_request_record_draft": {
            ...
            "source_authorization_requested_by": source_summary.get("requested_by"),
            "source_authorization_requested_at": source_summary.get("requested_at"),
        ...
        "record_dry_run_plan_summary": {
            ...
            "plan_request_input": review.get("authorization_request_record_dry_run_plan_requested"),
        ...
        "source_summary": {
            ...
            "recording_request_observed": record_request_observed,
        ...
        "checks": {
            ...
            "record_dry_run_plan_blockers_visible_when_blocked": bool(plan_blockers) if not plan_ready else True,
        ...
        "warnings": [
            "authorization_request_record_dry_run_plan_status_is_informational_only",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build broker-free Pre-Armed Dry Run authorization request record dry-run plan/status from S63 JSON.")
    parser.add_argument("--record-preflight-status", required=True, help="Path to S63 authorization request record preflight/status JSON.")
    parser.add_argument("--record-plan-review", required=True, help="Path to record dry-run plan review JSON. This slice records nothing.")
    parser.add_argument("--out", default="", help="Optional record dry-run plan/status output JSON path.")
    return parser
...
def main() -> int:
    ...
    try:
        ...
        review = _read_json(Path(args.record_plan_review))
        payload = build_pre_armed_dry_run_authorization_request_record_dry_run_plan_status(record_preflight_status=preflight, record_plan_review=review)
        ...
        payload = {
            ...
            "blocked_by": ["pre_armed_dry_run_authorization_request_record_dry_run_plan_status_failed"],
            "read_only": True,
            "would_send_to_broker": False,
            "mode_changed": False,
            "autotrade_resume_authorized": False,
