# path: ./tools/test_phase4a_prediction_system_ps_q17r_close_guard.py
# desc: Close guard for PS-Q17R WarRoom prediction widget read-only component skeleton contract.
...
FOCUSED_GUARD = "tools/test_phase4a_prediction_system_ps_q17r_warroom_prediction_widget_read_only_component_skeleton_contract_guard.py"
CLOSE_REL = "tools/test_phase4a_prediction_system_ps_q17r_close_guard.py"
EXPECTED_DIRTY = {
...
def _assert_false_boundaries(report: dict, failures: list[str]) -> None:
    for key in (
        "read_only",
        "non_executing",
        "component_skeleton_contract_only",
        "contract_only",
        "diagnostic_only",
...
def main_guard() -> int:
    ...
    if "test_ps_q17r_builds_component_skeleton_contract_from_q17q_fixture" not in unit_text:
        failures.append("unit test must cover Q17Q fixture to component skeleton contract")
    if "test_ps_q17r_keeps_component_file_import_render_and_actual_read_false" not in unit_text:
        failures.append("unit test must cover component/import/render/read false")
    ...
    if report.get("stage") != "warroom_prediction_widget_read_only_component_skeleton_contract_before_component_file_creation_import_and_rendering":
        failures.append("stage mismatch")
    if report.get("source_q17q_report_valid") is not True:
        failures.append("source Q17Q report should validate")
    ...
    if report.get("recommended_first_validation") != "latest_prediction_summary_widget_component_skeleton_contract_guard":
    ...
    result = {
        "ok": not failures,
        "guard": "ps_q17r_close_guard",
        "phase": "phase3_warroom_prediction_widget_component_skeleton_contract_closed_before_component_file_creation_import_and_rendering",
        "focused_guards_to_run_before_commit": [FOCUSED_GUARD],
        "contract": {
            "ps_q17r_closed": not failures,
            "component_skeleton_contract_only": True,
            "component_file_creation_allowed": False,
            "component_import_allowed": False,
            "streamlit_render_allowed": False,
            ...
            "no_autotrade": True,
            "no_broker": True,
            "next_slice": "PS-Q17S WarRoom prediction widget read-only component skeleton implementation or actual-source preflight",
        },
...
def test_ps_q17r_close_guard() -> None:
    assert main_guard() == 0
