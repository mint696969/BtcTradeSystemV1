# path: ./tools/test_phase4a_autotrade_milestone_bs_command_status_readiness_observer_note_visibility_guard.py
# desc: Guard Command Requests status/UI surfaces readiness observer details from command.note. Read-only; no append/runner/broker.
...
def write_command_row(path: Path) -> None:
    note = {
        ...
        "observer_latest_run_id": "obs_bs_blocked_off",
    ...
    row = {
        ...
        "command_id": "cmd_bs_live_request",
        "accepted": False,
        ...
        "command": {
            ...
            "confirmation": True,
            "reason_codes": ["guard", "readiness_preflight"],
            "note": json.dumps(note, ensure_ascii=False, sort_keys=True),
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_command_status_readiness_observer_note_visibility_hot"
    ...
    try:
        ...
        if command_path.exists():
            command_path.unlink()
        write_command_row(command_path)
    ...
    checks = {
        "summary_dataclass_has_readiness_observer_fields": all(field in status_source for field in NOTE_FIELDS),
        "summary_parses_latest_note_observer_fields": summary_data.get("latest_readiness_observer_run_id") == "obs_bs_blocked_off" and tuple(summary_data.get("latest_readiness_observer_blocked_by") or ()) == tuple(EXPECTED_BLOCKED_BY) and summary_data.get("latest_readiness_observer_would_send_to_broker") is False and summary_data.get("latest_readiness_observer_bounded") is True,
        "ui_command_requests_displays_readiness_observer_fields": all(field in ui_source for field in NOTE_FIELDS),
        "summary_read_only_no_append_runner_broker": bool(summarize_source) and not any(token in status_source for token in FORBIDDEN_STATUS_TOKENS),
        ...
        "summary_did_not_append_command": before_count == 1 and after_summary_count == before_count,
        "render_did_not_append_command": before_count == 1 and after_render_count == before_count,
    ...
    failures.extend(f"protected lower-layer dirty during milestone BS: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_bs_command_status_readiness_observer_note_visibility_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "summary_dataclass_has_readiness_observer_fields": checks["summary_dataclass_has_readiness_observer_fields"],
            "summary_parses_latest_note_observer_fields": checks["summary_parses_latest_note_observer_fields"],
            "ui_command_requests_displays_readiness_observer_fields": checks["ui_command_requests_displays_readiness_observer_fields"],
