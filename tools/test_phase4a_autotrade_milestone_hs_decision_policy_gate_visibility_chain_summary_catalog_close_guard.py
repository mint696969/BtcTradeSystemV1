# path: ./tools/test_phase4a_autotrade_milestone_hs_decision_policy_gate_visibility_chain_summary_catalog_close_guard.py
# desc: Close guard for S156 decision policy gate visibility chain summary catalog entry. Syntax-checks touched files and reruns focused HS guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hs_decision_policy_gate_visibility_chain_summary_catalog_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hs_decision_policy_gate_visibility_chain_summary_catalog_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hs_decision_policy_gate_visibility_chain_summary_catalog_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HS decision policy gate visibility chain summary catalog guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hs_decision_policy_gate_visibility_chain_summary_catalog_close_guard",
...
def test_decision_policy_gate_visibility_chain_summary_catalog_close_guard_closes() -> None:
    assert main() == 0
