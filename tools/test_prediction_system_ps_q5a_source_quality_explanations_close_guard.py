# path: ./tools/test_prediction_system_ps_q5a_source_quality_explanations_close_guard.py
# desc: Close guard for PS-Q5A Prediction WarRoom source-quality explanation panel.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_source_quality_explanations.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q5a_source_quality_explanations_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q5a_source_quality_explanations_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q5a_source_quality_explanations_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q4d_warroom_sample_packets_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q4d_warroom_sample_packets_close_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q5A source-quality explanations close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q5A source-quality explanations close guard")
    return 0


def test_prediction_system_ps_q5a_source_quality_explanations_close_guard() -> None:
    assert main() == 0
