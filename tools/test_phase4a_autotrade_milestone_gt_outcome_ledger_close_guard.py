# path: ./tools/test_phase4a_autotrade_milestone_gt_outcome_ledger_close_guard.py
# desc: Close guard for S131 outcome ledger contracts. Syntax-checks touched files and reruns focused GT guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "btcts_next/src/btcts/prediction/outcome_ledger.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gt_outcome_ledger_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gt_outcome_ledger_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_gt_outcome_ledger_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused GT outcome ledger guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_gt_outcome_ledger_close_guard",
