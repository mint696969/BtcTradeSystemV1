# path: ./tools/test_prediction_system_ps_q6a_latest_payload_preflight_status_close_guard.py
# desc: Close guard for PS-Q6A latest payload preflight status contract.
...
TOUCHED = (
    REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/prediction_warroom_latest_payload_preflight_status.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q6a_latest_payload_preflight_status_guard.py",
    REPO_ROOT / "tools/test_prediction_system_ps_q6a_latest_payload_preflight_status_close_guard.py",
)
GUARDS = (
    REPO_ROOT / "tools/test_prediction_system_ps_q6a_latest_payload_preflight_status_guard.py",
...
def main() -> int:
    ...
    if failures:
        print("[FAIL] Prediction System PS-Q6A latest payload preflight status close guard")
        for failure in failures:
            print(f"- {failure}")
        return 1
    print("[OK] Prediction System PS-Q6A latest payload preflight status close guard")
    return 0


def test_prediction_system_ps_q6a_latest_payload_preflight_status_close_guard() -> None:
    assert main() == 0
