# path: ./tools/test_phase4a_autotrade_milestone_cn_rechecked_preview_cli_candidate_note_fail_soft_guard.py
# desc: Guard rechecked preview CLI candidate readiness note parsing is fail-soft and read-only. No command/mode/observer append, runner, or broker.
...
def command_row(command_id: str, note: str, *, blocked_by: list[str]) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "confirmation": True,
            "reason_codes": ["guard", "preview_cli_candidate_note_fail_soft"],
            "note": note,
...
def run_case(*, case_name: str, note: str, blocked_by: str, run_twice: bool, failures: list[str]) -> dict[str, Any]:
    hot_root = REPO_ROOT / f"tmp/btc_ts_rechecked_preview_cli_candidate_note_fail_soft_{case_name}_hot"
    ...
    command_id = f"cmd_cn_candidate_{case_name}_note"
...
def first_result_is_fail_soft_preview(case: dict[str, Any]) -> bool:
...
def first_result_rejected_preview_and_read_only(case: dict[str, Any]) -> bool:
    data = case.get("first_result") or {}
    return (
        case.get("first_returncode") == 2
        and data.get("would_apply") is False
        and data.get("would_reject_by_readiness") is True
        ...
        and case.get("after_first_mode_count") == 0
        and case.get("observer_count_after_first") == case.get("observer_count_before")
        and data.get("read_only") is True
    )


def second_result_remains_same_read_only_candidate(case: dict[str, Any]) -> bool:
    data = case.get("second_result") or {}
    return (
        case.get("second_returncode") == 2
        and data.get("command_id") == case.get("command_id")
        and data.get("would_apply") is False
        and data.get("would_reject_by_readiness") is True
        and data.get("already_applied_command_ids") == []
        and data.get("candidate_command_count") == 1
        and data.get("read_only") is True
        and case.get("after_second_command_count") == case.get("before_command_count")
        and case.get("after_second_mode_count") == case.get("before_mode_count")
...
def main() -> int:
    ...
    repeat_case = next(case for case in cases if case.get("case_name") == "array")
    checks = {
        ...
        "cli_cases_return_preview_rejection_exit_code_expected": all(case.get("first_returncode") == 2 for case in cases),
        "cli_cases_ignore_malformed_wrong_kind_and_array_notes": all(first_result_is_fail_soft_preview(case) for case in cases),
        "cli_cases_recheck_preview_still_runs_with_bad_candidate_note": all((case.get("first_result") or {}).get("readiness") is not None and "readiness_recheck_not_ready" in tuple((case.get("first_result") or {}).get("blocked_by") or ()) for case in cases),
        "cli_cases_preview_does_not_append_anything": all(first_result_rejected_preview_and_read_only(case) for case in cases),
        "cli_single_candidate_second_preview_keeps_same_candidate_read_only": second_result_remains_same_read_only_candidate(repeat_case),
        "cli_output_keeps_candidate_note_fields_present": all(all(field in data for field in CANDIDATE_NOTE_FIELDS) for data in first_results) and all(field in (repeat_case.get("second_result") or {}) for field in CANDIDATE_NOTE_FIELDS),
        "cli_uses_result_to_dict": "result.to_dict()" in cli_source and "json.dumps" in cli_source,
        "cli_and_preview_no_runner_or_broker_or_append": not any(token in cli_source for token in FORBIDDEN_CLI_TOKENS) and bool(preview_source) and not any(token in preview_source for token in FORBIDDEN_PREVIEW_TOKENS),
    }
    ...
    failures.extend(f"protected lower-layer dirty during milestone CN: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_cn_rechecked_preview_cli_candidate_note_fail_soft_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "candidate_note_payload_helper_is_fail_soft": checks["candidate_note_payload_helper_is_fail_soft"],
            "cli_cases_return_preview_rejection_exit_code_expected": checks["cli_cases_return_preview_rejection_exit_code_expected"],
            "cli_cases_ignore_malformed_wrong_kind_and_array_notes": checks["cli_cases_ignore_malformed_wrong_kind_and_array_notes"],
            "cli_cases_recheck_preview_still_runs_with_bad_candidate_note": checks["cli_cases_recheck_preview_still_runs_with_bad_candidate_note"],
            "cli_cases_preview_does_not_append_anything": checks["cli_cases_preview_does_not_append_anything"],
            "cli_single_candidate_second_preview_keeps_same_candidate_read_only": checks["cli_single_candidate_second_preview_keeps_same_candidate_read_only"],
            ...
            "cli_and_preview_no_runner_or_broker_or_append": checks["cli_and_preview_no_runner_or_broker_or_append"],
