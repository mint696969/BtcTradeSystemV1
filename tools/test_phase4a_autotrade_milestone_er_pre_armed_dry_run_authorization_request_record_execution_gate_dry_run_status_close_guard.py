# path: ./tools/test_phase4a_autotrade_milestone_er_pre_armed_dry_run_authorization_request_record_execution_gate_dry_run_status_close_guard.py
# desc: Close guard for S77 Pre-Armed Dry Run authorization request record execution gate dry-run/status. Broker-free; non-recording; non-executing; non-authorizing.
...
SELF_PATH = "tools/test_phase4a_autotrade_milestone_er_pre_armed_dry_run_authorization_request_record_execution_gate_dry_run_status_close_guard.py"
S77_RUN_PATH = "tools/run_sr_fx_pre_armed_dry_run_authorization_request_record_execution_gate_dry_run_status.py"
...
EXPECTED_SLICE_FILES = {S77_RUN_PATH, S77_GUARD_PATH, SELF_PATH}
...
REQUIRED_RUN_FRAGMENTS = (
    ...
    "record_execution_gate_blockers",
...
def _run_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"S77 run missing required fragment: {frag}" for frag in missing)
    failures.extend(f"S77 run contains forbidden token: {tok}" for tok in forbidden)
    failures.extend(f"S77 run missing safety literal: {frag}" for frag in false_missing)
...
def _git_boundary(failures: list[str]) -> dict[str, object]:
    ...
    failures.extend(f"unexpected dirty file during S77 close: {line}" for line in unexpected)
    return {"lines": lines, "unexpected": unexpected, "expected_slice_files": sorted(EXPECTED_SLICE_FILES)}


def main() -> int:
    failures: list[str] = []
    syntax = {p: _syntax(p, failures) for p in (SELF_PATH, S77_RUN_PATH, S77_GUARD_PATH, S76_GUARD_PATH, COMMAND_STATUS_PATH)}
    ...
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_er_pre_armed_dry_run_authorization_request_record_execution_gate_dry_run_status_close_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "s77_execution_gate_status_present": (REPO_ROOT / S77_RUN_PATH).exists(),
            "s77_focused_guard_closed": s77_guard.get("ok") is True,
            "s76_eq_guard_closed": s76_guard.get("ok") is True,
        ...
        "guards": {"s77_er_guard": s77_guard, "s76_eq_guard": s76_guard},
