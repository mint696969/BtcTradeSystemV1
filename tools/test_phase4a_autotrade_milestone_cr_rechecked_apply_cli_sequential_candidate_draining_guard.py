# path: ./tools/test_phase4a_autotrade_milestone_cr_rechecked_apply_cli_sequential_candidate_draining_guard.py
# desc: Guard rechecked apply CLI drains multiple accepted REQUEST_MODE_CHANGE candidates latest-first, one per invocation. No command/observer append, runner, or broker.
...
EXPECTED_APPLY_SEQUENCE = (
    "cmd_cr_latest_live_request",
    "cmd_cr_middle_live_request",
    "cmd_cr_oldest_live_request",
)
    ...
    return json.dumps(
        {
            ...
            "warnings": ["cli_sequential_candidate_draining_snapshot"],
            "health_state": "warn",
...
def command_row(command_id: str, requested_at: str, blocker: str) -> dict[str, Any]:
    return {
        ...
        "command": {
            ...
            "confirmation": True,
            "reason_codes": ["guard", "cli_sequential_candidate_draining", command_id],
...
def main() -> int:
    ...
    hot_root = REPO_ROOT / "tmp/btc_ts_rechecked_apply_cli_sequential_candidate_draining_hot"
    ...
    try:
        ...
        write_jsonl(
            command_path,
            [
                command_row("cmd_cr_oldest_live_request", now_z(-30), "cr_oldest_candidate_blocker"),
                command_row("cmd_cr_middle_live_request", now_z(-20), "cr_middle_candidate_blocker"),
                command_row("cmd_cr_latest_live_request", now_z(-10), "cr_latest_candidate_blocker"),
            ],
        )
        env = os.environ.copy()
        ...
        for index in range(4):
            ...
            stderrs.append(proc.stderr)
            results.append(parse_stdout(f"run{index + 1}", proc.stdout, failures))
    ...
    skipped_result = results[3] if len(results) >= 4 else {}
    actual_apply_sequence = tuple(result.get("command_id") for result in first_three)
    ...
    expected_already_applied_before_each = (
        (),
        ("cmd_cr_latest_live_request",),
        ("cmd_cr_latest_live_request", "cmd_cr_middle_live_request"),
    )
    expected_candidate_counts = (3, 2, 1)
    checks = {
        "cli_invocations_return_expected_exit_codes": tuple(returncodes) == (2, 2, 2, 2),
        "cli_selects_latest_unapplied_candidate_each_run": actual_apply_sequence == EXPECTED_APPLY_SEQUENCE and tuple(result.get("candidate_command_count") for result in first_three) == expected_candidate_counts,
        "cli_tracks_already_applied_ids_before_each_run": tuple(tuple(result.get("already_applied_command_ids") or ()) for result in first_three) == expected_already_applied_before_each,
        "cli_stdout_includes_candidate_context_each_rejection": all(all(field in result for field in CANDIDATE_CONTEXT_FIELDS) and result.get("candidate_command_type") == "REQUEST_MODE_CHANGE" and result.get("candidate_requested_by") == "operator_ui" and result.get("candidate_readiness_note_present") is True for result in first_three),
        "each_cli_rejection_appends_one_mode_state_record": before_mode_count == 0 and after_mode_count == 3 and all(result.get("record_appended") is True and result.get("rejected_by_readiness") is True and result.get("skip_reason") == "readiness_recheck_not_ready" and (result.get("mode_state_record") or {}).get("source_command_id") == result.get("command_id") for result in first_three),
        "mode_state_source_command_ids_follow_cli_sequence": actual_mode_state_sequence == EXPECTED_APPLY_SEQUENCE,
        ...
        "mode_state_summary_latest_is_oldest_after_drain": mode_state_summary.get("latest_source_command_id") == "cmd_cr_oldest_live_request" and mode_state_summary.get("latest_ledger_event") == "autotrade.mode_state_readiness_recheck_rejected" and (mode_state_summary.get("blocked_by_counts") or {}).get("readiness_recheck_not_ready") == 3,
        ...
        "cli_and_apply_latest_unapplied_one_shot_no_runner_broker": not any(token in cli_source for token in CLI_FORBIDDEN_TOKENS) and bool(apply_source) and "candidates[-1]" in apply_source and "row.command_id not in already_applied" in apply_source and not any(token in apply_source for token in APPLY_FORBIDDEN_TOKENS),
    }
    ...
    failures.extend(f"protected lower-layer dirty during milestone CR: {hit}" for hit in protected_dirty_hits)

    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_cr_rechecked_apply_cli_sequential_candidate_draining_guard",
        "status": "closed" if not failures else "open",
        "contract": {
            "cli_invocations_return_expected_exit_codes": checks["cli_invocations_return_expected_exit_codes"],
            "cli_selects_latest_unapplied_candidate_each_run": checks["cli_selects_latest_unapplied_candidate_each_run"],
            "cli_tracks_already_applied_ids_before_each_run": checks["cli_tracks_already_applied_ids_before_each_run"],
            "cli_stdout_includes_candidate_context_each_rejection": checks["cli_stdout_includes_candidate_context_each_rejection"],
            "each_cli_rejection_appends_one_mode_state_record": checks["each_cli_rejection_appends_one_mode_state_record"],
            "mode_state_source_command_ids_follow_cli_sequence": checks["mode_state_source_command_ids_follow_cli_sequence"],
            "mode_state_records_keep_rejection_metadata": checks["mode_state_records_keep_rejection_metadata"],
            "final_cli_apply_skips_after_all_candidates_drained": checks["final_cli_apply_skips_after_all_candidates_drained"],
            ...
            "cli_and_apply_latest_unapplied_one_shot_no_runner_broker": checks["cli_and_apply_latest_unapplied_one_shot_no_runner_broker"],
