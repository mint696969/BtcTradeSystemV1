# path: ./tools/test_phase4a_dashboard_hub_display_source_registry_guard.py
# desc: Guard dashboard hub display source registry. No Streamlit/app routing/layout/runtime/broker wiring.
...
OPERATOR_CATALOG_GUARD = "tools/test_phase4a_operator_display_source_catalog_guard.py"
FILES = [
...
def _compile(rel_path: str, failures: list[str]) -> dict[str, object]:
    ...
    cache = REPO_ROOT / "tmp" / "_guard_py_compile_cache" / "dashboard_hub_display_source_registry"
...
def _check_source(failures: list[str]) -> dict[str, object]:
    ...
    for fragment in missing:
        failures.append(f"dashboard hub display source registry missing fragment: {fragment}")
    forbidden_hits = [token for token in FORBIDDEN_SOURCE_TOKENS if token in joined]
    for token in forbidden_hits:
        failures.append(f"dashboard hub display source registry contains forbidden token: {token}")
...
def main() -> int:
    failures: list[str] = []
    checks = {
        "compile_self": _compile("tools/test_phase4a_dashboard_hub_display_source_registry_guard.py", failures),
        "compile_files": {rel: _compile(rel, failures) for rel in FILES},
        "operator_catalog_guard": _run_json_guard(OPERATOR_CATALOG_GUARD, failures),
        "plain_test": _run_plain_ok("btcts_next/src/btcts/apps/operator_ui/tests/test_dashboard_hub_display_source_registry.py", failures),
        "source": _check_source(failures),
    }
    payload = {
        "ok": not failures,
        "phase": "phase4a_dashboard_hub_display_source_registry_guard",
        "failures": failures,
        "checks": checks,
    }
