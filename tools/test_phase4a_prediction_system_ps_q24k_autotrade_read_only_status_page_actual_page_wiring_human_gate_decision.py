# path: ./tools/test_phase4a_prediction_system_ps_q24k_autotrade_read_only_status_page_actual_page_wiring_human_gate_decision.py
# desc: Focused pytest guard for PS-Q24K human-gate decision with page wiring still blocked.
...
SPEC = REPO_ROOT / "docs/strategy/PREDICTION_SYSTEM_PS_Q24K_AUTOTRADE_READ_ONLY_STATUS_PAGE_ACTUAL_PAGE_WIRING_HUMAN_GATE_DECISION_2026-06-29.md"
MODULE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/components/autotrade_prediction_status_page_human_gate_decision.py"
DIAG = REPO_ROOT / "tools/diagnose_phase4a_prediction_system_ps_q24k_autotrade_read_only_status_page_actual_page_wiring_human_gate_decision.py"
AUTOTRADE_PAGE = REPO_ROOT / "btcts_next/src/btcts/apps/operator_ui/views/autotrade_page.py"


def test_spec_declares_q24k_human_gate_decision_contract() -> None:
...
def test_live_q24k_human_gate_decision_not_granted_ready() -> None:
    ...
    assert result["q24j_actual_page_wiring_patch_plan"]["ready"] is True
    packet = result["human_gate_decision"]
    assert packet["decision_type"] == "autotrade_prediction_status_page_actual_page_wiring_human_gate_decision_packet"
    assert packet["human_gate_decision_ready"] is True
    assert packet["human_gate_decision_packet_available"] is True
    ...
    assert packet["human_gate_decision_packet_only"] is True
    ...
    assert safety["human_gate_decision_packet_only"] is True
...
def test_autotrade_page_is_not_modified_or_wired_by_q24k_slice() -> None:
    ...
    assert "autotrade_prediction_status_page_human_gate_decision" not in page_text
    assert "build_autotrade_prediction_status_page_human_gate_decision_packet" not in page_text
...
if __name__ == "__main__":
    test_spec_declares_q24k_human_gate_decision_contract()
    test_live_q24k_human_gate_decision_not_granted_ready()
    test_autotrade_page_is_not_modified_or_wired_by_q24k_slice()
