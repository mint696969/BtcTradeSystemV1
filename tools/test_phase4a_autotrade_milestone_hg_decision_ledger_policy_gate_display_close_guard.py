# path: ./tools/test_phase4a_autotrade_milestone_hg_decision_ledger_policy_gate_display_close_guard.py
# desc: Close guard for S144 decision ledger policy gate display packet. Syntax-checks touched files and reruns focused HG guard.
...
TOUCHED = (
    ...
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hg_decision_ledger_policy_gate_display_guard.py",
    REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hg_decision_ledger_policy_gate_display_close_guard.py",
)
GUARD = REPO_ROOT / "tools/test_phase4a_autotrade_milestone_hg_decision_ledger_policy_gate_display_guard.py"
...
def main() -> int:
    ...
    if guard_proc.returncode != 0 or guard_payload.get("ok") is not True:
        failures.append("focused HG decision ledger policy gate display guard did not close")
    payload = {
        "ok": not failures,
        "phase": "phase4a_autotrade_milestone_hg_decision_ledger_policy_gate_display_close_guard",
...
def test_decision_ledger_policy_gate_display_close_guard_closes() -> None:
    assert main() == 0
