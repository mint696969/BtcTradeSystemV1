...
2. モード構成と動作
   ...
   OFF 通常運用・監査停止 なし なし 最軽量。生成抑止。
   DEBUG 開発調整・通常観測 WARN 以上のみ Lite スナップショット自動生成 普段使い。
   BOOST 詳細調査・デバッグ集中 すべて（DEBUG 含む） Full スナップショット自動生成＋ DOM プローブ 問題再現・解析用。
   2.1 モード切替のトリガ

DEBUG / BOOST への切替時に 1 回だけスナップショット生成。

OFF では生成を抑止。

同一モード内では N 秒以内の再生成を抑制（多重生成防止）。
...
DEBUG → Lite Snapshot 自動生成

BOOST → Full Snapshot 自動生成（DOM プローブ付き）

OFF → 生成なし

生成結果を st.code() で表示

下部に ⧉ Copy ボタン（クリックで内容をクリップボードへ）
