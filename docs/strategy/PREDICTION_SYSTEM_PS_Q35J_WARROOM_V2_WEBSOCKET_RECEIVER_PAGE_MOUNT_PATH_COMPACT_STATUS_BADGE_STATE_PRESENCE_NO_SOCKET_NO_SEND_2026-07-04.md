# path: ./docs/strategy/PREDICTION_SYSTEM_PS_Q35J_WARROOM_V2_WEBSOCKET_RECEIVER_PAGE_MOUNT_PATH_COMPACT_STATUS_BADGE_STATE_PRESENCE_NO_SOCKET_NO_SEND_2026-07-04.md
# desc: PS-Q35J WarRoom v2 WebSocket receiver page-mount compact status badge state presence. One visible line, no socket open, and no send.

# PS-Q35J WarRoom v2 WebSocket receiver page-mount compact status badge state presence no-socket no-send

Date: 2026-07-04
Profile: BtcTradeSystem
Base gate: PS_Q35I_WARROOM_V2_WEBSOCKET_RECEIVER_PAGE_MOUNT_PATH_COMPACT_STATUS_BADGE_READBACK_STATUS_NO_SOCKET_NO_SEND_DONE
Slice: PS-Q35J_WARROOM_V2_WEBSOCKET_RECEIVER_PAGE_MOUNT_PATH_COMPACT_STATUS_BADGE_STATE_PRESENCE_NO_SOCKET_NO_SEND

## Decision

PS-Q35J adds a compact receiver state presence label to the existing one-line badge. The badge now reports whether the receiver state readback is present, missing, or unknown, while preserving the existing readback status and message count.
...
compact_status_badge_state_presence_version=prediction_warroom.v2.transport.ws_receiver_only_client_page_mount_path_compact_status_badge_state_presence.ps_q35j.v1
...
rendered_line_template=`WS Receiver` mount ready · state={present|missing|unknown} · readback={ready|blocked|unknown} · msgs={receiver_state_message_count} · no socket/send
...
```text
`WS Receiver` mount ready · state={present|missing|unknown} · readback={ready|blocked|unknown} · msgs={receiver_state_message_count} · no socket/send
```
...
- Missing hidden observation displays state=unknown and readback=unknown.
- Present receiver state displays state=present.
- Missing receiver state displays state=missing.
...
- transport/__init__.py and v2/__init__.py are not modified for Q35J exports.
```
